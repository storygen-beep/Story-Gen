...
8. **The wiring contract — this arc's place in the machine** (`redesign_phase_3/22` / `step-2-toplevel.md`
   §7). Record the player-namespaced signals this arc **SETS** (the milestone flags / `<this_npc>_stage`
   other arcs will read — e.g. the first-night sets `<npc>_opened_up`; taking the bar sets `bar_seized`) and
   the ones it **READS** (another arc's flag/stage that gates one of *this* arc's **mid/late** rungs). This is
   where the casting **place in the machine** + the rough-sketch threads become concrete wires. Bound by the
   three disciplines: **D1** never gate this arc's ENTRY on another arc (a cross-read gates a mid/late rung or
   capstone, never the on-ramp — the on-ramp is the ungated non-lewd meeting, §binding 2); **D2** the read
   points at *another* arc, never a mutual lock (the roster runs the DAG check); **D3** every cross-gated
   rung ships a **locked-visible telegraph naming the other arc's state** (`14` L7). Mechanism: milestone → a
   shared **player flag**; "how far" → the **`<npc>_stage` player trait**; a wage/price banded by an NPC's
   trait → **band-gated sibling choices** (`beat-authoring.md`) — never a raw cross-NPC trait read. (Forms
   1+2; a *finished* arc producing income/capability = **G6**, deferred.)
...
6. **The wiring contract (§8 above) is part of the brief** (`redesign_phase_3/22`). Seed it from the casting
   **place in the machine**: write which player flags / `<npc>_stage` this arc SETS and READS, under D1–D3.
   This is what makes the cast a *machine* instead of islands — and it's authored as ordinary `cross_npc` /
   `economic` beats with `deps`, no new schema. (Form 3 — a finished arc *producing* a resource — is **G6**.)
...
- **Wiring contract written** (§8 / `22`): the player flags + `<npc>_stage` this arc SETS and READS are
  recorded; every cross-**read** gates a mid/late rung (D1 — never entry), points at another arc (D2 — no
  cycle), and is telegraphed naming that arc's state (D3 — `14` L7). Never a raw cross-NPC trait read.
