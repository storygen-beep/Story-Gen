...
3. **Doctrine self-audit** — check each against what THIS beat authored (cite the doctrine in
   ...
   - **machine wiring — `cross_npc` / `economic` beats (`redesign_phase_3/22`)** — if this beat carries a
     ...
     cold-start bullet above); **D2** it reads *another* arc's signal (no mutual lock — the roster traced the
     DAG); **D3** the gated rung is **locked-visible with `locked_text` naming the other arc's state** ("Sal
