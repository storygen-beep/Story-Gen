...
**Place in the machine (core NPCs — `step-2-toplevel.md` §7 / `redesign_phase_3/22`).** Step 2 designed the
...
(`22`) bound it: a wire constrains an arc's *depth ordering*, never its *availability* (D1).
...
**Place in machine** = one phrase for each core NPC's node in the core loop (`22`); blank for peripherals.
...
- **Machine placed** (`22`): the core loop is named and **every core NPC has a place in it** — the cast isn't
  a set of islands. (Full DAG + legibility check happens at the roster, `step-5-roster.md`; here it's just
  "is each core NPC wired in at all?")
...
## Cross-references
`redesign_phase_3/06` (full detail) · `redesign_phase_3/22` + `references/step-2-toplevel.md` §7 (the machine —
the "place in the machine" each core NPC gets) · `references/trait-design.md` (the arc-shape → spine menu each
hook implies) · `references/lanes.md` (per-arc-shape budgets) · Step 4 = `references/step-4-npc-arcs.md`
(builds the per-NPC arc on each cast row; its §8 = the arc's wiring contract).
