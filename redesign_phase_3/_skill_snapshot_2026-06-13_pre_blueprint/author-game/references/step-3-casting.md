...
- **Machine placed** (`22`): the core loop is named and **every core NPC has a place in it** — the cast isn't
  a set of islands. (Full DAG + legibility check happens at the feedback review, `step-5-feedback.md`; here
  it's just "is each core NPC wired in at all?")
...
(Pass 2 builds the per-NPC arc on each cast row; its §8 = the arc's wiring contract).
