...
2. **Seed the ledger's structure** — fill `structure_registry` (every location/npc/flag the scaffold
   declares, each location tagged reachable/locked/offscreen) and `plan` (one beat per desire-ladder want /
   subject-brief scene, `id = beat_0001…`, `status = planned`, `target_phase` set, `introduces` filled) + `next_up`.
...
1. **Propose the next beat — ideate, then ask-or-inform (`references/run-mode.md`).** Take the head of
   `next_up` (or let the user pick / inject a new beat). **A beat fills a scene from a subject brief in
   `design_book.md`** (the player thread / an NPC's R7 brief / the world brief — Step 4,
   `references/step-4-deep-design.md`) — pull the scene's track / tier / **want** / hook from that brief
   rather than improvising cold. **Name the WANT the beat serves**
...
3. **Doctrine self-audit** — check each against what THIS beat authored (cite the doctrine in
   ...
   - **machine wiring — `cross_npc` / `economic` beats (`redesign_phase_3/22`)** — if this beat carries a
     ...
     cold-start bullet above); **D2** it reads *another* arc's signal (no mutual lock — the reactivity pass
     (Step 4 Pass 4) + the Step-5 feedback traced the DAG); **D3** the gated rung is **locked-visible with
     `locked_text` naming the other arc's state** ("Sal
     won't go further while the bar's still in jeopardy"), never a silent lock (`14` L7). **Mechanism (real
   ...
   - **endgame stays carnal (`redesign_phase_3/14` P7 / `13`)** — a late/empire beat cashes out as
     **content**: a recruit is a **full `step-4-deep-design.md` arc** (Pass 2 — own double-lock + capstone + loop), an
   ...
   - **legibility — name PLACE + TIME-WINDOW + REQUIREMENT verbatim (`14` L1–L2 / G1)** — the **active** card
     ...
     ("Sal won't go further while the bar's still in jeopardy" — `14` L7 / the machine D3); a silent
   ...
   - **feeder economy / player-odometer reachable** — a player-track beat actually RAISES the player
     ...
     on the **player** tier — NOT an NPC walk-in (the player thread, `references/step-4-deep-design.md` Pass 1).
...
- **Name place + time-window + requirement verbatim (`14` L2 / G1).** The active card's `text`/`tip` (and
  ...
  machine, `22`) is **locked-visible with `locked_text` naming the gating arc's state** (`14` L7); never a
  silent cross-lock.
- Author quest cards into the beat's `target_phase` (or `5_scenes.toml`), alongside its canvases.

## Cross-references
`references/run-mode.md` (the ask/inform + build-green discipline) · `references/step-4-deep-design.md` (the
subject briefs this beat fills) · `references/content-framework.md` (the question set those briefs answer) ·
`references/trait-design.md` / `lanes.md` / `sex-loop.md` / `systems.md` / `toml-gotchas.md` ·
`redesign_phase_3/{04,07,09,11,14,17,22}` (the doctrine the new self-audit lines enforce; `22` = the machine —
cross_npc/economic wiring + the D3 cross-gate telegraph).
