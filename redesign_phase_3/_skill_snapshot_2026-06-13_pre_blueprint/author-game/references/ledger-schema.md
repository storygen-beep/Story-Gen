...
```jsonc
{
  ...
  "pipeline_phase": "authoring",    // which pipeline step we're in — the phase-aware dispatch reads THIS,
                                    ...
                                    //   "casting" | "deep_design" | "feedback" | "authoring". The ledger
                                    //   is CREATED at setup carrying this; structure_registry/plan stay
  ...
  "npcs": {                         // per-NPC intent from the R7 brief (Step 4 Pass 2); sizes the continue loop
    "npc_sal": {
  ...
  "deep_design": {                  // Step 4 per-subject progress (meaningful only while "deep_design")
    "player": "done",               // pending|active|done — §2 the player thread (Pass 1, FIRST)
    "npcs": "active",               // pending|active|done — §3 each NPC's R7 brief (Pass 2; START only when player=="done")
    "world": "pending",             // pending|active|done — §5 the world brief (Pass 3)
    "reactivity": "pending"         // pending|active|done — §4 the wiring (Pass 4, LAST; START only when npcs+world done)
  },
  "feedback": {                     // Step 5 review progress (meaningful only while "feedback")
...
**`deep_design` + `feedback`** (Step 4/5 progress; both additive — absent on a pre-restructure ledger
just means "the phase's start"). `deep_design` tracks the four Step-4 passes and **enforces the
supply→demand→stage order**: don't start `npcs` until `player == "done"`, and don't start `reactivity`
until `npcs` and `world` are both done (`step-4-deep-design.md`). `feedback` tracks the Step-5 review —
which framework subjects are walked + the `open_gaps` the navigation proposes from (`step-5-feedback.md`).
**Phase aliases** keep old ledgers readable: `"npc_arcs"` → `deep_design`, `"roster"` → `feedback`
(no in-flight game is at those phases, but the alias removes all risk). `schema_version` stays **2** —
these are additive fields, not a format break.
