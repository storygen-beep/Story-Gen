...
- **Three disciplines** (the firewalls): **D1** never gate an arc's ENTRY on another arc — only mid/late
  ...
  dependency cycles (the F1 wires form a DAG — checked at the feedback review, `step-5-feedback.md`); **D3** every
...
- **The opening / cold start (§1E).** Design the first session as the desire ladder's *first rung*: the first
  ...
  *speaks it back* (`@player`/`@npc` tokens, never baked into labels — `doctrine/14`).
...
- **The fail-state declaration (§1C).** Declare whether **failure exists at all**: when she refuses a beat,
  ...
  are traced at Step 4 Pass 4 / framework §4F; here just declare whether the negative axis exists — the
...
start + the systems-in-play + the fail-state). Set `pipeline_phase = "casting"` when done.
