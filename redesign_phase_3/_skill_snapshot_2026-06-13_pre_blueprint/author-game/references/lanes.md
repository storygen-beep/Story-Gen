...
- **A solo host can fire a PLAYER-LEWD event, not only an NPC walk-in** (the player thread —
  `references/content-framework.md` §2 / `references/step-4-deep-design.md` Pass 1).
  ...
  **player/world lewd track** — solo self-acts, location flashing/exhibitionism, public dares, job-lewd
  ...
  raising those odometers. This is the **corruption-feeder economy**: the supply that makes NPC
  seduction floors (the `requirementsMC` player-corruption gates) reachable. The lane model is otherwise
  NPC-centric and omits this whole category — invent it in the **player thread** (Step 4 Pass 1), not at the
  canvas. A game with rich NPC arcs but no feeder catalog starves its player odometer (the Last Call finding).
