...
# 🧊 FROZEN 2026-05-14
#
# Do NOT edit this file. All new engine work goes to v2.py.
# v1 exists only as a safe-mode rollback path during the v2 transition
# (invoke with `--gen-version v1` on package_from_toml / package_game).
# Once v2 is stable for ≥2 weeks, this file gets deleted.
#
# If you find a bug in v1 that affects a project still running on v1,
# prefer migrating the project to v2 over patching v1. v1 patches risk
# divergence between v1 and v2 in regions the byte-equality regression
# test (apps/game_generation/tests.py::TestV1V2ByteEquality) protects.
# ───────────────────────────────────────────────────────────────────────────
...
setup.checkRandomEncounters = function(locationId) {{
    try {{
        ...
        var shuffled = afterEntryGate.slice();
...
'''}
    ...
    // RTS arousal "daily auto-rise": e.g. player arousal +1 (cap 10), NPC +1 (cap 3).
...
// doc 45 G7 — corruption LEVEL (0-4) from raw points via tier thresholds.
...
<p>Please close this page.</p>
    ...
    def _resolve_theme(self, raw: dict) -> dict:
        ...
        """
        mode = raw.get("mode", "dark")  # match v2 — a no-[theme] build is dark (was "light", a latent trap)
...
.phone-jobs { padding: 8px; } .phone-jobs-xp { color:#bbb; font-size:13px; margin-bottom:8px; }
.phone-job-card { background: rgba(255,255,255,0.06); border-radius:10px; padding:10px 12px; margin-bottom:8px; }
.phone-job-name { font-weight:bold; } .phone-job-meta { font-size:12px; color:#bbb; margin:4px 0; }
.phone-job-btn, .phone-bank-btn { background:#0a84ff; color:#fff; border:none; border-radius:16px; padding:8px 14px; font-size:13px; cursor:pointer; margin-right:6px; }
.phone-bank { padding:12px; } .phone-bank-balance { font-weight:bold; font-size:16px; } .phone-bank-cash { color:#bbb; margin:6px 0 12px; }
.phone-daily-btn {
    display: block;
    width: 100%;
    background: transparent;
...
"""
    ...
    def _render_location_nav_card(self, loc, video_path):
        ...
        (v1 preserves its own badge set — unlocked badge + canvas-presence portraits.)"""
        loc_id = str(loc.id)
        ...
        indicators = (
            ...
            f'<<if setup.locationHasNewUnlockedChoices("{loc_id}")>><span class="nav-unlocked-badge">\U0001f513</span><</if>>'
            f'<<for _npc range setup.getNpcsWithCanvasesAtLocation("{loc_id}")>><<if _npc.portrait>><img @src="\'{video_path}/\' + _npc.portrait" class="nav-npc-badge" @alt="_npc.name"><</if>><</for>>'
            f'</div>'
        )
    ...
    def _render_location_nav_link(self, loc, video_path):
        ...
        open_link = (
            ...
            f'<<if setup.locationHasNewUnlockedChoices("{loc_id}")>> <span class="nav-unlocked">\U0001f513</span><</if>>'
            f'<<for _npc range setup.getNpcsWithCanvasesAtLocation("{loc_id}")>><<if _npc.portrait>> <img @src="\'{video_path}/\' + _npc.portrait" class="nav-npc-portrait" @alt="_npc.name"><</if>><</for>>'
            f'{cost_tag}'
        )
