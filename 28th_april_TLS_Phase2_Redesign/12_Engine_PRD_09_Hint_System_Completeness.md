...
|----|-------------|-------|--------|-------|
| E14 | `trait_checks` items in hint conditions | B | ✅ Shipped | Slice exercises in Frank Stage 0 transitionals |
| E15 | Global hints rendering ("Story Goals" section) | C | ✅ Shipped | Slice re-exposes rent / hygiene / energy |
| E16 | Stage hint visual split (` — 🎯 ` separator) | (pre-batch) | ✅ Shipped | All 13 slice templates use the format |
| E17 | Cleared-but-not-triggered detection | C | ✅ Shipped | Auto across all NPCs; no slice changes |
| E18 | Counter sidebar items (auto-emitted) | D | ✅ Shipped | 4 bars auto-emit in slice |
| E20 | Decay warnings (auto-emitted) | D | ✅ Shipped | Snapshot-vs-current; amber banner |
| E21 | Cooldown opt-in (`show_when_blocked`) | D | ✅ Shipped | No slice demo (no daily-cooldown solo activities to opt in) |
| E22 | Cross-NPC prerequisite reference | B | ✅ Shipped | No slice demo (no cross-NPC arc dependencies in current scope) |
| E23 | Build-time hint linter | A | ✅ Shipped | 8 detection rules; 2 known false positives |
...
### §2.10 Pattern 2 — Computed 🎯 goal block (Option D)
...
### §2.11 Pattern 3 — Activity-list panel under each Quest section
