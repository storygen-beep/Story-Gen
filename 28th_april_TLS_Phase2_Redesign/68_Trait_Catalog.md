# Doc 68 — Trait Catalog (Canonical Trait Doctrine)

**Date:** 2026-05-26
**Author:** ENI (with LO)
**Status:** Doctrine — applies to all RTS-shape sandbox games on this engine. **Source for `prompts_v2/doctrine/09_trait_catalog.md`.**
**Triggered by:** LO question — *"should we define traits sort of definitions, list of traits, formats like arousal to follow in the game, so that the generated game know what to use for what kind of systems."*
**Extends:** Doc 24 (Lane mechanism uses trait conditions) + Doc 30 §7.5 (vocab ceilings tier on corruption bands) + Doc 40 (player arousal 0–10, NPC arousal 0–3 family per 2026-05-21) + Doc 49 (body-state vs progression distinction) + Doc 56 (R3 Lane 3 budget; R4 sidebar) + Doc 57 (capstones use stage triggers) + Doc 67 (multi-NPC dispatcher conditions use trait predicates).
**Supersedes:** nothing.
**Decisions locked this session:**
- **Q1 (LO):** Stage trait = stored integer per-NPC, **internal-only, NEVER player-facing**.
- **Q2 (LO):** Player corruption = 0–100 with 4 named bands (Pure / Lewd / Slutty / Whore).
- **Q3 (LO):** NPC affection trait name = `relation` everywhere (single canonical name, RTS-aligned).
- **Q4 (LO):** Body-state canon = `energy` + `hygiene` only (engine-grounded per Doc 49).
- **Q5 (ENI, deferred):** Phase 2+ traits OMIT from canon; listed in §6.1 off-limits until LO scopes Phase 2+ engine work per Doc 65.
- **Q6 (ENI, deferred):** NPC trait sidebar visibility per-arc declaration in R7 design brief; catalog provides per-arc-shape defaults.

---

## §1 — The question this doc answers

`prompts_v2/` has canonical doctrine for mechanism (Docs 24/56/57/67) but no canonical vocabulary for **state**. Every game generated against the v6/v4 prompts invents its own trait names + ranges + thresholds — `lust` vs `arousal`, 0–10 vs 0–100, ad-hoc gate thresholds per arc. The mechanism doctrine ("corruption ≥ 25 unlocks exhibitionism") doesn't translate cross-game without a stable `corruption`.

This doc canonizes the trait vocabulary. Three tiers:
- **Tier 1 — Required** for every RTS-shape sandbox. The LLM ships these traits on every game it generates.
- **Tier 2 — Common extensions** the LLM uses when the arc/setting calls for it.
- **Tier 3 — Per-game additions** the LLM can author free-form against the design book, EXCEPT for the off-limits list (§6.1) which is reserved for Phase 2+ engine work.

Per LO's load-bearing addition (*"the doc should also contain when to use them and why"*), each canonical trait section is more than a registry entry — it explicitly tells the LLM **when** to use the trait, **why** to use it instead of close-neighbor traits, and **what NOT** to use it for. That converts the catalog from a name lookup into authoring guidance.

---

## §2 — How to read this catalog

Every Tier 1 + Tier 2 trait is documented with the same 13-field template:
...
**Reading priority for a fresh LLM session:**
1. §3 + §4 (Tier 1 traits — every game has these)
2. §6.1 (off-limits list — never author against these without LO scoping)
3. §7 (engine effect schema — how to emit traits in TOML)
4. §8 (NPC visibility doctrine — what surfaces to sidebar)
5. §5 (Tier 2 traits — only if arc/setting needs them)
6. §10 (anti-patterns — pre-ship audit)

---

## §2.5 — Trait initialization requirement (READ BEFORE §3)

**Every player trait referenced in any effect, condition, or sidebar item MUST be pre-declared in the game's `[player.core_traits]` block at game start with an initial value.** This is an engine requirement, not a stylistic recommendation.
...
hygiene = 100
money = 80
# Per-NPC stage traits live here too — one per NPC with an arc:
frank_stage = 0
ryan_stage = 0
jake_stage = 0
# Plus any Tier 2 traits the game uses:
fitness = 0
beauty = 0
exhibitionism = 0
intelligence = 0
...
**Why this matters:**
- **Validator enforcement (sidebar items only):** `template_import.py:2382-2547` builds `_player_trait_keys = set((template.player.core_traits or {}).keys())` and rejects sidebar items (`trait_words`, `trait_bar`, `trait_status_text`) that reference traits not in this set with a hard error.
- **Runtime silent failure (effects + conditions):** `triggerConditionsSatisfied` at v2.py:3370 reads `(player.core_traits || {})[String(tkey)]` — an undeclared trait reads as `undefined`. Conditions silently evaluate against `undefined`; effects via `applyAndNotifyTrait` produce garbage deltas. **No build error fires for effects or conditions on undeclared traits.**

**Rule:** at game-book generation, the LLM emits the full `[player.core_traits]` block listing every Tier 1 + every Tier 2 + every `<slug>_stage` trait the game uses. Tier 3 free-form traits added by the game must also appear here.

**NPC traits:** initialized inside each `[[npcs]]` block via its own `core_traits` table. Same requirement — declare before use.

---

## §3 — Tier 1 Player Traits

These five traits exist in every RTS-shape sandbox. Always declare them in the game's initial `[player.core_traits]` block (per §2.5).
...
**What it tracks:** Maya's accumulated transgression of her own pre-game norms. The universal cross-arc currency for content-tier unlocking. Climbs through explicit choices (flashing, masturbating, sexual scenes) AND through passive exposure (being groped, watching NPC sex scenes). Never decays; the player's slope is forward-only.
...
- Per-arc Stage transitions (most arcs gate Stage 2 at corruption 15+, Stage 3 at 30+ — see Doc 30 §7.5 vocab ceilings)
- Lane 2/3 substitution preconditions (the substituted scene's `conditions` reference it)
- Capstone triggers (paired with a flag — see Doc 57)
...
- Lane 2/3 substitution eligibility (target canvas conditions)
...
**Anti-pattern:** "Reduce corruption when player declines a Lane 1 menu item." Wrong. Failing taboo actions doesn't add OR subtract corruption (per `rts-arc-trace/notes.md` 2026-05-02 correction — RTS's `<<NotifyCorruption N>>` is a UI hint widget, NOT a state mutator. Verified live: clicked "Have sex with him 🔥" at corruption 0 → silent fail, no stat change at all). Decline = no change.

**RTS analog:** `$player.corruption` (0–200 in RTS; scaled to 0–100 here per LO §6.2).
...
**Sidebar render:** `trait_bar` with optional bands (Cold / Warm / Hot / Burning)
**Bands (optional):** Cold (0–2) / Warm (3–5) / Hot (6–8) / Burning (9–10)

**What it tracks:** Maya's short-term sexual readiness — the per-attempt fuel meter for masturbation + lewd activities. Climbs from beats + days; resets to 0 on climax. Distinct from corruption (which is long-term progression).

**When to use it:**
- Masturbation activity gate (must be > 0 to masturbate)
- Lane 3 substitution preconditions ("if Maya aroused")
- Some Lane 1 menu items at high arousal (flash unlocks at arousal 2+; specific lewd activities at arousal 5+)
...
- Climax (masturbation completion + NPC sex completion): **author-emitted** reset to 0 via `{ targetType = "player", trait = "arousal", op = "set", value = 0 }` on the climax canvas's exit_block effects. **No engine macro auto-zeroes arousal** — RTS has `FinishMasturbation` / `FinishSex` macros that do this; TLS does not. The reset is the author's responsibility on every climax canvas.
...
- Self-touch / flash menu items at NPC hubs (high arousal can unlock new options)
...
- Capstone triggers (capstones use flags + stage; arousal is too volatile)
- Permanent stat ladders

**Anti-pattern:** Decaying arousal per day. Doc 40 explicitly locks the no-decay rule based on RTS live verification — sleep RAISES arousal, never resets it. Resets happen only at climax. If a game generation emits `[engine.daily_tick].traitEffects` with `arousal -1`, that's wrong.
...
**Range:** 0–100 integer
**Default at game start:** 100 (full energy at game start)
**Decay:** Per-action (-10 to -25 per activity); restored by sleep (+50 to +100 to full); nap +15
**Sidebar render:** `trait_status_text` (body-state Tier 2 sidebar primitive per Doc 49)
...
- Forced-sleep mechanic (when energy hits 0, next-action requires sleep)
- Some Lane 3 substitutions (NPC walk-in scenes that require Maya have energy to engage)
...
**Don't use it for:**
- Hard-gating ALL NPC interactions (it's a soft modifier; don't block entire arcs on hygiene)
...
**RTS analog:** No direct RTS equivalent (RTS has hygiene-like content in pregnancy mechanic but not a standalone hygiene trait). TLS-specific per Doc 49.
...
**Default at game start:** Varies by setting (TLS starts at $80 per Doc 30 §4.1 economic engine)
...
**When to use it:**
- Rent payment (recurring monthly drain; insufficient = game-over-ish per Doc 30 §4.1)
...
**What it gates (downstream content):**
- Rent payment (game state — can't pay rent = game-over-ish)
...
These four traits exist on every NPC in every RTS-shape sandbox. Always emit them in each NPC's initial `[[npcs]]` block.
...
- Climax in a sex scene with the NPC: **author-emitted** reset to 0 via TWO effects on the climax canvas's exit_block — one for Maya, one for the NPC. **No engine macro auto-zeroes either** — RTS has `FinishSex` that zeroes both at once; TLS does not. Author must explicitly emit `{ targetType = "player", trait = "arousal", op = "set", value = 0 }` AND `{ targetType = "npc", npcId = "npc_X", trait = "arousal", op = "set", value = 0 }`.
...
- NPC stage transitions (per-arc thresholds — see Doc 56 §5)
...
### `stage` (per-NPC, stored on PLAYER namespace) — 🔒 INTERNAL-ONLY (NEVER PLAYER-FACING)
...
> **⚠️ STORAGE DOCTRINE (corrected 2026-05-26 against engine source):** Stage is stored as a **PLAYER trait keyed by NPC slug**, NOT as a trait on the NPC object. The trait name pattern is `<npc_slug>_stage` (e.g., `frank_stage`, `ryan_stage`, `jake_stage`) and it lives at `player.core_traits.<slug>_stage`. The engine has special-case logic at v2.py:5077-5087 (`applyAndNotifyTrait` stage advancement detection) that recognizes this pattern via the regex `/^([a-z_]+)_stage$/` and updates `setup.npc_arc_stages` registry on upward delta. The NPC's `arc_stages = [...]` declaration at `[[npcs]] ... arc_stages` is just the LIST of stage NAMES (display strings); the CURRENT stage value lives on player.
...
**What it tracks:** Discrete arc-progression milestone for one NPC's arc. Stored as an integer on the player namespace (`player.core_traits.<slug>_stage`); used by authors + LLM for content gating; never surfaced to player. Player feels stage progression through what the world DOES (new menu items appear, NPC behavior shifts, location access opens), NOT through a stage number.

**When to use it:**
- Lane 1 hub canvas selection (multiple canvases per location; engine picks by stage via `selectAutoFireCanvasForLocation` at v2.py:3839)
...
- Daily passive: NONE
- No regression (stages don't go down)
- Engine logs the advancement timestamp in `game_state.stage_advancement_log[slug]` on upward delta (v2.py:5083-5087) — used by E9 stalled-detection
...
```toml
# Form A — raw player-trait check (simpler; recommended for most uses)
{ type = "trait", subject = "player", trait_key = "frank_stage", operator = "gte", value = 2 }

# Form B — dedicated stage predicate via helper (used internally by stage_label sidebar)
{ type = "stage", helper = "frank_stage_2_plus", operator = "is_true" }
```

Form A reads the player trait directly. Form B uses a helper from `stage_helpers_map` that resolves to a condition block. See §9 for when to use which.
...
These traits are common but optional — use them when the arc/setting calls for them. The LLM should not emit them unless the design book proposes the corresponding mechanic.
...
**Decay:** None (progression trait — see Doc 49)
...
**Range:** 0–N (derived from worn outfit's beauty value via `worn_beauty` predicate — see Doc 37)
...
**Anti-pattern:** Collapsing exhibitionism into corruption. They're related but distinct — a Maya can be high-corruption + low-exhibitionism (sexually active but private) or low-corruption + high-exhibitionism (loves being seen but not having sex yet). Keep them separate if the game's arc uses exhibitionism as a distinct progression.

**RTS analog:** `$player.exhi` (RTS integer trait).

---

### `intelligence` (Player) — also accepted: `intel`
...
- `combat_skill` (if game has combat — non-canonical for sandboxes)
...
- Tier 3 traits must follow the same engine effect schema (§7) — declared in `[player.core_traits]` or per-NPC `core_traits` initial state (see §2.5), mutated via `{ targetType = "player", trait = "X", op = "add", value = N }` (effect syntax) and tested via `{ type = "trait", subject = "player", trait_key = "X", operator = "gte", value = N }` (predicate syntax). Effect + predicate use different field names — see §7.6 reference card.
- Tier 3 traits should be documented in the game's own design book + per-game README; the catalog doesn't enumerate them.

### §6.1 — Off-limits list (Phase 2+ traits — DO NOT AUTHOR AGAINST)

These traits surface from the Phase 2+ engine work scoped in Doc 65. The LLM MUST NOT emit these in any generated game until LO calls the corresponding strategic decision per Doc 65. Each is listed with the trigger that would unlock authoring:
...
**Doc 65 reference:** §3 row 1 (Pregnancy E10b — ENI recommendation defer Phase 2+; LO has not yet committed).
...
#### `scandal_level` / `reputation` (Doc 65 E10c — Scandal/reputation global state)

**Why off-limits:** Global scandal/reputation is a new engine system requiring multiple writer arcs (outdoor scenes), multiple reader arcs (Diana confrontation gate, town-NPC reactions), and UI surface. Currently only TLS's `diana_awareness` partial-implementation exists.

**Doc 65 reference:** §3 row 2.
...
#### `gallery.*` flags (Doc 65 E10e — Gallery + achievements)

```
gallery.unlocked_scenes[]         (array of scene IDs)
gallery.achievements[]            (array of achievement IDs)
```

**Why off-limits:** Gallery UI requires Doc 62 (canvas `guide` field) + Doc 64 (sidebar radar) + the gallery panel itself. None shipped.

**Doc 65 reference:** §3 row 3 (ENI recommendation: ship in Phase 2 when 9+ once-only capstones exist).
...
```
player.completed_scenes[]          (array of scene IDs across all NPCs)
npc.X.completed_scenes[]           (per-NPC completion list)
```

**Why off-limits:** Cross-arc state writers + readers require the tracker mechanism shipped + the canvas `guide` field (Doc 62). Neither in engine yet.

**Doc 65 reference:** §3 row 4.
...
> **⚠️ CRITICAL — effect syntax vs predicate syntax use DIFFERENT field names.** Both shapes appear below; do not mix them. This was a correctness error in Doc 68's first draft (audited + corrected 2026-05-26 against engine source at `apps/projects/services/template_import.py:481-505` + `apps/game_generation/twee_comprehensive/generators/v2.py:5071,4962`).

### §7.1 — Trait effect (mutate a trait value)

Canonical shape for trait effects (used in `[[canvases.nodes]]` body effects, `[canvases.exit_block.effects]`, `[engine.daily_tick].traitEffects`, choice effects):
...
# Optional clamp + cap on any trait effect
{ targetType = "player", trait = "arousal", op = "add", value = 1, clamp = true, cap = 10 }
```

**Required fields:**
- `targetType` — `"player"` or `"npc"` (NOT `subject` — that's the predicate field name)
- `trait` — the trait name (NOT `trait_key` — that's the predicate field name)
- `op` — operation (see allowed values below)
- `value` — numeric value to add or set

**Optional fields:**
- `npcId` — required when `targetType = "npc"` (NOT `npc_id` — that's the predicate field name)
- `clamp` — boolean; when true, the resulting value cannot go below 0
- `cap` — numeric upper bound; if applying the op would exceed `cap`, value is set to `cap` instead

**Schema source:** `TemplateChoiceEffect` dataclass at `template_import.py:481-495`. Verified live in TLS at `games/the_long_summer_test/toml_phases/7_final_game.toml:82` — `{ targetType = "player", trait = "arousal", op = "add", value = 1, cap = 10 }`.

### §7.2 — Allowed `op` values

Engine supports exactly **two `op` values** for trait effects (verified at v2.py:4962-4964 — `if (op === 'add') { ... } else if (op === 'set') { ... }`):
...
**There is no `"sub"` op.** Decay / subtraction is achieved via `op = "add"` with negative `value` (e.g., `value = -10` to subtract 10). Doc 68's first draft incorrectly listed `sub` — corrected per audit.

### §7.3 — Per-trait operation conventions

How each canonical trait is typically mutated. These are authoring conventions, not engine restrictions — the engine accepts any allowed op on any trait.

| Trait | Common ops | Notes |
|---|---|---|
| `player.corruption` | `add` (positive) | One-way climb; never decreases except dev shortcuts |
| `player.arousal` | `add` (positive); `set` to 0 only at climax | **No engine macro auto-zeroes arousal** — author must explicitly emit `op = "set", value = 0` on the climax canvas. RTS has `FinishMasturbation` / `FinishSex` macros; TLS does not. |
| `player.energy` | `add` with negative value (decay); `set` to 100 (sleep restore) | `cap = 100` recommended |
| `player.hygiene` | `add` with negative value (daily decay); `set` to 100 (shower) | `cap = 100` recommended |
| `player.money` | `add` (positive/negative) | Income / costs |
| `npc.X.arousal` | `add` (positive); `set` to 0 at climax | Same author-emitted reset pattern as player arousal |
| `npc.X.corruption` | `add` (positive) | One-way climb |
| `npc.X.relation` | `add` (positive/negative; negative rare) | Doesn't decay daily; specific scenes can decrement |
| `<npc_slug>_stage` (player namespace — see §9) | `set` only | Stages are deliberate, set on capstone exit. See §9 for storage details. |
| Tier 2 player traits | `add` (positive) | One-way climb (no decay) |
| Body-state (`energy`, `hygiene`) | `add` (negative for decay), `set` for restores | Daily decay via `[engine.daily_tick].traitEffects`; restores via activity exit_block |

### §7.4 — Flag effect (set/unset/toggle a flag)

Flag effects use a different schema from trait effects (separate dataclass `TemplateFlagEffect` at `template_import.py:498-505`):
...
- `flag` — flag name (NOT `flag_key` — that's the predicate field name)
- `op` — `"set"` (default), `"unset"`, or `"toggle"`

**Optional fields:**
- `npcId` — when `targetType = "npc"`

**Allowed `op` values for flags:** `"set"`, `"unset"`, `"toggle"`.

**Schema source:** `TemplateFlagEffect` at `template_import.py:498-505`.

### §7.5 — Predicate (trigger condition) schema

> **The predicate schema uses DIFFERENT field names from effects.** This is the engine's actual API — there's no translation layer that accepts both.
...
items = [
  ...
  { type = "trait", subject = "player", trait_key = "frank_stage", operator = "gte", value = 2 },   # stage check — see §9
  { type = "flag", subject = "player", flag_key = "frank_caught", operator = "is_true" },
]
```

**Predicate field names:**
- `type` — `"trait"`, `"flag"`, `"modifier"`, `"days_since_flag"`, `"clothing_slot"`, `"clothing_item"`, `"pass"`, `"item"`, `"stage"`, `"quest"`, `"worn_beauty"`, `"worn_corruption"`
- `subject` — `"player"` or `"npc"` (NOTE: predicate uses `subject`; effects use `targetType`)
- `trait_key` (for `type = "trait"`) — the trait name (NOTE: predicate uses `trait_key`; effects use `trait`)
- `flag_key` (for `type = "flag"`) — the flag name (NOTE: predicate uses `flag_key`; effects use `flag`)
- `npc_id` (when `subject = "npc"`) — the NPC slug (NOTE: predicate uses `npc_id`; effects use `npcId`)
- `operator` — comparison operator (`"gte"`, `"lt"`, `"eq"`, etc.)
- `value` — comparison value

**Allowed predicate `operator` values** (per `triggerConditionsSatisfied` at v2.py:3364-3386 + sibling type-dispatch blocks):
...
The single most common Doc 68 authoring mistake is mixing effect + predicate field names. This card disambiguates:
...
| Type discriminator | (parser dispatches by presence of `trait` vs `flag` field) | `type` (required: `"trait"`, `"flag"`, etc.) |

Using effect field names in a predicate (or vice versa) causes silent no-ops — the engine reads `undefined`, the math goes weird, and no build error fires. The validator does not catch field-name mismatches.

---

## §8 — NPC trait sidebar visibility doctrine

Per LO Q6 (ENI call): each NPC's design brief (R7 per Doc 56) declares which traits surface to the player. The catalog provides per-arc-shape defaults; the brief can deviate with reason.

| Arc shape | Sidebar surfaces (default) | Rationale |
|---|---|---|
| **Family/ambient** (e.g., Frank) | location + arousal + corruption + relation | Player needs to plan Lane 3 attempts (arousal), Lane 1 escalation (corruption), late-game intimacy (relation). All three are mechanically relevant. RTS surfaces all three for family NPCs (Stepbrother/Stepfather/Stepgrandfather). |
| **Slow-burn family** (e.g., Jake) | location + arousal + relation | Corruption stays low in slow-burn arcs by design; surfacing it would mislead the player. Arousal + relation are the player-relevant dimensions. |
| **Peer/dating** (e.g., Ryan) | location + relation | Dating chain is relation-driven. Arousal is bounded + less player-controllable. Corruption isn't meaningful for peer arcs (most peer NPCs cap low). |
| **Service** (e.g., Marge) | location + relation only | Workplace bond is the operative axis. Arousal/corruption don't apply to service register. |
| **Antagonist/witness** (e.g., Diana) | location only | Awareness/scandal accumulator stays HIDDEN — dramatic surprise depends on player NOT seeing how close confrontation is. Doc 30 §6 + Doc 60 lock this. |
...
The brief can override the default with reason (e.g., a family/ambient NPC whose arousal stays constant by design could hide it). But the override must be documented in the brief.
...
### §9.0 — Storage location (corrected per 2026-05-26 engine audit)

Stage is stored as a **player-namespace trait keyed by NPC slug**: `player.core_traits.<slug>_stage`. The NPC's `arc_stages = [...]` declaration on `[[npcs]]` is just the LIST of stage NAMES (display strings used by the optional `stage_label` sidebar item — which Doc 68 forbids using for player-facing rendering); the CURRENT stage integer lives on the player object.
...
### §9.0.1 — Checking stage in a condition: two forms

The engine supports two ways to test stage in a trigger condition:

**Form A — raw player-trait check (RECOMMENDED for direct content gating):**
```toml
{ type = "trait", subject = "player", trait_key = "frank_stage", operator = "gte", value = 2 }
```

Reads the player trait directly. Simple, readable, no indirection. Use this for almost all stage checks in canvas trigger conditions + Lane 2/3 substitution conditions.

**Form B — dedicated `stage` predicate via helper (engine has it, but rarely needed for authoring):**
```toml
{ type = "stage", helper = "frank_stage_2_plus", operator = "is_true" }
```

Resolves the named helper from `setup.stage_helpers_map` and recursively evaluates the helper's condition block (v2.py:3507-3521). Used internally by the `stage_label` sidebar widget. **Don't author this form in trigger conditions** — Form A is cleaner. Form B is engine plumbing.

### What stage IS used for

- **Lane 1 hub canvas selection.** Engine picks the highest-priority valid canvas at a location via `selectAutoFireCanvasForLocation` (v2.py:3839); multiple canvases per location can exist, each gated on a specific stage. Frank's kitchen morning hub at Stage 0 is a different canvas from Frank's kitchen morning hub at Stage 3.
- **Lane 2/3 substitution conditions.** Per Doc 67 §4 — substitution rule conditions reference NPC stage to gate eligibility.
- **Per-stage menu items inside hub canvases.** `group` blocks inside a canvas's `[[canvases.nodes]]` can gate on stage (Tier 2 dinner-time content visible only at Frank stage ≥ 2).
- **Capstone trigger fingerprints.** Per Doc 57 R1 — capstones flag-set + stage-advance on exit.
- **Quest card `when` conditions.** Quest cards can route on NPC stage (per Doc 50 R2) WITHOUT exposing the stage number in card text.

### What stage is NEVER used for

- **Sidebar items.** No `trait_bar`, no `trait_status_text`, no `trait_words` rendering of stage. Engine should not even know how to render it.
...
- **NPC menu text.** Hub canvas prose says *"Frank watches you with a new tension in his shoulders"*, NOT *"Frank Stage 2 unlocked"*.
- **Achievement / notification toasts.** No "Stage 2 reached!" popup. The new content appearing IS the notification.
- **Dev shortcuts in player-visible builds.** Dev canvases that set stage are fine for authoring/testing (`dev_advance_frank_to_2`); they should not ship in player-facing builds (use the cheats menu or non-shipped builds).

### How the player feels stage progression (without seeing it)

The player experiences stage progression through **what the world does**, not through what the world says:

| Stage transition | What the player sees |
|---|---|
| Frank Stage 0 → 1 (Maya helps Frank with bookkeeping) | New "Stand close while he reads" menu item appears in Frank's office hub. Coffee Alone Lane 2 ambient now ends differently. |
...
The player understands the arc moved without needing a stat number. RTS calls this **"emergent escalation"** (per `notes.md` 2026-05-02) — the world fills out around the player as they escalate; the world model itself is the progress feedback.
...
Concrete shapes to NOT ship. The LLM must check against these before emitting trait references.
...
- **Per-NPC trait references without proper namespacing.** Two forms — predicate vs effect — and they use different field names (see §7.6 reference card).
  ...
  - The engine stores NPC traits nested at `$npc.frank.core_traits.corruption` (v2.py:3380, 5065), not as flat player traits. EXCEPTION: stage traits live on the player namespace as `<slug>_stage` — see §9.

### Range / type anti-patterns

- **Player corruption outside 0–100.** Per LO Q2 lock. Don't author against the RTS 0–200 scale; doctrine uses 0–100 with 4 bands.
- **Family NPC arousal outside 0–3.** Per Doc 40. Family NPC arousal is bounded; if the LLM emits `{ trait_key = "arousal", op = "add", value = 5 }` for Frank, that's wrong (overflows the 0–3 range, breaks Stage-gating).
- **Float values in integer traits.** All canonical traits are integer; don't emit `value = 0.5`.

### Decay anti-patterns

- **Decaying corruption.** Corruption never decays. Doc 40 + 49 lock this.
- **Decaying arousal.** Per Doc 40 — no-decay rule. Reset to 0 only at climax (FinishMasturbation / FinishSex).
...
- **NOT decaying energy or hygiene.** These ARE supposed to decay (body-state per Doc 49). If the LLM authors `energy` as no-decay, that's wrong — energy/hygiene need daily_tick or per-action decay.

### Gating anti-patterns

- **Using arousal for content-tier gating.** Wrong axis — arousal is short-term per-attempt. Use `corruption` for content-tier unlocks.
- **Using corruption for activity-attempt gating.** Wrong axis — corruption is content-progression. Use `energy` for "can the player do this activity right now."
...
- **Exposing stage to the player.** Per LO Q1 + §9. No sidebar, no quest card, no menu text.
...
- **Hiding body-state from player.** Energy + hygiene MUST surface (per Doc 49). Player needs to know when to sleep/shower.

### Phase 2+ anti-patterns

- **Authoring against `pregnancy.*` traits.** Off-limits per §6.1 until LO scopes per Doc 65.
- **Authoring against `scandal_level` / `reputation` globally.** Use per-NPC `awareness` instead (per Doc 30 §6) — global scandal waits for Phase 2+.
...
- **Effect using predicate field names (`subject` / `trait_key` / `npc_id`).** Effects use `targetType` / `trait` / `npcId` (see §7.6 reference card). Wrong: `{ type = "trait", subject = "player", trait_key = "corruption", op = "add", value = 1 }`. Right: `{ targetType = "player", trait = "corruption", op = "add", value = 1 }`. Using predicate field names in an effect causes the parser to read `_require_str(e, "trait", "")` and return empty — the effect silently no-ops with NO build error.
- **Predicate using effect field names (`targetType` / `trait` / `npcId`).** Predicates use `subject` / `trait_key` / `npc_id` (see §7.6). Same silent-failure risk in reverse direction.
- **NPC effect without `npcId`.** Wrong: `{ targetType = "npc", trait = "corruption", op = "add", value = 1 }`. Right: includes `npcId = "npc_frank"`.
- **Stage effect targeting NPC namespace.** Wrong: `{ targetType = "npc", npcId = "npc_frank", trait = "stage", op = "set", value = 2 }`. Right: `{ targetType = "player", trait = "frank_stage", op = "set", value = 2 }`. Stage lives on player namespace as `<slug>_stage` (see §9).
- **`op = "sub"` for decay.** Engine has no `sub` op (only `add` + `set`). Use `op = "add"` with negative `value` instead: `{ targetType = "player", trait = "energy", op = "add", value = -10 }`.
- **Stage advance via `op = "add"`.** Wrong: `{ targetType = "player", trait = "frank_stage", op = "add", value = 1 }`. Right: `{ targetType = "player", trait = "frank_stage", op = "set", value = 2 }`. Stage advances are deliberate via capstones; engine doesn't auto-promote on accumulated `add`s.
...
- **Doc 13** — Road to Success Reference (RTS trait variables: `$player.*`, `$npc.X.*` — source ground truth)
- **Doc 24** — 3 Lanes for Repeatable NPC Content (Lane 1/2/3 mechanism uses trait conditions)
- **Doc 30 §4.4** — Per-NPC stat ladder template (universal corruption tier model)
- **Doc 30 §7.5** — Per-arc vocab ceilings (tier-on-corruption gates content register)
- **Doc 40** — Player arousal 0–10 + family NPC arousal 0–3; no-decay rule for both (Doc 68 §3-§4 inherit)
- **Doc 49** — Story Goals vs Sidebar (body-state vs progression distinction)
- **Doc 56** — RTS Principles + Alignment (R3 Lane 3 budget uses arc shape + stage; R4 sidebar uses trait surfacing)
- **Doc 57** — Capstone Doctrine (capstone fingerprint includes flag-set + stage-advance)
- **Doc 65** — Phase 2+ Strategic Scope (off-limits list in §6.1 cross-references this)
- **Doc 67** — Solo Activity Design (multi-NPC dispatcher conditions use trait predicates)
- **Doc 66** — Session Record / Prompts Rewrite Pivot (Doc 68 is added to mechanism quintet in §15.1; source for `prompts_v2/doctrine/09_trait_catalog.md`)

### Engine primitives referenced

- `triggerConditionsSatisfied` — `apps/game_generation/twee_comprehensive/generators/v1.py:2684–2952` (predicate vocabulary; defines allowed `op` values)
- `applyAndNotifyTrait` — `v1.py:4926` (core trait-mutation + sidebar update function)
- `getNpcLocation` — `v2.py:2898` (NPC location predicate)
- `checkAndSubstituteCanvas` — `v2.py:4597` (Lane 3 dispatcher uses trait conditions)
- `selectAutoFireCanvasForLocation` — `v2.py:3839` (per-stage canvas selection at location)
- `TemplateTrigger` — `apps/projects/services/template_import.py:430+` (schema)
- `[engine.daily_tick].traitEffects` — `v1.py:4642+` (execution); `template_import.py:789` (schema)

### Memory entries referenced

- `feedback_tls_scene_body_style.md` — voice register (RTS-flat default; Tier-3 literary capstones)
- `rts_three_arc_shapes.md` — P4 source (family / peer / career shapes — drives per-arc trait visibility in §8)
- `rts_arousal_system.md` — Doc 40 source (no-decay rule)
- `rts_clothing_system.md` — Doc 36/37 source (worn_beauty derived predicate)

### RTS source artifacts (verified extractions)

- `game_explorations/rts-arc-trace/passage_catalog.json` — RTS trait variable usage (e.g., `$player.corruption`, `$npc.Brother.arousal`)
- `game_explorations/rts-arc-trace/variable_index.json` — variable cross-reference for trait names
- `game_explorations/rts-arc-trace/notes.md` — 2026-05-02 correction on NotifyCorruption being UI-hint not state-mutator (used in §3 corruption anti-pattern)

---

## §12 — Confidence ladder

Per Doc 24 methodology (source + decisions, not paraphrase).

✅ **HIGH confidence (locked this session):**
- 4 LO-locked decisions (Q1 stage / Q2 corruption / Q3 relation / Q4 body-state)
- Player arousal 0–10 + no-decay (Doc 40 source-verified)
- Family NPC arousal 0–3 + no-decay (Doc 40)
- Corruption is one-way climb / never decays (RTS source + Doc 49)
- `triggerConditionsSatisfied` predicate vocabulary at v1.py:2684–2952 (re-verified)
- Engine effect schema in §7 (matches template_import.py + applyAndNotifyTrait)
- NotifyCorruption is UI hint, not state mutator (notes.md 2026-05-02 live verification)
- `worn_beauty` is derived predicate, not stored (Doc 36/37)

🟡 **MED confidence (ENI calls — overrideable):**
- Per-arc-shape NPC visibility defaults in §8 — based on RTS observation + slice need; can be revised per playtest or LO override per-NPC in R7 brief
- Off-limits list in §6.1 — based on Doc 65 + Doc 62/63/64 PRD state; revisit when LO scopes Phase 2+ work

❌ **NOT yet established (deferred):**
- Whether Phase 2+ traits ever get added to canon — depends on Doc 65 LO decisions
- Whether Tier 2 trait list expands beyond fitness/beauty/exhibitionism/intelligence — additive per future game settings
- Whether new Tier 1 traits are ever needed — currently no gap identified, but evaluate when next non-RTS-shape game gets proposed

---

## §13 — Doc 66 cross-ref closure

Doc 66's prompts_v2 plan needs 5 small updates to keep cross-references straight after Doc 68 lands. Following edits applied (or to apply) post-Doc-68:

1. **Doc 66 §7 source-material table** — add `doctrine/09_trait_catalog.md` row with Doc 68 as source.
2. **Doc 66 §8 writing order** — add `doctrine/09_trait_catalog.md` as step 4.5 (alongside doctrine/03 + doctrine/04; trait canon is foundational).
3. **Doc 66 §9.1 batch 1 quality gate** — add bullet for `doctrine/09_trait_catalog.md` content checks.
4. **Doc 66 §14 cross-references** — add Doc 68.
5. **Doc 66 §15.1 boot order** — promote mechanism quartet → quintet (Doc 24 + Doc 57 + Doc 50 + Doc 67 + Doc 68).
