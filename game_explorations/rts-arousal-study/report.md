# rts-arousal-study — Exploration Report

Generated: 2026-06-05T14:00:35.571Z
Source URL: https://mopoga.com/road-to-success

## Session Summary

- Sessions run: 1
- Total wall-clock: 21m 44s
- Total clicks: 18
- Total choices explored: 18
- Unique states seen: 18
...
|---|---|---|---|---|
| `player.energy` | number | 40..100 | 3 | high |
...
| `npc.Dad.corruption` | number | 0..10 | 1 | high |
| `npc.Brother.corruption` | number | 0..15 | 1 | high |
| `npc.Grandpa.corruption` | number | 5..99 | 1 | high |
...
| name | type | range / samples | mutations | confidence |
|---|---|---|---|---|
| `dev.devMode` | boolean | true, false | 1 | high |
...
| `game.random` | number | 5..28 | 12 | low |
...
| `game.randomMedia` | string | ``, `sleep1.webp`, `sleep.webp` | 2 | low |
| `game.timeIcon` | string | `🌤️` | 0 | low |
| `game.weather` | string | `clear` | 0 | low |
| `game.weatherIcon` | string | `☀️` | 0 | low |
| `game.lastPassage` | string | ``, `History`, `Bedroom` | 4 | low |
...
|---|---|---|---|---|---|---|
| 1 | 2026-06-05T13:38:51.756Z | 21m 44s | 18 | 18 | 18 | no |

## Graph coverage (observed vs. static)

- Static-graph edges (every navigation parsed from passage source): **460**
- Observed edges during play: **8** unique `(from, clicked_text, to)` tuples.
- Static edges covered by at least one observation: **2** (a single observation covers every static edge with the same `(from, to)` pair — gated branches collapse to one observable move).
- Observed-only edges (no matching static edge, typically self-loop `<<link>>` wrappers that `<<replace>>` in-place): **6**.
- Coverage: **0.43%** of the static graph explored.
...
- Distinct passages visited at least once: **4** — playable-passage coverage: **1.1%**.
