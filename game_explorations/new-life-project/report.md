...
Generated: 2026-04-19T10:52:02.109Z
Source URL: https://mopoga.com/new-life-project

## Session Summary

- Sessions run: 5
- Total wall-clock: 92m 49s
- Total clicks: 481
- Total choices explored: 481
- Unique states seen: 665
...
| name | type | range / samples | mutations | confidence |
|---|---|---|---|---|
| `day` | number | 1..16 | 1 | high |

### item (4)

| name | type | range / samples | mutations | confidence |
|---|---|---|---|---|
| `tickets` | number | 8..8 | 0 | medium |
| `mon.monsmall.hasVisited` | boolean | false | 0 | high |
| `mon.monconstruct.hasVisited` | boolean | false, true | 1 | high |
| `mon.guard.hasVisited` | boolean | false, true | 1 | high |
...
| `chips` | number | 0..15000 | 4 | low |
| `bet` | number | 0..5000 | 1 | low |
| `luckyCasino` | number | 0.49..0.5 | 1 | low |
| `casinoSus` | number | 0..0.09999999999999999 | 1 | low |
| `pot` | number | 0..10000 | 2 | low |
| `winAmount` | number | 0..1 | 1 | low |
| `totalWinAmount` | number | 0..10000 | 1 | low |
| `totalBetAmount` | number | 0..5000 | 1 | low |
| `pMiss` | number | 0..0.6993637127741004 | 1 | low |
| `dCheat` | number | 0..0.20487501727876123 | 1 | low |
| `vibeLevel` | number | 0.1..0.3 | 1 | low |
| `countperiods` | number | 0..113 | 3 | low |
| `questDays` | number | 0..11 | 3 | low |
| `mansionCook` | number | 0..0 | 0 | low |
| `cellarClean` | number | 0..1 | 1 | low |
| `intoxic` | number | 0..0 | 0 | low |
| `allure` | number | 20..30 | 1 | low |
...
| `kitchenClean` | number | -200..0 | 1 | low |
| `zackClean` | number | -200..0 | 1 | low |
| `rngesus` | number | 0..12 | 3 | low |
...
| `return` | string | `Beginning`, `caineOutside`, `chipCage` | 64 | low |
...
- **name** (33): `name`, `pet.name`, `garage.street.name`, `garage.city.name`, `garage.mansion.name`, `garage.luxapt.name`, …
...
| 5 | 2026-04-19T10:37:26.611Z | 14m 35s | 54 | 54 | 77 | no |

## Graph coverage (observed vs. static)

- Static-graph edges (every navigation parsed from passage source): **3586**
- Observed edges during play: **351** unique `(from, clicked_text, to)` tuples.
- Static edges covered by at least one observation: **371** (a single observation covers every static edge with the same `(from, to)` pair — gated branches collapse to one observable move).
- Observed-only edges (no matching static edge, typically self-loop `<<link>>` wrappers that `<<replace>>` in-place): **43**.
- Coverage: **10.35%** of the static graph explored.
- Synthetic edges (Claude's out-of-band `eval`/`keys`/`restore`/`pop`): 74
...
- Distinct passages visited at least once: **47** — playable-passage coverage: **2.9%**.
