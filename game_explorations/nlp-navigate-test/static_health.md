# Static health — nlp-navigate-test

Generated: 2026-04-17T16:06:01.800Z

## Verdict: ❌ FAIL

| Metric | Value | Bar | Result |
|---|---|---|---|
| definite_ambiguous_rate | 0.7% | ≤ 5.0% | ✅ |
| potential_ambiguous_rate | 2.2% | ≤ 20.0% | ✅ |
| dynamic_goto_rate | 0.2% | ≤ 10.0% | ✅ |
| effective_gate_unknown_rate | 45.0% | ≤ 30.0% | ❌ |
| complex_setter_density | 0.5% | < 5.0% | ✅ |

## 1. Click-text ambiguity

Edges bucketed by (source_passage, click_text). Raw duplicate buckets (≥2 edges sharing the key): **252**. Most of these are expanded `<<if>>/<<elseif>>` chains where only one gate fires at a time — not a real runtime ambiguity.
...
- `tableClear` → "Finish shift" × 2 (gates: 2 true, 0 unknown, 0 false)
- `tallMan` → "Clean up" × 2 (gates: 2 true, 0 unknown, 0 false)
- `zackPhone` → "Ask him on a date" × 2 (gates: 2 true, 0 unknown, 0 false)
...
**Top offenders:**
- `trainDestination` → "Step off" × 9 (gates: 0 true, 9 unknown, 0 false)
- `trainSit` → "Step off" × 9 (gates: 0 true, 9 unknown, 0 false)
- `trainStand` → "Step off" × 9 (gates: 0 true, 9 unknown, 0 false)
...
- `spinJob` → "Staff room" × 2 (gates: 0 true, 2 unknown, 0 false)
...
- 1920 edges have non-empty gate stack
- true: 225 | false: 768 | unknown: 927
- raw unknown rate: 48.3%

**Unknown breakdown by gate variable class:**
- temp-var-only (`_var` — resolves at runtime): 63
- story-var-only (`$var` — real evaluator gap): 596
- mixed: 268

**Effective unknown rate** (temp-only gates excluded since they resolve at runtime when the current passage is being rendered): 45.0%
