# nlp-phase0-fix-test — Exploration Report

Generated: 2026-04-17T09:41:28.033Z
Source URL: https://mopoga.com/new-life-project

## Session Summary

- Sessions run: 1
- Total wall-clock: 10m 42s
- Total clicks: 50
- Total choices explored: 50
- Unique states seen: 49
...
| `apt401Key` | boolean | false | 0 | high |
...
| `vibeLevel` | number | 0.3..0.3 | 0 | low |
| `countperiods` | number | 0..17 | 10 | low |
...
| `inhib` | number | 100..125 | 0 | low |
| `corrupt` | number | -25..0 | 2 | low |
...
| name | type | range / samples | mutations | confidence |
|---|---|---|---|---|
| `charPron.gender` | string | `female` | 0 | low |
| `charPron.subject` | string | `she` | 0 | low |
| `charPron.object` | string | `her` | 0 | low |
| `charPron.possessive` | string | `her` | 0 | low |
| `charPron.reflexive` | string | `herself` | 0 | low |
| `charPron.determiner` | string | `her` | 0 | low |
| `charPron.contraction` | string | `she's` | 0 | low |
| `charPron.noun` | string | `woman` | 0 | low |
| `charPron.partner` | string | `girlfriend` | 0 | low |
...
| `return` | string | `Beginning`, `exploreIntro`, `introCafe` | 49 | low |
...
|---|---|---|---|---|---|---|
| 1 | 2026-04-17T09:30:45.961Z | 10m 42s | 50 | 50 | 49 | no |

## Graph coverage (observed vs. static)

- Static-graph edges (every navigation parsed from passage source): **3586**
- Observed edges during play: **50** unique `(from, clicked_text, to)` tuples.
- Static edges covered by at least one observation: **62** (a single observation covers every static edge with the same `(from, to)` pair — gated branches collapse to one observable move).
- Observed-only edges (no matching static edge, typically self-loop `<<link>>` wrappers that `<<replace>>` in-place): **2**.
- Coverage: **1.73%** of the static graph explored.
