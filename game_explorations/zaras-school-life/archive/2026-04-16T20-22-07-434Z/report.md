...
Generated: 2026-04-16T20:18:13.896Z
Source URL: https://mopoga.com/zaras-school-life

## Session Summary

- Sessions run: 1
- Total wall-clock: 40s
- Total clicks: 3
- Total choices explored: 3
- Unique states seen: 4
...
| `return` | string | `Welcome Page`, `dream1`, `wakeup_dream` | 2 | low |
| `showMinute` | string | `00`, `01` | 1 | low |
...
|---|---|---|---|---|---|---|
| 1 | 2026-04-16T20:17:33.629Z | 40s | 3 | 3 | 4 | no |

## Graph coverage (observed vs. static)

- Static-graph edges (every navigation parsed from passage source): **2416**
- Observed edges during play: **3** unique `(from, clicked_text, to)` tuples.
- Static edges covered by at least one observation: **4** (a single observation covers every static edge with the same `(from, to)` pair — gated branches collapse to one observable move).
- Observed-only edges (no matching static edge, typically self-loop `<<link>>` wrappers that `<<replace>>` in-place): **1**.
- Coverage: **0.17%** of the static graph explored.
