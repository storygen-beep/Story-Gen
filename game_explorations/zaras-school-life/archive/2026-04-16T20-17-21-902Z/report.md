...
Generated: 2026-04-16T20:17:19.022Z
Source URL: https://mopoga.com/zaras-school-life

## Session Summary

- Sessions run: 1
- Total wall-clock: 2m 53s
- Total clicks: 2
- Total choices explored: 2
- Unique states seen: 3
...
| name | type | range / samples | mutations | confidence |
|---|---|---|---|---|
| `PlayerCorruption` | number | 0..1 | 1 | high |
...
| `famSexUnlocked` | boolean | false | 0 | high |
...
| `PlayerMoney` | number | 0..200 | 0 | low |
| `PlayerEnergy` | number | 100..100 | 0 | low |
| `PlayerMaxEnergy` | number | 100..100 | 0 | low |
| `PlayerSchoolRep` | number | 0..0 | 0 | low |
| `PlayerFitness` | number | 0..0 | 0 | low |
| `PlayerActiveQuest` | number | 0..0 | 0 | low |
| `PlayerArt` | number | 0..0 | 0 | low |
| `PlayerSci` | number | 0..0 | 0 | low |
| `PlayerComp` | number | 0..0 | 0 | low |
| `playerActiveQuests` | number | 0..0 | 0 | low |
...
| `dayOfWeek` | string | `Monday`, `Tuesday` | 1 | low |
...
| `return` | string | `Welcome Page`, `dream1`, `wakeup_dream` | 1 | low |
| `showMinute` | string | `00` | 0 | low |
| `showHour` | string | `08` | 0 | low |
| `timeOfDay` | string | `Day` | 0 | low |
...
|---|---|---|---|---|---|---|
| 1 | 2026-04-16T20:14:25.529Z | 2m 53s | 2 | 2 | 3 | no |

## Graph coverage (observed vs. static)

- Static-graph edges (every navigation parsed from passage source): **2416**
- Observed edges during play: **2** unique `(from, clicked_text, to)` tuples.
- Static edges covered by at least one observation: **3** (a single observation covers every static edge with the same `(from, to)` pair — gated branches collapse to one observable move).
- Observed-only edges (no matching static edge, typically self-loop `<<link>>` wrappers that `<<replace>>` in-place): **1**.
- Coverage: **0.12%** of the static graph explored.
...
- `static_graph.json` — every navigation edge parsed from passage source (M2)
