...
Generated: 2026-04-18T10:15:11.292Z
Source URL: https://mopoga.com/shady-deals

## Session Summary

- Sessions run: 3
- Total wall-clock: 43m 30s
- Total clicks: 74
- Total choices explored: 74
- Unique states seen: 245
- Unexplored frontier (queued for next session): 0
- Endings reached: 1 (Debug)
...
|---|---|---|---|---|
| `energy` | number | 6..100 | 1 | high |
| `money` | number | 0..500450 | 3 | high |
| `reputation` | number | 0..20217 | 4 | high |
...
| `pc_boobs_size` | number | 5..5 | 0 | medium |

### time (1)

| name | type | range / samples | mutations | confidence |
|---|---|---|---|---|
| `hour` | number | 20..20 | 0 | high |
...
| `pc_lips_piercing` | boolean | false, true | 1 | high |
| `pc_nipple_piercing` | boolean | false, true | 1 | high |
| `pc_navel_piercing` | boolean | false, true | 1 | high |
...
| `met_mechanic` | number | 0..1 | 1 | medium |
...
| `charm_sex_buff` | number | 0..1 | 1 | low |
| `charm_base` | number | 3..30 | 3 | low |
| `charm` | number | 3..31 | 3 | low |
| `base_energy` | number | 6..100 | 1 | low |
| `energy_mod_boss` | number | 0..0 | 0 | low |
| `energy_mod_car` | number | 0..0 | 0 | low |
| `stam` | number | 6..100 | 2 | low |
| `combat_base` | number | 3..20 | 2 | low |
...
| `adolesence` | string | ``, `Bully`, `Leader` | 2 | low |
| `first_crime` | string | ``, `Robbery` | 1 | low |
...
| `city_name` | string | `Default Name`, `Rackettown` | 1 | low |
| `nc_name` | string | `Swaying Bazongas` | 0 | low |
| `boss_type` | string | `Bandit Boss` | 0 | low |
| `band_name` | string | `Band`, `Faction` | 1 | low |
| `original_band_name` | string | `Band`, `Faction` | 1 | low |
...
- **order** (15): `order_1_inv`, `order_2_inv`, `order_3_inv`, `order_1`, `order_1_desc`, `order_1_reward`, …
...
- **yacht** (12): `yacht`, `yacht_trader`, `yacht_dampers`, `yacht_cargo_max`, `yacht_trader_cd`, `yacht_party_initiated`, …
...
| 3 | 2026-04-18T09:57:52.643Z | 17m 19s | 44 | 44 | 97 | yes |

## Graph coverage (observed vs. static)

- Static-graph edges (every navigation parsed from passage source): **2120**
- Observed edges during play: **44** unique `(from, clicked_text, to)` tuples.
- Static edges covered by at least one observation: **69** (a single observation covers every static edge with the same `(from, to)` pair — gated branches collapse to one observable move).
- Observed-only edges (no matching static edge, typically self-loop `<<link>>` wrappers that `<<replace>>` in-place): **13**.
- Coverage: **3.25%** of the static graph explored.
- Synthetic edges (Claude's out-of-band `eval`/`keys`/`restore`/`pop`): 200
...
- Distinct passages visited at least once: **70** — playable-passage coverage: **10.3%**.
