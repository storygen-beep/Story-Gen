...
Generated: 2026-04-18T08:19:30.568Z
Source URL: https://mopoga.com/shady-deals

## Session Summary

- Sessions run: 1
- Total wall-clock: 11m 45s
- Total clicks: 8
- Total choices explored: 8
- Unique states seen: 28
...
| `money` | number | 0..200 | 1 | high |
| `reputation` | number | 0..93 | 1 | high |
...
| `heat` | number | 0..3 | 1 | low |
| `gangs_heat` | number | 0..6 | 1 | low |
| … | … | … | … | and 525 more |
...
| `city_name` | string | `Default Name`, `Ambush Alley` | 1 | low |
| `nc_name` | string | `Swaying Bazongas` | 0 | low |
| `boss_type` | string | `Bandit Boss` | 0 | low |
| `band_name` | string | `Band` | 0 | low |
...
- Observed edges during play: **7** unique `(from, clicked_text, to)` tuples.
- Static edges covered by at least one observation: **15** (a single observation covers every static edge with the same `(from, to)` pair — gated branches collapse to one observable move).
- Observed-only edges (no matching static edge, typically self-loop `<<link>>` wrappers that `<<replace>>` in-place): **2**.
- Coverage: **0.71%** of the static graph explored.
- Synthetic edges (Claude's out-of-band `eval`/`keys`/`restore`/`pop`): 25
...
- Distinct passages visited at least once: **18** — playable-passage coverage: **2.7%**.
