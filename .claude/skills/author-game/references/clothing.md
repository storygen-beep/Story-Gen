...
`type` the engine evaluates in `triggerConditionsSatisfied` (`v2.py:3611-3646`).

| Axis | What it is | How it reads | Gates | Predicate `type` |
|---|---|---|---|---|
| **`worn_corruption`** | how revealing the *currently equipped* outfit is | **live**, MAX over equipped items (`getWornStatMax`, `v2.py:1232-1251`); take the outfit off and the gate closes | PUBLIC / world events | `worn_corruption` |
...
re-reads them — `getWornStatMax` iterates `player.equipped` every call, `v2.py:1238-1248`).
...
comment, `v2.py:1231`). They are different quantities and the engine treats them as such.
...
`getWornStatMax('beauty')` (`v2.py:1252`). RTS-lore function names like `getBeauty`/`getExb`/`getCorruptionLevel`
...
- **MAX-aggregate, live** (`v2.py:3615-3624` → `getWornCorruption` → `getWornStatMax('corruption')`,
  `v2.py:1253`): returns the highest `corruption` among equipped items. `operator` defaults to `gte`,
  `value` defaults to 0 (`v2.py:3617-3618`). Change clothes, next render re-reads. A key you hold, not a
  level you bank.
- **WEAN — Wardrobe-Effect-Adds-Nothing.** A `worn_corruption` beat is prose/flavor only — it must NOT
  ...
  touch the global player.corruption trait," `v2.py:1231`.)
...
`v2.py:1252`; predicate at `v2.py:3615-3624`). Use it for: warmer stranger reception, entry to a nicer venue,
...
- **Monotonic — no daily decay.** Do NOT add it to `[engine.daily_tick].traitEffects` (there is no hardcoded
  passive — the tick only applies what you author, `v2.py:5115-5139`).
...
`type` string (`getWornTypes` collects unique non-empty `type` tags across equipped items, `v2.py:1259-1278`;
predicate at `v2.py:3630-3646`). `operator` is `eq` (member) or `neq` (not member); empty value never matches.
...
dress nulls both, `v2.py:1198-1201`; and a dress satisfies a `top`/`bottom` coverage requirement, §8).
...
(`v2.py:885-901`) — cover every slot or she starts under-equipped and the coverage gate (§8) can't be
...
- `buyItem` (`v2.py:1548-1575`) requires `money >= price` (`:1562`) AND `triggerConditionsSatisfied(item.conditions)`
  (`:1564`) before debiting money + adding to wardrobe; on a gate miss it toasts "Not yet — needs …".
- `equipItem` re-checks `item.conditions` (`v2.py:1195-1196`) — a buy-gate stays enforced even if equipped
  some other way.
- **Make the buy-gate a `corruption gte` condition specifically.** The shop UI reads the FIRST
  `trait_key="corruption", operator="gte"` value via `getCorruptionThreshold` (`v2.py:1480-1489`) to draw the
  "🔒 Corruption N+" badge (`v2.py:1676`). A buy-gate written any other way still *enforces* (it runs through
  ...
  *(Code-vs-lore note: the shop's section headers are hardcoded UI bands keyed off the item's gate THRESHOLD,
  not the player's corruption — `Basic 0 / Cute 45 / Bold 85 / Daring 135` (`v2.py:1591-1595`). So an item
...
How the engine resolves it (`checkLocationClothing`, `v2.py:1411-1451`): on entry it walks the location's
`clothing_rules` and picks the **first rule whose `conditions` are satisfied** (a rule with no/empty
conditions is auto-active, `v2.py:1428-1431`); for that rule it checks every slot in `slots_required` against
`player.equipped`, and a `dress` covers both `top` and `bottom` (`v2.py:1444`). Any missing slot → it returns
...
`setup.canRemoveSlot` reads `req.always_required`, `v2.py:1362`/`:1367`, used at `:1348`). Use it for a hard
...
- `references/trait-catalog.md` — `corruption` / `exhibitionism` ranges, defaults, no-decay, sidebar render;
  `getCorruptionLevel` tiers `[0,5,15,30,45]` (`v2.py:5486`).
