...
| `auto_exit` | — | bool | default **true**. `false` = a **TRANSIT STOP**: no engine `[[Leave <name>]]` link, and an empty nav list is read as intentional so the list-every-location fallback stays quiet. For a location reached and left **by canvas** (a car drop-off, a lift) — the engine's tree model assumes every location is a root-with-children or a child-with-a-parent, and a transit stop is neither. **The author owns the way out; there is no safety net.** Both halves are required: without the fallback half, the location dumps the whole map. (`v2.py _generate_hierarchical_navigation`; the label `[[Leave {location.name}]]` is otherwise a hardcoded f-string with no override.) |
