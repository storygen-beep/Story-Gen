...
| **`gates.py` citations are mostly unanchored** | ⚠️ **CORRECTED 2026-08-29, same day it was written.** The measurement holds: **26 of 28 `gates.py:NNNN` citations resolve as UNVERIFIABLE**, and two of them drifted that day (`:405`→`:409`, `:6515`→`:6519`) with the checker silent — both caught by hand. **The CAUSE given here was wrong.** It said the checker *"anchors on a code token after the citation"*; `cite_check.py:221` sets the context window to `m.start() - 200` through `m.end() + 200`, so it already reads 200 characters in **both** directions. Measured properly: of 403 UNVERIFIABLE citations project-wide, **304 have no code token near them at all** — unverifiable by construction, which is what the verdict name means. The proposed fix (resolve a dotted `Class.method` to its `def method(`) would rescue **3 of 403**, so it was not built. What is actually true is duller and worth writing down: a citation written as bare prose with no identifier beside it cannot be checked by anything, and `gates.py`'s are mostly written that way. |
| **`fill_finished` — FOUND, FIXED, still uninstrumented** | Found 2026-08-29 by the Attack Panel's first run: every location declares a finished-world word budget and `grep fill_finished` returned **0** in both scripts. `pitch_pack.py` now prints **both** budgets and flags a finished figure below its working one. `mrs_vance`'s `kerr_crossing` was the one flagged — and git says it was **not a typo**: `df5b66e` deliberately re-declared `fill` 300→620 for the truck chain, wrote a `fill_note` explaining why, and left `fill_finished` on 300. A half-finished edit that nothing could see. Corrected to 620, the only number in evidence (this file's own finished==fill rooms). ⚠️ **`gates.py` still does not read the field** — the gate tally was 44/44 before and after the change, which is the proof. Whether it should is undecided: the working `fill` is what gate 1 judges, and a second budget with a gate on it would need a rule nobody has written. |
| **The load rules — DOCTRINE DRIFT, found 2026-08-30, and nine shipped games carry it** | LO read `the_route` beside `new-life-project` and said ours was harder to follow. He was right, and this skill caused it. Measured over 27 corpus games / 14.5M prose words against every game here: **9 of 9 games this skill authored sit above the entire field's maximum** on `, which is` glosses (field MAX 0.24 per 1k, ours 1.34–2.71) and on negation (field MAX 20.22% of sentences, ours 22.5–38.0%); 8 of 9 on history-on-a-repeatable (field MAX 5.41%, ours 4.8–12.1%). **The six games the v1 skill authored mostly do not** — the split is clean, and the lowest v2 game is double the highest v1 one on the gloss. The mechanism is `register.md`'s own "Sweeping backwards": *replace the hedged clause with the specific one* produces a specifying clause, and a gloss **is** one. Shipped as three lints and a new `register.md` section, **"The load rules"**. ⚠️ **The prose in nine games is not fixed** — `the_route`'s rewrites are written and measured (`work_roy_morning`: 319 words → 254, negation 41% → 3%, history 29% → 0%, every fact intact) and land separately on LO's word; the other eight are untouched per the standing rule. ⚠️ **The 2026-08-28 padding study is not contradicted**: it measured `-ly` adverbs, hedges, commas, trigrams and vocabulary variety, correctly found nothing, and none of those five is any of these three. It looked for volume; this is load. |
