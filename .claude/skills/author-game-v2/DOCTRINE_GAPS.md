...
| **6** | **Onboarding** | the first hour | `off_season/REVIEW_1.md` O1–O4 N1–N2 | ✅ **ADDRESSED 2026-08-22** → `the-first-hour.md` F1–F9, gates `the opening opens a door` · `every hub is met first`, lints `named before met` · `the place says what it is`. ⚠️ **A third gate shipped here — `the anchor introduces itself` — and was DELETED 2026-08-26**: it required a device 18 of 26 field games do not use, and F9 now teaches the location's own description instead. Row corrected 2026-08-28, after the deletion was found still documented in six places. |
...
| **9** | Clothing / wardrobe as a system — **✅ ADDRESSED 2026-08-28**: the mechanic is exposure, the engine now derives it (`worn_exposure`), and `the-meters.md` W7 + `engine.md` §17 carry it. Not gated: how many places must read it |
| **10** | Sex-loop shape — **✅ ADDRESSED 2026-08-29 → `the-voice.md` R6, the button names what she does. THE FOUR NULLS WERE CLOSED 2026-09-01 → `references/the-arc.md`, A1–A9 — by READING five arcs end to end, not by counting them.** The 2026-08-29 instrument measured passage *shape* (choices per screen, dead-end share, self-loop share) across 20 games and returned a spread with no signal — median 1 choice against a max of 68 — because shape is not where the answer lives. What the arcs actually share is a **numbered ladder of one-time steps that converts into the repeatable surface**, and the first third of it carries no sex at all. ⚠️ **Two claims made while closing this were WRONG and are recorded so they are not re-proposed: (a) a volume floor** — DoL, the reference game, has the LOWEST explicit share in the 25-game field at 4.8% against our 9.3%, and a count would be manufactured; **(b) "sex is a small fraction of an arc"** — true of `course-of-temptation` (111 harasser passages, 20 explicit) and false of `zaras-school-life` (15 detention passages, 15 explicit). Density is a house decision; the ladder is the finding. ⚠️ **Original note, still true:** **Four of the five questions came back null and nothing was written for them** — the shape of the surface (menu against single-exit chain) predicts nothing, and two correlations were withdrawn; the survivor was a label count that needed no correlation. See the Log row for 2026-08-29. ⚠️ **The rule was recorded as shipped and was not on disk.** The CHANGELOG and the Log row both described R6 in detail on 2026-08-29; `the-voice.md` still read *"The five rules"* and stopped at R5, and R6 existed only as a checks-table row, a paragraph beginning *"R6 has no gate either"*, and a citation in `gates.py` — three references to a rule no commit had ever contained (`git log -S "R6 · "` returns nothing). Written the same day it was found. **`--selfcheck` could not see this**: it reconciles gate and lint NAMES against `SKILL.md`, and a rule cited by a reference file with no section defining it is outside what it reads. |
| **11** | Pre-ship discipline — **still open, and it must NOT be built as a checklist.** §3a of this file already rules on that: *"checklists do not hold… v2 must not inherit the checkbox"*, with v1's 13-point audit followed by the very bug it was written to prevent. **Half of it closed 2026-08-23**: `the-release.md` loop step 5 now says the lints get read as well as the gates passing, and the log step records the figures shipped with in `v2_state.json` — a tracked number rather than a box. **✅ THE REST CLOSED 2026-08-29, AND THE INSTRUMENTS ARE WHAT CLOSED IT.** The rule for this item was *"it arrives as instruments or not at all"*, and since the row was written three more shipped: **`--release`** (the built artefact — dev mode, missing media, three version numbers), **`--saves`** (the difference between two releases), and **`--selfcheck`** (this skill against its own scoreboard), beside the 46 gates and 28 lints. ⚠️ **The last piece was WIRING, not building.** The loop stopped at *"Ship, and log"* while the six-step build procedure and `--release` sat two hundred lines below in `§ Shipping the build` — so an author walking the loop was never sent to the check named after the release boundary, which is how `the_inheritance` reached the published grid with a full `--dev --debug` build and 115 missing files. Step 6 is now **Build, and cross the boundary**, logging moves to step 7, and a table names all four instruments in the order they run. ⚠️ **THIS DOES NOT CLOSE AS A GUARANTEE AND MUST NOT BE READ AS ONE.** Nothing checks that an author ran them, and nothing can — a check that a list was read IS the checkbox §3a rules out. What holds is the log step: anything left in a list is left on purpose, named in the ledger, with the reason. That is a discipline. Cause: Off Season shipped **37/38 with 67 flagged words**, two of which reached LO on a button. What remains open is everything the lints do not cover, and it arrives as instruments or not at all. |
| **12** | Optional systems — phone, customization. **PHONE ADDRESSED 2026-08-29 → Study 9, applied. CUSTOMIZATION ADDRESSED 2026-08-29 → `the-want.md` §1 W1–W5 + gate `what she picks is read`. ITEM 12 IS CLOSED.** ⚠️ The customization half landed as **one subsection, not a reference file**, because Part H measured the whole subject at **0.12% of 22,614 comments** against lostness 4.7% and grind 0.9% — a file would over-weight it. ⚠️ **The engine change was REFUSED** on three grounds: the field gates on a created value only **1.8%** of the time, every axis it uses is already expressible here, and nobody is asking. ⚠️ **W5 is NOT P1's refusal rule** — P1 had a verdict (24 likes to 0); here there is none in either direction, so the rule is conditional. The engine has shipped **eight** phone app types since doc 45 (`v2.py:2458-2465`) and the skill said nothing about any of them — a grep for `phone` returned four incidental hits. Consequence, measured across all thirty games in `games/`: **`post_actions` authored zero times, `scheduleEffects` zero times**, three of the five games with a phone give it one app, `under_one_roof` ships a feed with 34 posts she **cannot post to**, and `mothers_place` declared `phone ON (8_phone.toml)` in its systems spec and shipped with **no such file and no `[phone]` block**. Now `references/the-phone.md`, P1–P11, and one line in `SKILL.md` marking it the one optional world file. ⚠️ **P1 is a refusal question** — most games should not have a phone, and a thin one is worse than none. ⚠️ **P3 (a message is ~15 words, pooled over 369 real bubbles in three games and two languages) must NOT become a gate** — a threshold there fails a correct three-word message, per the R4 / study-6 / P0 / duplicate-wardrobe precedent. ⚠️ **P11 is the one rule where corpus prevalence and the player verdict point in opposite directions** (17/27 games ship a battery; the players' cleanest verdict in 622 comments, 24 likes to 0, is that everyone hates it) — the verdict wins, because prevalence measures what authors built, not what worked. Nothing gated yet; two zero-failure candidates named at the foot of the file. |
| **13** | **Prose that copies a field** — a price, a window, a room name written into a beat is a duplicate of the TOML with no link back, and it goes stale the day the field moves. v1 carried a whole file on this (`author-game/references/prose-truth.md`, 121 lines); a grep of the v2 skill for `prose is a copy \| re-price \| stale prose` returns **nothing**. Found 2026-08-22 while closing the currency item. ⚠️ **That "first measured instance" was in the SKILL's own worked example** — `the-voice.md` R1 taught *The Lodger's Room* until the house vocabulary rule moved — **not in a game**, and `cite_check.py` plus the own-words lint cover that class now. **✅ ADDRESSED 2026-08-29 → `the-release.md` § The prose quotes the fields.** ⚠️ **NO GATE AND NO LINT SHIPPED, and the reason is measured.** Nothing in the repo is stale: 128 money amounts written into prose all match a declared field, and rent-in-prose against the declared obligation returns zero mismatches. What IS true is the exposure — **ten of eleven rent-enabled games hand-write their own rent amount**, the most fragile copies sitting inside the rent block beside `amount` (`off_season` three, `late_shifts` two). A debt list, not a bug list. ⚠️ **The digit sweep found ZERO and was wrong to be believed**: `125` is written *"Hundred and twenty-five"*, so the spelled-out sweep found 122 candidate strings where the numeral sweep found none — that lesson is the section's load-bearing warning. ⚠️ **Most of this item was already checked under other names** — gate `a price is on its label` compares the amount, and four lints cover places, hours, currency and house words; the section's table says so, and says that a spelled-out amount in a beat is the one coupling nothing catches. |
...
| **16** | **The release boundary** — what separates a test build from a published one, and whether anything holds it. **✅ ADDRESSED 2026-08-28 → `mrs_vance/REVIEW.md` B2, applied.** The rule is LO's and it is right — *dev mode and missing media block RELEASE, not testing* — and it lived **only as a comment on a JS object literal** (`games-data.js:44-49`), restated by hand in **nine of twenty-eight portal entries in three wordings**. `gates.py` had **zero lines that read a built game**; `the-release.md`, 164 lines and named for this, never mentioned `--dev`, `--debug` or a build. ⚠️ **Every gate in this skill measures the SOURCE, and a release is the one moment the ARTEFACT is what is judged** — which is why the drift was already shipped: **`the_inheritance` sits in the published grid carrying a full `--dev --debug` build with 115 missing files**, and `forty_miles` reads portal `0.1` / `[project] 0.1.2` / archives `{0.1, 0.1.1, 0.1.2}`. Now `the-release.md` **§ Shipping the build** (six steps lifted from the JS comment, not invented, plus the rule the schema never stated: **`dev: true` and `version` are mutually exclusive**) and `gates.py --release`. ⚠️ **B2's own proposed instrument was CORRECTED, not adopted** — the `[IMAGE MISSING]` markers it named are `--debug`-only (`v2.py:12404`), so the grep would have passed `under_one_roof` with **183 missing files**; the check reads the flags-init map and `MissingMediaPage` instead. ⚠️ **Byte-equality against the archive is printed and never judged** — vesper's differ, and vesper is the one game whose triangle is whole. **Baseline: 0 of 29 builds clean.** |
| **17** | **The returning player** — what a new release does to a save written by the old one. **✅ ADDRESSED 2026-08-29 → `references/the-returning-player.md` + `engine.md` §40.** ⚠️ **This is not a gap v2 discovered; it is one v2 LOST.** v1 shipped `references/save-safety.md`, 141 lines, written after a live soft-lock: Vesper's `cover_analyst` was granted on a one-shot dispatch that every 0.1.4→0.1.5 carry-over had already burned, so the save jammed on that floor forever. **v2 was derived without it and carried nothing** — across all of v2's doctrine there was no statement that renaming a canvas id strands every save in the wild, and no gate can see it: every check in this skill reads one snapshot of `7_final_game.toml` and a join-key break is only visible in the **difference between two releases**. The engine half was measured the same day and was worse than believed: `setup.backfillStateDefaults` covered flags / `player.core_traits` / npcs only, so turning the phone (or rent, passes, inventory, clothing) on in a patch release left the whole sub-map `undefined` in every existing save — the engine's own comment said so at the `media_cycle` guard, one of exactly **two** hand-written `ndef` heals ever added, both after something broke. Now the skeleton is one object serialized to both `:: Start` and `setup.stateDefaults`, the backfill reaches `$game_state` and `$player`, and every save records the release it started on. **The check shipped the same day**: `gates.py --saves <slug>` diffs the current build's join keys against the newest `releases/v<version>.html` and exits non-zero on a removal — one command, not four greps, because §3a already ruled on checklists. ⚠️ **It found breaks that had already shipped**: three passages dropped between vesper 0.1.3 and 0.1.7, one between forty_miles 0.1 and 0.1.2. Saves parked on those landed nowhere and nothing said so at the time. **Still human, and stated in the check's own output: a rescaled stat (the key never moves) and a burned one-shot grant (not a name at all).** |

| **18** | **Whose want starts the game** — whether the protagonist's own appetite is the opening position, and what carries content once the opening pressure goes soft. **✅ ADDRESSED 2026-09-01 → `the-want.md` §1b/§2/§3, `templates/want.md`, `the-economy.md` **R3d**, `state.md`.** LO asked the question; four of four `WANT.md` files had answered §2 with one sentence shape (*"to be wanted by men who…"*) because the template's ✅ example **was** that sentence — the protagonist-drift mechanism (item 14) one section further down, and the skill's own *"a template is copied harder than a reference"*, fifth instance. Measured: the obligation gates **57 of 91,814** conditions in `degrees-of-lewdity` and the tier rungs gate **1,336**, so §2's claim to be *"the line that decides whether the game can run forty updates"* was false and the release-41 test moved to §3. ⚠️ **Scope shrank on contact with the file it was going to duplicate** — `the-economy.md` R3/R3b/R3c already carry the date, the face, the ratchet and DoL's exact ladder line, so the proposed new obligation section was dropped and only the one thing they lack shipped: **R3d, that the bill is allowed to go quiet.** ⚠️ **No gate** — four corpus games carry an obligation. Study: `~/Documents/Ignition_Study_20260901/`. |

| **19** | **The systems list** — what the game keeps track of about her, decided before the rooms exist. **✅ ADDRESSED 2026-09-02 → NEW `references/the-systems.md` (SY1–SY6), `state.md` `board.systems[]` + `board.locations[].labels`, `the-board.md` §1, `the-surfaces.md` R2c, `the-sheets.md`, and one lint.** ⚠️ **This is not a gap the field revealed; it is one this skill wrote down and then did not build.** `the-surfaces.md` R2c said on 2026-09-01 that *"the room list cannot be written before the systems list is"* — and the phrase appeared three times in the skill and was defined **nowhere**: no file, no ledger field, no sheet, no board step, no check. `the-release.md` never uses the word *system* at all, so a release adds events to existing surfaces and a system could only be born in a board phase that had no place for one. Measured on the female-lead set (`~/Documents/Systems_Study_20260902/`): **systems come in two kinds** — *ambient*, fed by nearly every room (`family-ties` writes time in 49 of 50 rooms), and *sourced*, fed in one or two places and read all over (piercings **2 rooms → 117** read sites, clothes **1 → 53**, the skill ladder **1 → 19**; `zaras-school-life` corruption **18 → 376**). **A game of only ambient systems ships a duty list**, which is `night_desk`. And **a room declares what kind of place it is** — `course-of-temptation` carries **349 distinct tags, median 9 per room**, half read by the map and half by the systems; `degrees-of-lewdity` runs the coarse form, 2,760 room screens over **69 place types** with 616 sites keying off them. ⚠️ **Nothing was ported from v1**, LO's call: `author-game/references/system-patterns.md` carries nine authored recipes and was read and excluded, because a cookbook written from taste is exactly what this skill's standard forbids. ⚠️ **R2c's own "DoL 693 · night_desk 6" is RETIRED by this work** — it compared a `$var` census against our declared traits and overstated by an order of magnitude; matched, the field median is **82** against our **37.5**, a factor of two, and composition rather than count is the gap. ⚠️ **One lint, no gate**, and the direction is why it is safe: declaring more labels makes the output *worse*. P0 is respected — every game declares zero today, so a gate would fail twelve games for the age of the doctrine. **OPEN HALF — the notice.** `degrees-of-lewdity` prints a line from whichever system has news onto **2,736 of its 2,760** room screens (*your rented book is due*), which is the answer to lostness and the one mechanism here that does not scale with room count. Specified at `the-systems.md` SY6 with the build path named (`v2.py:9848` already emits a conditional chain onto the room screen), **and deliberately not built** — LO scoped this round to docs. It arrives as an instrument or not at all. ⚠️ **Also open and stated rather than fixed: a canvas belongs to exactly one location** (`template_import.py:1939`), so the field's write-once-appear-everywhere construction is unavailable and a row wanted in three rooms is authored three times. Priced honestly at our 8–14 locations; revisit past ~30. ⚠️ **EXTENDED SAME DAY → `the-systems.md` SY2b, from a SECOND study run independently (`~/Documents/Load_Bearing_Systems_Study_20260902/`), and it closes a hole in SY2 above.** Its field tables, its `worn_exposure` count and its DoL passage count were re-run before they were believed and match row for row. The finding: **SY2's shape — fed in one place, read in many — is satisfied perfectly by a plot bookmark, and all eight of ours are one.** Verified by running SY2's own test over them: `margaret_broken` 1 write / 21 reads, `first_shift_done` 1/24, `met_roy` 1/12 — eight for eight, and not one describes her, against `zaras-school-life` whose 9 include `PlayerCorruption` at 376 reads / 3 writes and six that describe her. ⚠️ **And we are not short of descriptive systems — we run them backwards**: 50 body-and-disposition systems across the twelve v2 games, median read:write **0.40**, and **zero** clearing the study's 10:1 bar. The costliest instance is `worn_exposure`, an engine predicate built 2026-08-28 specifically so a scene could ask whether she is covered, and **read three times in 26 built games**. ⚠️ **No gate and no lint**: the 10:1 bar is invented (field spread 3–121), and a parser cannot decide whether a value describes her — a check that pretended otherwise would fail correct introduction bookmarks, which `the-first-hour.md` F5 requires. ⚠️ **The study's own §8 proposed this as `W8` in `the-meters.md`; that placement was decided before `the-systems.md` existed and was NOT followed** — SY1/SY2 own what-kind-of-system, and a second id in a second file is the R6 defect the study itself named. ⚠️ Two claims it refuted mid-run are recorded in SY2b so they are not resurrected: *"our systems don't read each other"* (false — coupling shape matches) and *"no game we have built has a load-bearing system"* (false — `the_inheritance` has two, at a higher density than DoL). |
...
| date | what |
|---|---|
| 2026-09-01 | **The offer — the third kind of choice, found because a shipped game hid its own best one.** `mrs_vance`'s ask (*"Tell him you will be short on Friday"* — he covers the rent, and owing him opens his ladder) is gated on `money lt 260` against an income of **$208 a day and a $260 week**, so it is close to unreachable; `commuter` repeats the shape (`prep lt 30` → `prep +45`). **Two authors, no shared context, both obedient** — R5b–R5d covered the choice she is *refused* and the one she *declines*, and the file had no word for a choice offered **before she needs it**. ⚠️ **The field check ran BEFORE the rule was written**, because three claims died earlier the same session for resting on n = 1: three of the four corpus obligation games let her volunteer for a bigger bill — DoL *"Take on Robin's debt"* (**doubles the rent, permanently**, gated on `$robindebtknown`, price on the button in red, *"irreversible"* stated before the commit, and what it buys is the PERSON across 42 `$robinpaid` sites) · `course-of-temptation` Greek dues (`!$firsttime.greekduespaid`) · `corpo-life` apartment tiers (`$rent` 200 → 30,000, gated on not owning it) — and **not one is gated on scarcity**; the gates are *knowledge* and *not-yet-done*. SHIPPED: **`the-surfaces.md` R5e**, *a gate opens when she has earned it; an offer is there before she needs it*, test **"can she take this when she doesn't need to?"** ⚠️ **The obvious phrasing is refused in the rule itself** — *"never gate a grant on a lack"* is false, a rescue should require needing rescue, and **50 sites across our games do it correctly**. ⚠️ **No gate and no lint:** the signature (*the grant clears the very lack that gated it*) needs two exclusions — the ceiling pattern, and once-only counters written `X lt 1` → `X +1` — before it discriminates, and the subject is then **2**; unfiltered it fires on **49 correct implementations**, the R4 failure exactly. The signature is recorded inside R5e for the day it has a subject. **Bonus for R3d:** `new-life-project` caps its rent as well, making it **4 of 4** obligation games with a hand-placed ceiling. Verified: rule pointers **115 → 116, 0 dangling**; lints 32/32; gate rows 46/46; no false friend; `gates.py` untouched. Study: `~/Documents/Ignition_Study_20260901/` (`offers.py`). |
| 2026-09-01 | **The appetite was never the engine — item 18, and the third study in a row whose scope shrank on contact with the file it meant to fix.** LO asked whether we design the protagonist's appetite and then build the world to feed it. We did: `templates/want.md` §2 shipped a filled-in ✅ example — *"be wanted by people who shouldn't want her"* — and **four of four `WANT.md` files wrote that sentence**. Item 14's mechanism exactly, one section further down, and the fifth instance of *"a template is copied harder than a reference"*. **Measured** (`~/Documents/Ignition_Study_20260901/`, `probe.py` regenerates it): only **4 of 28** field games carry a recurring obligation; **every one of them ratchets and every one caps** — `degrees-of-lewdity` writes seven rent rungs and reaches six (`Math.clamp($rentstage,1,6)`), `course-of-temptation` authors five, ships three, and kills the top two with a literal `false and` beside its own note *"must be removed someday once money is easier"*. **Why capping is correct: the obligation gates almost nothing.** 57 of 91,814 DoL conditions read `$rent*` and they are *is it due* / *can she pay*; **1,336** sites are gated on tier rungs (`<<promiscuity3>>`). ⚠️ **The tier idiom is a WIDGET, not a comparison — a numeric-threshold probe finds 8 where the truth is 1,336, a silent 167× undercount that reads as a finding.** SHIPPED: `the-want.md` §2 stops claiming to be *"the line that decides whether the game can run forty updates"* and becomes the destination; **the release-41 test moved to §3**, where the meters are; §1b asks for a demand with a face and a date; `templates/want.md`'s ✅ example is replaced by a *shape* with an explicit do-not-copy warning; `the-economy.md` gains **R3d** — the obligation is an ignition, not a tax, and may go quiet once the meters carry the gating; `state.md` gains `obligation`. ⚠️ **Scope shrank, deliberately:** R3/R3b/R3c already teach the date, the face, the ratchet and DoL's exact ladder line, so the new obligation section this study proposed was **dropped** rather than duplicating a better file. ⚠️ **No gate — n = 4.** ⚠️ **Three claims withdrawn during the same investigation and recorded so they are not re-derived:** *"move the meter onto her"* (field is 16-of-20 per-person and `the-meters.md` W1 already owns the fork as `board.who_climbs`), a read:write table (superseded by `Player_Legibility_Study_20260825`, which excludes chrome), and *"the ask was never built"* (`mrs_vance` already ships it). **This was the third derivation of the player-identity gap; the memory entry warning "don't derive it a third time" had not been read.** Verified: `--selfcheck` rule pointers **105 → 106, 0 pointing at nothing**; vocabulary re-swept on all three edited files (no new false friend); `mrs_vance`, `the_season`, `back_home`, `vesper` all hold **46 green** — `gates.py` was not touched by this batch. |
| 2026-08-29 | **Pre-ship discipline — item 11, closed by the instruments, and the last piece was wiring rather than building.** The rule for this item was *"it arrives as instruments or not at all"*; since the row was written on 2026-08-23 three more shipped — **`--release`** (the built artefact), **`--saves`** (two releases diffed), **`--selfcheck`** (the skill against its own scoreboard) — beside 46 gates and 28 lints. ⚠️ **The loop named two of the four.** It stopped at *"Ship, and log"* while the six-step build procedure and `--release` sat two hundred lines below in `§ Shipping the build`, so an author walking the loop was never sent to the check named after the release boundary — the same *built-documented-unwired* shape as R6 and the stale `ne` row, both found today. That gap is how `the_inheritance` reached the published grid with a full `--dev --debug` build and 115 missing files. SHIPPED: loop step 6 is now **Build, and cross the boundary** (ending on `gates.py --release <slug>`), logging becomes step 7, and a table names all four instruments in the order they run. ⚠️ **CLOSED AS TOOLS-AND-WIRING, NOT AS A GUARANTEE.** Nothing checks that an author ran them and nothing can: a check that a list was read IS the checkbox §3a rules out. What holds is the log step — anything left in a list is left on purpose, named in the ledger, with the reason. Written that way deliberately so the row is not read as more than it is. **Tier 3 is now closed; every inventory item is addressed.** |
| 2026-08-29 | **Prose that copies a field — item 13, closed with a rule and deliberately without a check.** v1 carried `prose-truth.md` (121 lines); v2 said nothing. ⚠️ **Most of it turned out to be checked already under other names**: gate `a price is on its label` compares the amount against the label, and lints cover places, hours, currency and house words. ⚠️ **And the item's own "first measured instance" was in the SKILL's worked example, not a game.** Measured before writing: **128 money amounts in prose all match a declared field**; rent-in-prose against the declared obligation returns **zero mismatches**; and three unchecked couplings (a threshold named in prose, a band word in prose, an NPC's hours in prose) returned **zero real hits across 14 games** — every candidate was dev-jump text, an author's design note, or a coincidence, plus an off-by-one in my own span check. ⚠️ **THEN v1'S OWN FILE REFUTED MY INSTRUMENT**: *"`125` never appears as a digit in that prose — it's spelled out, in voice, as it should be"*. Re-swept with spelled-out forms: **ten of eleven rent-enabled games hand-write their own rent amount**, ~17 unmistakable copies, the most fragile inside the rent block beside `amount` — `off_season` has three (`greeting`, `cant_pay`, `eviction_response_soft`), `late_shifts` two. **Nothing is stale today; all of it breaks on the next re-price.** The engine half verified live: an authored `greeting` REPLACES the interpolating default (`v2.py:16604`) while the live number prints four lines below (`:16608`) — re-price and the collector contradicts the UI in one screenshot. SHIPPED: `the-release.md` **§ The prose quotes the fields** (the model, the measured exposure, the four amendment edits, a pre-commit grep recipe whose load-bearing warning is *grep the WORDS not the digits*, and a table of what already catches what), a pointer on loop step 4, and one SKILL.md dispatch row. **No gate and no lint** — a check for a debt with no current victim is the P0 error, made and reversed once already today. ⚠️ **Five measurement bugs in one day, all one family**: grepping phase files instead of parsing the build, and searching digits for a value written in words. |
| 2026-08-29 | **Customization: the field says the answer back, and we asked four times without listening.** Item 12's remaining half and the last never-studied subject in the inventory. The engine ships a creation screen; the skill mentioned it three times and its only instruction was *"set it false unless you are actually shipping the fields."* Study `~/Documents/Customization_Study_20260829/`, Parts C/B/H of 11, five instruments kept. **Part C — the field does not GATE on what she created, it PRINTS it**: 1.8% of reads are conditions, ~95% are the value spoken back; field median **4** reads per created field with the median game leaving **none** unread. **Part B corroborates it mechanically** — 82% free text, 0% numeric, and a typed value cannot be gated on; screen size matches ours at a median of 3; **zero stat fields**, which confirms §1's *a memory, not a slider* from a new direction. **Part B's real find: the distinctive axis is the CAST** — *"Veronika is my ___"*, *"What is her relationship to you and $karlee:"* — **and we already ship it** (`relationship_options` + the `@<npc>.rel` token, written **11 times in the whole repo**). **Part H — nobody cares either way**: 22,614 comments, the subject at **0.12%** against lostness 4.7% and grind 0.9%; two ask for a skip button, six ask for more. SHIPPED: `the-want.md` §1 **W1–W5** and gate **`what she picks is read`** (fails only on zero, n/a is not a pass, `sets_portrait` counts as a read — without that exemption every image_select field in all five games fails, which is the R4 error). ⚠️ **ENGINE CHANGE REFUSED** on three independent grounds. ⚠️ **A lint was built and removed the same day**: zero dead declarations exist — the four games that appeared to have one carry a TOML *comment* recording a deferral, which is correct practice. ⚠️ **The central figure was wrong twice before it was right and all three passes reached LO** — *2 reads* (missed the `@player.x` house token), *20 reads* (counted TOML comments, and a lookahead ate `@player.` at a sentence end), then **12 reads across 14 fields with 6 never read**. **The lesson, twice this week: measure the artefact the check reads and PARSE it — do not grep the phase files.** Verified: `--selfcheck` **46 gates** / 28 lints / 46 rows, and it caught the missing index row itself; **0 existing verdicts changed across 22 games**; blast radius exactly as predicted — `last_call` and `late_shifts` PASS, `mothers_place` / `seventh_day` / `the_inheritance` FAIL, 17 n/a; `cite_check` unchanged at 20 drifted / 8 missing. |
| 2026-08-29 | **A stale `STATUS.md` row put a wrong engine recommendation in front of LO, and the evaluator count was one short.** LO asked what engine changes we need; I said `ne` was missing and the fix was *"one word in one list"*. **It shipped 2026-08-24**, and `engine.md` §37 — which documents it correctly — had been rewritten that same day *because its first version made the identical mistake in the same words* (*"three whitelist entries and no runtime work"*). `STATUS.md:275` was never updated and is the line I read. ⚠️ **§37 said THREE evaluators and there are FOUR**: `setup.describeUnmetConditions` carries its own operator chain and phrase table, calling neither of the other two evaluators. It handles `ne` already, so nothing was broken — the section's own rule was undercounting the thing it exists to enforce. ⚠️ **AND SIX LIVE VIOLATIONS OF THAT RULE ARE NOT `ne`**: on a trait, `compare()` accepts **12** operators, `describeUnmetConditions` and `checkSingleCondition` **6** each, `checkQuestsCondition` **5** — so `in`/`not_in`/`contains`/`not_contains`/`exists`/`not_exists` pass on a canvas gate and read FALSE in the locked-reason line, every hint and `_findFlagSetterCanvas`. **Recorded, not fixed**: measured over every predicate item carrying a `trait_key` across the 20 games that have one, the authored operators are **gte 2,287 · lt 527 · eq 191 · lte 31 · gt 1 · ne 0** and none of the six appears. **DECISION — no engine change.** Quest-card `ne` is the one real gap (`checkQuestsCondition` has no case; `template_import.py:5509` excludes it deliberately) and is parked **on evidence**: `ne` has been legal on a canvas since v2 shipped and is authored zero times, because until today the only two mentions in the whole skill were §37 and `the-voice.md` R2's *don't-use-it* trap. **That zero measures absent teaching, not absent demand** — widening the evaluator now is the P0 / R4 / study-6 error. SHIPPED: §37 retitled FOUR with **all nine citations re-anchored** and a new four-set operator table; the do-not-cite bullet *which identifiers orphan a live save* struck (answered 2026-08-29 by `the-returning-player.md` §2–§5, the list's own rule is delete-once-promoted); **`the-board.md` names the six operators a trait gate may use** — no rule, no gate, no threshold; `the-voice.md`'s *third evaluator* → *one of four*; and **six stale `STATUS.md` statements** — the `ne` row, *"42 gates, 17 lints"* (**45 and 28**), *"eight open study sections"* over a table saying all eleven are done, `block_pool` *"used by zero v2 games"* (five use it, `mrs_vance` 77), *"three engine facts"* on a list holding two. ⚠️ **One of my own figures was overstated and caught pre-commit** — I conflated trait comparisons with type-specific operators (`is_present`, `equipped`, `owned`, `is_active`) and re-measured scoped to `trait_key`. ⚠️ **The in-code comment beside that `ne` line still says "THE SECOND EVALUATOR"**; code right, count wrong, **not edited — no engine code is touched here**. Verified: `--selfcheck` 45/45 · 28/28 · 4/4 · 45/45 (and those are the numbers written into STATUS.md); `cite_check` drifted **21 → 20**, OK **61 → 68**, missing **8 → 8**, and **zero drifted citations in any file touched** — ⚠️ but only after a first pass took drift to **22**, because three citations I had just written sat on lines carrying several refs and several code names, so nearest-anchor matching mispaired them. **A long table row is a citation hazard**; **0 of 22 games moved**, with the baseline asserted non-empty *before* the diff was trusted. |
| 2026-08-29 | **The worked examples that were never written — and the rule that says which ones are safe to write.** Prompted by my own claim to LO that the skill *"grades work it never taught"*. **Measured, and it was wrong**: `gates.py` has **45 distinct gate names and every one appears verbatim in a doctrine file** — no orphans — and v2 carries **54 worked TOML blocks against v1's 37**, so it shows more, not less. What survived: the examples are concentrated in the fact sheet and absent from the files that own the judged subjects. **31 of the 54 sit in `engine.md`**, while `the-economy.md` (5,919 words), `the-voice.md` (2,909), `the-map.md` (2,747) and `register.md` (9,428) had **zero** between them. Two subjects had **no worked example anywhere in the skill**, both judged: (a) **a condition on a currency** — zero blocks, while the only money example the skill has ever shown is `engine.md` §27's `costs` block, which is the *other* channel, the one this file's own lint says **seven of our ten rent-enabled games over-use by gating on price alone**; and (b) **a media block nested in a cascade beat**, which `engine.md` §5 states outright *is the shape `register.md` S1 requires* and then models at node level (`back_home` 0/169, `steam` 0/623, `forty_miles` 0/938). ⚠️ **The two directions in `register.md`'s *"an example outranks every rule beside it"* were reconciled rather than picked between.** Instances 1–3 say an example is dangerous; instance 4 says an absence is worse. Both hold, and the line is not size: **show the mechanism, never the world** — a mechanism copied verbatim produces a correct game, a world copied verbatim produced three games with the same box room. All eight new blocks use placeholder ids and no proper nouns. ⚠️ **`the-map.md`'s standing refusal was HONOURED, not reversed** (LO's call): it gains one key, `entry_from` present or absent, and no rooms — plus the recorded fact that `back_home` fails both map gates because it declared **no `board.map` at all**, which no example would have prevented. ⚠️ **Two defects in my own examples were caught before commit and are the reason the check exists**: the first quest card carried `group` **and** `npc_id`, which `template_import.py:1105` ignores with a validator warning; and three blocks split an **inline table across lines**, which TOML forbids. **A worked example that does not parse is worse than none.** ⚠️ **One warning I wrote was overstated and corrected against source**: writing the canvas condition form on a quest card is **caught at build time** (`template_import.py:5509`), not silent. The genuinely silent one is `ne`, deliberately excluded from the card validator because the third evaluator has no `ne` case and falls through to `return false`. ⚠️ **A stale citation in existing doctrine was found while verifying my own**: `v2.py:13952` — cited in `engine.md` §5 and twice in `register.md` S1 for the linkreplace fact — is a broken-exit branch. Repointed to `_render_cascade` `v2.py:14426` and `_render_cascade_tail` `:14512`/`:14572`. SHIPPED: `the-economy.md` R1 (money-condition ladder, ordered highest-first) and R5 (a capped income rung carrying six rules); `the-voice.md` R2 (the ascent ladder as TOML) and R5 (the terminal card); `register.md` S1 (beat-nested media, `file` and `pool_dir` forms) and **`## Show the mechanism. Never show the world.`**; `the-map.md` R3. **No gate, constant or threshold added, and `gates.py` untouched** — `--selfcheck` green, `mrs_vance` **44/44**, `back_home` **16/36**, `forty_miles` and `vesper` byte-identical: **0 verdicts and 0 tallies moved**, which is structural, since `gates.py` reads no file under `references/`. No game touched. **All 8 new blocks parse under `tomllib`.** ⚠️ **Deferred, not abandoned**: `register.md`'s `How far is far enough` carries the adverb/hedge field figures (28 of 28 of our builds below the field floor) and nothing prints them in the output an author actually reads. Article density stays **withdrawn** as a seam artifact — do not re-propose it. |
| 2026-08-29 | **Save safety: the doctrine v2 lost, and an engine seam that covered three keys out of everything.** LO asked what save safety is; the honest answer needed the engine read rather than remembered, and it came back worse than the question assumed. `setup.backfillStateDefaults` migrated flags, `player.core_traits` and npcs — nothing else — so switching on any optional system in a patch release left its whole `$game_state` sub-map `undefined` in every save in the wild, with no build error and no symptom until a player opened the feature. The only remedy was an author remembering a hand-written `ndef` guard at each read site; **two exist in the entire engine, both added after something broke.** Fixed at the root: `$player` and `$game_state` are now serialized into `:: Start` and into `setup.stateDefaults` from ONE pair of dicts, so the defaults cannot fall behind a fresh start. Depth is deliberately asymmetric — `$game_state` fills one level in (every non-empty default there is engine bookkeeping), `$player` top level only (`wardrobe` is an id→garment map, and a deeper fill hands back a garment the player sold). Every save now records `origin_*` / `last_*`, with the defaults carrying `null` for `origin_*` so a pre-stamp save reads as *unknown* rather than being relabelled as having started on whichever build first migrated it. The inert `Config.saves.version` stamp stays inert **by decision, LO's**: `Config.saves.onLoad` logs the mismatch and returns, because the backfill heals what a player can hit and refusing a save costs a whole run. Doctrine: `the-returning-player.md` (v1's `save-safety.md` restored and corrected — five drifted `file:line`s, one central claim no longer true) and `engine.md` §40. ⚠️ Also found: the build declares SugarCube **2.36.1** and compiles against the **2.30.0** actually installed, which is why the hook here is `Config.saves.onLoad` and not the 2.36 `Save.onLoad.add()`. Verified: 23 new tests that execute the emitted migration **in node** against synthetic old saves rather than grepping for it; 15 of 16 red against the pre-change generator; a deliberate mutation removing the `origin_*` divergence turns 5 red; old-vs-new skeleton diff across **13 games including vesper — 0 keys removed, 0 changed**; `node --check` clean on every emitted `[script]` passage of 5 builds; 314 generator tests green; the 10 wider failures identical on the baseline. |
| 2026-08-29 | **The phone study — the gap was never the engine, it was that nobody wrote the manual.** Prompted by LO picking the phone off the open list. Method: 107 questions written and frozen BEFORE any measurement (`~/Documents/Phone_System_Study_20260829/questions.md`), then answered against 27 shipped sandboxes and 22,622 player comments. ⚠️ **Corpus repair, recorded and deliberately NOT acted on:** two games store passages in the old Twine 1 `<div tiddler=>` store-area, which every earlier extraction here skipped — adding that parser recovers **`college-daze` (9,221 passages, 3.56M words, the second-largest game in the corpus and its second-heaviest phone user) and `free-cities`**, taking the corpus from **25 to 27 of 28** parseable. Every existing `gates.py` figure was taken on the 25 and is internally consistent; restating them would move verdicts, so nothing was touched. The next study extracts with both parsers. **Headline: our engine already has more phone than any of our games use.** Eight app types shipped since doc 45; `post_actions` — the field's single most common phone porn mechanic, present in 20 of 27 games — has been authored **zero times in thirty games**, and `scheduleEffects` zero times. `under_one_roof` ships a social feed with 34 posts **she cannot post to**. `mothers_place` specced `phone ON (8_phone.toml)` and shipped with no such file. **Measured findings:** the phone is a communication and self-display device, never a utility hub (messaging 24/27, dating 7, bank 7, jobs 4, shop 2, **map 0**) — which indicts two of our own eight app types; **a message bubble is 11–16 words**, pooled over 369 real bubbles in three games and two languages, against this skill's 35–40-word beat everywhere else; the feed that works reads her **reputation** back to her; a daily thread costs **~1,300 words per NPC**; and **battery is the one mechanic players openly hate** (24 likes / 0 dislikes). **Wiring, asked as a follow-up:** exactly **1 of 27** games stores a real appointment — 26 talk about plans and keep none. The phone gates on **meters (22/27)** and **the hour (20/27)**, both of which are state the world already holds. ⚠️ **We cannot express the hour at all** — no `time_of_day` condition exists, and a conversation's trigger is a **latch** (`v2.py:2202`), so a condition means *the first moment this is true*, never *only while it is true*. ⚠️ **Two of my own numbers were withdrawn mid-study**: a first pass called four games appointment-keepers, but `degrees-of-lewdity`'s `$harper_appointments` counts sessions already had and `patriarch`/`sluttown-usa`'s are unlock booleans — the true figure is **1**; and hour-windows read 15/27 until the pattern was allowed to match dotted variables (`$time.hour`), which moved it to 20/27 and `family-ties` from zero to 23. **Applied:** `references/the-phone.md` (P1–P11) and one routing line in `SKILL.md`. **No gate, no lint, no constant, no game touched.** Two zero-failure gate candidates named at the foot of the file and left for LO. |
| 2026-08-29 | **The sex-loop study: five questions, four nulls, and the survivor was found by accident yesterday.** ⚠️ **NOTHING was written for the four.** Menu vs single-exit chain predicts nothing (**rho −0.13 / +0.09** over the 16 readable games; the two least-engaged sit at opposite extremes, 63% menus and 88% chains, the two most-engaged mid-range) — **both machines ship and no rule prescribes either**. *Loops folding back on themselves do better* **WITHDRAWN**: +0.52, still +0.34 controlling for explicit-screen count, but **exactly two games loop at all** (`corpo-life` 68%, `family-ties` 64%) and the two most-engaged loop 7% and 3% — a rank correlation carried by two points. *More explicit content does better* **WITHDRAWN**: +0.61 raw, **+0.18** holding total game size constant, i.e. bigger games collect more comments. Scene length −0.11, choice count −0.33. ⚠️ **Two extraction failures caught before any conclusion**: counting only `[[links]]` reported "0 choices" for games with plenty (`family-ties` has ZERO `[[links]]` and 6,246 `<<link>>`; `destroyer` 8,925 anchors; DoL splits 37,285/37,871), and 8 games stay unreadable because each invents its own navigation macros — figures are stated over the readable 16 and say so. **WHAT SURVIVED, and it needs no correlation**: across **38,039 explicit-screen labels**, **9.2% name an act** and **1.01% open `let`**; ours are **0.1%** and **20% inside sex loops**. Robustness: including the 8 unreadable games moves every figure **≤0.6 points**. ⚠️ **My first version of that figure was overstated** — "over half are transport" came from summing per-game top-10s; counted over all labels it is **32% transport, 9.2% acts**, and the field uses filler too. SHIPPED: `the-voice.md` **R6 — inside an explicit surface the button names what SHE does**, with four rewrites of our own shipped labels, the ⚠️ that permitting is legitimate and 1% of the field's, and the ⚠️ that the surface SHAPE is not a rule; plus both withdrawals recorded in its not-gated paragraph. **`gates.py` change is one string** — the existing `she permits or she acts` lint now cites R6. Verified: `--selfcheck` 45/45 and 27/27; **0 verdicts and 0 tallies moved across 22 games**, proved by running HEAD's gates.py beside the working copy. ⚠️ **My own first move-check was broken**: it compared against a scratchpad baseline cleared overnight, diffed against empty files, and reported 22/22 moved. **A missing baseline does not fail loudly — it reports total change. Diff against `git show HEAD:<file>`.** Scripts: `~/Documents/Sex_Loop_Study_20260829/`. |
| 2026-08-28 | **Clothing: the shape this skill told authors to copy was not buildable with the parts it named.** LO asked for the clothing system built for the same player experience as the reference game. The mechanic is EXPOSURE — `degrees-of-lewdity` tests its derived `$exposed` **961 times against 54** reads of any per-slot `.exposed`, and **71% of its 407 world gates** ask how much skin is showing, in Cliff Street, the Arcade, the Canteen and the Park rather than in sex scenes; `the-hellfire-club` asks `$top eq "none"`, `course-of-temptation` `$pc.wearing_underwear()` — three notations, one mechanic. ⚠️ **`the-meters.md` W7 had already identified `$exposed` and already said *"that is the shape to copy"* — and then named `worn_corruption` and `worn_beauty` as our equivalents, which CANNOT be**: both are `getWornStatMax` (`v2.py:1578-1579`), which skips an empty slot, so naked and plainly-dressed return the same value. SHIPPED: engine `exposure` on a garment + `setup.getWornExposure` (`v2.py:1608`, derived 0/1/2, the only aggregate that reads an empty slot) + the `worn_exposure` predicate (`v2.py:4111`, lock text `:7900`), verified in a packaged build (dressed 0 · underwear 1 · topless 2 · naked 2, with `worn_corruption` reading 0 for all four); W7's wrong equivalent corrected with the reason kept beside it, plus the half it never carried — **copy where the reads LIVE**, ordinary places, ~20 per-district reactions in the reference game, one ambient on `worn_exposure gte 1` as the starting move; `engine.md` §17's gate family gains it. ⚠️ **TWO THINGS BUILT, WRONG, AND REVERTED BEFORE COMMIT, both caught only by reading §17 while writing the doctrine.** (1) A gate `the world sees what she wears`, built on *"five of ten wardrobe games read clothing zero times"* — a figure from grepping `worn_*` only, missing `clothing_slot`, `clothing_item`, portrait overrides and location `clothing_rules`, all four listed in §17 as legitimate reads. **A gate `the wardrobe is read` already existed and counts all of them**; the true figure is **four** games, and `vesper` reads its wardrobe **30 times** and passes it while my narrower gate called it zero and failed it — **a check that fails a game for obeying the doctrine is a bug in the check**, R4's error, binned with its R9 section and its SKILL.md row. (2) The claim *"nakedness was unaskable"* — false, `clothing_slot` (empty/filled) always could; what did not exist is the derived scalar. Verified: `--selfcheck` **45/45 and 27/27, unchanged** — nothing was added to the scoreboard and `gates.py` has a zero-line diff across the arc; `mrs_vance` 44/44; five engine citations re-read off live source; 280 tests pass. ⚠️ **Not gated: how many places must read it.** The reference game carries ~20 per-district reactions and we carry zero, and any floor between those is invented. Item 9 (clothing) moves from *v2 has nothing* to **addressed**. Scripts: `~/Documents/Clothing_System_Study_20260828/`. |
| 2026-08-28 | **What does she DO — the field puts the act on the button, four of our games never let her decline, and our engine cannot tell naked from dressed.** LO's question after clothing and corruption both measured as decoration. Answered from the corpus's own words: **113,134 clickable labels across 25 games** against our 2,114 authored choices. Field verbs `leave` 6,010 · `fuck` 1,349 · `cum` 1,073 · `refuse` 964 · `wear` 504; **ours** `fuck` **1**, `cum` **0**, `wear` **0**, and our commonest opening word in every choice this project has written is **`let`** — 8.0% overall and **20% inside sex loops against 5% elsewhere**. The act is on their button and in our prose. ⚠️ **The refusal count was wrong first time**: a loose pattern read `leave`/`ignore` as refusals (navigation — *"Leave the shop"* declines nothing), putting `the_inheritance` at zero when it has three and `forty_miles` at two when it has none. Tightened, both sides: field **1,763/84,458 = 2.09%**, ours **four games at ZERO** — `forty_miles` (218 choices), `steam` (216), `seventh_day` (114), `the_allowance` (74), 622 choices and not one refusal. **The field's refusals are not theatre**, checked before building: of 4,973 sitting beside another option **79% go where the yes does not**, median destination **262 words**, only 3% a stub. SHIPPED: gate **`she can say no`** (fails only on zero, rate printed unjudged) and lint **`she permits or she acts`** (the `let` share, reported never judged), plus `the-surfaces.md` R5b's box and two rows in its checks table. ⚠️ **R5b was DELIBERATELY ungated and this does not overturn it** — its stated reason (four games read in source is *"an observation, not a field"*, and refusal QUALITY is a judgement no parser makes) still holds; only the countable EXISTENCE half is gated, on the whole corpus. The paragraph recording that decision now says so. ⚠️ **Found by crashing**: the lint first referenced a `run_gates` variable from inside `main` — a NameError on every run, hidden by a `2>/dev/null` in my own verification. **A check verified with stderr discarded is not verified.** Verified: `--selfcheck` 45/45 and 28/28; **0 pre-existing verdicts moved across 22 games**; new gate on 22/22 with reds on exactly the four named; `mrs_vance` 43/43 → **44/44**. ⚠️ **STILL OPEN — the clothing half that prompted the question.** Our engine cannot represent an empty slot: `getWornStatMax` skips unfilled slots and starts at zero, so **fully naked and plainly dressed return the same value** to every condition, and `worn_type neq` cannot separate *wearing something else* from *wearing nothing*. All three clothing-heavy field games gate on exactly that — `degrees-of-lewdity` `$exposed` (252 reads, 407 world gates, **71% about how much skin**), `the-hellfire-club` `$top eq "none"`, `course-of-temptation` `$pc.wearing_underwear()`. Meanwhile 5 of our 10 wardrobe games read clothing **zero** times and across all of them exactly **one** choice is ever gated on it. That is an ENGINE gap and is recorded, not fixed. Scripts: `~/Documents/Clothing_System_Study_20260828/`. |
| 2026-08-28 | **The register was swept in one direction with no stopping point, and the skill had 419 words of worked example to steer by.** Prompted by F95zone 312420, six of nineteen posts calling `vesper`'s prose machine-made. Padding refused earlier the same day; **broken English refused too** — we run ~half the field's malformed-word rate and a quarter of its duplicated-word rate, and burstiness, the most-cited machine-text signal, sits inside the field range. ⚠️ **The study was wrong twice and both corrections matter.** (1) **Basis**: every figure first compared the field's built HTML against our authored TOML; a build carries labels, sidebar and quest text with almost no modifiers, so the two bases do not compare. (2) **Extraction**: a line-level blockquote pass reported "4,104 words of example, every file off-field" — mostly wrapped continuation lines of my own explanation; at block level the real figure is **419 words in 13 blocks**. ⚠️ **A HEADLINE WAS WITHDRAWN BY (1)**: article density, reported as our largest and most invisible habit at 101/1k against a field max of 86 with "11 of 11 games outside", reads **65.0 against a field median of 58.3** on one basis — inside the field's 33.3–86.0, 3 of 28 builds above the maximum, modestly above the middle and nowhere near outside. **There is no article finding**, and it is recorded in `register.md` so it is not re-proposed. **What survived, matched basis, 28 of our builds against 25 of theirs:** `-ly` adverbs field 8.9–22.6 vs ours 1.3–8.6 and hedges field 5.4–22.4 vs ours 0.5–7.1, **28 of 28 builds below the floor on adverbs and 27 of 28 on hedges** — every game this project has produced under either skill; dashes 15 of 28 above the field maximum. Cause is a rule doing exactly what it says: `Sweeping backwards` tells an author to replace the hedged clause with the specific one and **names no floor**. SHIPPED: `register.md` **How far is far enough** (the test that replaces a number, a real over-stripped/weighted pair, the ⚠️ that this is not permission to pad) and **The model beats** (four worked beats, one per kind in the file's own table), plus the **fourth instance** of *an example outranks every rule beside it* — and the first where the failure is an **absence**: 419 words of example against 185,575 of instruction is 0.23%, so authors model the explanation, and with nothing anchoring them the games scatter at **1.6 to 137 dashes across builds from one skill**; plus `the-first-hour.md`'s worked cold open. ⚠️ **NO gate, NO reporting, NO constant** — a draft adding article and adverb rates under G43 was written, tested and **reverted** when the basis test showed no marker survives the seam. Verified: the new examples measured BEFORE commit — **264 words, 0 dashes, adverbs 18.9/1k, hedges 11.4/1k, both inside the field range**; `--selfcheck` 44/44 and 27/27; **0 verdicts moved across 22 games**, trivially — `gates.py` has a zero-line diff. ⚠️ **Two things left open at LO's call:** `back_home` FAILS G43 at **79.4/10k** (284 dashes), named not fixed; and **G43's calibration, since INVESTIGATED and CLEARED** — the reported 1.11x–2.43x spread was an artifact of the study script's `<[^>]{1,200}>` tag strip, which let this engine's inline-styled `<img>` tags through and counted CSS tokens as words on our side only; unbounded, across six games the move is **0.68x–1.27x**, so the dash rate does survive the seam exactly as the gate claims. `DASH_CEILING` stands, no line of `gates.py` changed. Scripts and both superseded measurements: `~/Documents/Prose_Machine_Sound_Study_20260828/`. |
| 2026-08-28 | **Padding measured and REFUSED — the fourth check this skill has turned down, and the first turned down because our games were BETTER than the field on it.** Prompted by F95zone thread 312420, where readers of a shipped v2 game called the prose machine-written and the most-argued post named the mechanism: long but *"FAILS to communicate"*, skimmed not read, caused by *"turn the one line you wrote into a couple of paragraphs with an LLM."* Nothing here measured that — G19 reads sentence LENGTH, which our games pass at the field median, and G43 reads one habit. Five markers over 25 field games and our 14, each a **rate per 1,000 words** because the G43 arc proved per-sentence figures do not survive the HTML/TOML seam: `-ly` adverbs, hedges, commas, repeated trigrams, vocabulary variety. **The hypothesis is dead and the direction is inverted** — ours p50 4.22 adverbs against a field floor of **8.87** and a median of 13.43, with **13 of 14 games BELOW the field's leanest game** and 0 of 14 above the maximum on any of the five; hedges 10 of 14 below; commas and vocabulary variety entirely inside; repeated trigrams 6 below, 0 above. `vesper` itself writes **3.01** adverbs and **4.00** hedges and repeats phrasing LESS than the field (210.71 vs p50 299.39). ⚠️ **Round one was contaminated and the correction ran AGAINST the conclusion** — `degrees-of-lewdity`'s JS block comments and `friends-of-mine`'s `[img[...]]` leaked in, both adding non-prose WORDS that dilute a per-word rate and push the FIELD figure DOWN; cleaning moved the adverb floor 6.16 → 8.87 and `love-and-vice` from 23,687 words to 9,110, being 60% script. **NO GATE BUILT**: a ceiling anywhere in the field's range passes all fourteen of our games forever, which is the same reason R4, study 6's anchoring check and P0 were refused. ⚠️ **One claim withdrawn before it was written** — the leanness was about to be blamed on `CLAUDE.md`'s filler ban leaking from chat, but that file scopes itself explicitly (*caveman has never governed a beat*); what is citable instead is `register.md:122`, *"Replacing the hedged clause with the specific one"*, and **no rule in this skill names a floor for either marker**, so the numbers are recorded and nothing is prescribed. **What it does point at**: beside G43, the one marker where we sit off-field on the HIGH side, `vesper` writes **118.3 dashes/10k** against a corpus max of 35.41 and median 0.99 — dash-joined, modifier-light, sentences at the field median. A **texture, not a volume**, already measured and already failing. SHIPPED: `register.md` "What is not measured here" only. No inventory item — nothing is broken and nothing is owed. Verified: `--selfcheck` green both directions 44/44, `mrs_vance` 43/43 with 1 n/a, no gate/lint/threshold touched so no verdict can move, no game touched. Scripts, data and the contaminated first pass kept for inspection: `~/Documents/Prose_Padding_Study_20260828/`. |
| 2026-08-28 | **A gate was deleted and seven surfaces went on teaching it — LO's audit, and the reverse direction the day-old self-check could not see.** No inventory item: nothing here is a doctrine gap, and what belongs in the record is the PROPAGATION FAILURE. LO asked for everything the recent arcs shipped to be verified one by one; the audit ran the checks instead of reading this file, and **what passed, passed running** — Study 7's gate printing three flags read 5× each, E1's two lints, Study 8's gate red on `the_season` at `has_boots (20) opens 0`, `--release` finding `the_inheritance` **published with 115 missing files and `--dev --debug`**, and §38's `template_import.py` citations exact. Five gaps survived, four of them one cause: **a check changed and the places that describe it did not.** ⚠️ **The sharp one: G35 `the anchor introduces itself` was deleted 2026-08-26** — the device is 1 of 26 field games, 18 have none, and it *"sent one author to write nine arrivals that were reverted the next day"* — the instrument and `the-first-hour.md` were both cleaned, and **seven other surfaces kept teaching it**, including BOTH templates. ⚠️ **The root was `templates/board.toml:249`**, whose location `description` placeholder asked for *"what she sees; what it smells of"* and never the FUNCTION — which is precisely the measured failure — and neither `the-board.md` nor `the-map.md` teaches it, so that placeholder was the only prescription an author saw. That is the skill's own *"a template is copied harder than a reference"* rule, fourth instance, this time against a placeholder that sounded right. Also fixed: **a row shipped the day before pointed `the wardrobe is read` at `engine.md` §23, the guidance page, instead of §17**; `the-economy.md`'s own checks table was missing **three** of its checks, one of which (`money gates content, or only prices it`) appeared nowhere in the file that owns money; **three engine citations were off by 1–3 lines in four files each** (`v2.py:216→217`, `12403→12404`, `14903→14906`) and a fourth was caught by the verification step itself (`4453→4459`); and `engine.md` §38's two ranges under-reached the join and contained no `StoryCaption` call site. SHIPPED: **`--selfcheck` gains the reverse direction** — every gate name in SKILL.md's table must still be emitted. ⚠️ **No threshold, and proved exact before it was written**: scoped to the one table whose header is `\| gate \|`, 45 documented against 44 emitted, naming exactly the stale row, zero noise either way — the unscoped version reads 8 false names off the other tables. ⚠️ **Lints stay one-directional** — wrapped prose has no exact extraction, and a check that fires wrongly is what took R4, study 6's anchoring check and P0 back out. Verified: red before green (16→0 was the last arc; **44/45→44/44** here), **0 verdicts and 0 tallies moved across 22 games**, all 20 touched citations re-read off the live source, no game touched. ⚠️ **Left undone and named:** nine gate names appear in no reference file; most are argued there under other wording, so a coverage check would fail correct work. |
| 2026-08-28 | **The index reopened twelve days after an audit closed it — maintenance, and the check that makes it the last time.** No new inventory item: nothing here is a doctrine gap, and what belongs in the record is the RECURRENCE. `SKILL.md:177` is what an author reads when a gate fails and says so in its own words; the **2026-08-16 whole-skill audit** closed it — *"nine gates were documented in zero reference files, now indexed in `SKILL.md` (23/23 findable)"* — and by 2026-08-28 the script emitted **44 gates and 27 lint headlines** against an index carrying **39 and 19**, naming **none of the three alternate modes**. Five of the eight missing lints and three of the five missing gates predate the recent arcs, so this was not one careless pass: **nothing compared the script to the file that documents it**, and a fix that only adds today's rows would have left the class alive. ⚠️ **A chat report was corrected in the same turn**: three missing lints was the number given before measuring; it was eight. SHIPPED: the index at **44/44 · 27/27 · 3/3**, each new row's doctrine home verified rather than assumed; a modes table beside *"Gates before ship"*, which until now named the only command that cannot see a build; and **`gates.py --selfcheck`**, which needs no game and exits non-zero on a gap. ⚠️ **It invents NO threshold** — a set difference over strings, names read out of the script's own source, proved exact (44 of 44, nothing extra either way) before it was written; that is why it is safe where R4, study 6's anchoring check and P0 were not. Also **`engine.md` §38** (`[project] version` / `release_date` → the sidebar footer, four `file:line`s) plus the field in `templates/board.toml`, because ⚠️ **the release doctrine written the day before required a field the whole skill mentioned NOWHERE else** — the same shape as R3's *"an instruction with no field is a wish"*, one layer up. ⚠️ **A verification step in the plan was itself wrong and is corrected in the CHANGELOG, not dropped:** `templates/board.toml` has never parsed as TOML and cannot — its `<tier_1> = 0` placeholders are invalid by design — so the real check is that the failure is the same statement, shifted by the inserted lines. Verified: `--selfcheck` **red before green** (16 names → 0), **0 verdicts and 0 tallies moved across all 22 scorable games**, no game touched. |
| 2026-08-28 | **The release boundary — the first instrument in this skill that reads the ARTEFACT instead of the source.** `mrs_vance/REVIEW.md` B2. LO's rule (*dev mode and missing media block RELEASE, not testing*) was correct and held by **nothing**: it lived as a comment on a JS object literal (`games-data.js:44-49`), **hand-copied into nine of twenty-eight portal entries in three wordings**, while `gates.py` had **zero lines reading a built game** and `the-release.md` — 164 lines, named for this — never said `--dev`, `--debug` or *build*. Every one of the 43 gates measures `7_final_game.toml`, which cannot see an artefact. **The drift was already shipped:** parsing the flags-init map and `MissingMediaPage` out of all 29 builds, **`the_inheritance` is in the published grid as a full `--dev --debug` build with 115 missing files**, `the_long_summer` carries `--debug` with 122, `under_one_roof` ships **183 missing** silently, and `forty_miles` holds `dev: true` **and** `version: "0.1"` on one entry while its TOML prints `0.1.2` to the player and three archives exist. ⚠️ **B2's own fix was corrected rather than adopted** — the `[IMAGE MISSING]` / `[… POOL MISSING]` markers it specified are emitted **only under `--debug`** (`v2.py:12549`, `:14753`, `:14906`), so a clean build renders silent gaps and the grep passes it: the proposed instrument detects scaffolding, not missing media, and would have passed the 183. Read instead: the build's own flags-init map (`v2.py:1077`, `:1081-1082`) and `MissingMediaPage`, *"always generated, but button only shows in debug mode"* (`v2.py:217`). SHIPPED: `the-release.md` **§ Shipping the build** — six steps **lifted from the JS comment, not invented**, the three-places-that-say-what-shipped table, and the relationship the schema never declared (**`dev: true` and `version` are mutually exclusive**) — plus **`gates.py --release <slug>`**, six checks, off for every ordinary run, **exiting non-zero** where `words_mode` deliberately never does. ⚠️ **Two things NOT gated, each stopped by a measurement:** byte-equality against `releases/v<n>.html` (vesper's differ, and vesper is the only whole triangle — the fourth time this skill has printed instead of judged, after R4, study 6's anchoring check and P0), and any repo-wide sweep (nine legacy entries carry no `version`; failing all nine on day one is noise). ⚠️ **The media count is a build-time snapshot**, so a red is fixed by a REBUILD, never a file copy. **Baseline recorded as the number that has to come down: 0 of 29 builds clean**, best 5/6. Verified with every branch driven — the reds on real shipped work, the fixture-only branches (`dev`+`version` together, no build, no portal entry, no `games-data.js` → **n/a, not a pass**) on a fixture root — and **0 verdicts and 0 tallies moved across all 22 scorable games**; six games' two nondeterministic headline lines proved inherent by running the identical file twice. No game rebuilt or modified. |
