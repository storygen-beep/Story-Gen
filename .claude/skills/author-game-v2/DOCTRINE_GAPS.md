...
`v2.py:13319` falls back to the choice text (`locked_text or choice_text`), so the other seven render
...
| card selection: `pickQuestsCards(scope)` for the top tier, `pickQuestsCard(slug)` returns the single highest-`priority` match per character | `v2.py:15442`, `:14065` |
