...
| **3 · A counted daily cap** | `max_triggers_per_day` on a *triggered* canvas, or a `_today` flag cleared in `[engine.daily_tick]` (§28) | **`max_triggers_per_day` is read off the trigger (`v2.py:11583`) — a triggerless rung has none.** And a single removable flag is brittle as the *only* brake: v1 records a whole seduction climb collapsing on first play the moment its one daily-cap flag was deleted |
