...
- **The loud pass (rev 237, 2026-09-25)** — the opening now follows the v2 staged shape (`the-first-hour.md` F1b). Each flavor choice gets Mercer's reaction before the shared punishment. Wren's feelings are made obvious and she **misnames them in her own thought** ("Fatigue. It's only fatigue."; LO's option B, the same move as *Crying = malfunction* above). The hinge ends with one beat of plain game-voice instructions (the Anchor's hours, the coveralls, the depot's Coin, Charge and the cradle, the Quests page) and then the hook. The Mission 1 card carries three goal steps: hired, back office, drained.
...
### The Face blueprint (Act 2 · 1c — Step 5, rev 144)

> Full record: `games/vesper/design_the_face.md`. Locked with LO 2026-08-16. The chapter after The Leash,
> built from the shipped 0.1.8 end-state (un-leashed, chip out and left on Kess's bench, holding her own
> page, and with **nothing to do** — `cap_extraction` sets two flags and opens no standing content). It
> **supersedes** the shelf file's own "TO RESTORE AT 0.1.9" header, which describes a paste plus three
> reversals; that instruction assumed a leash that no longer exists. **~22 canvases plus three systems.**

**The verb — she takes a job and earns her way through a door.** Distinct from every verb on the roster:
...
she is **staff** — a new hire with no leverage, working up a bar floor, and the promotion **is** the
seduction because it is his call at every rung. Underneath it a second verb: **she waits**, because after
the drain the only thing that brings Cain into reach is being on shift when he walks in.

**The double blindness that carries the chapter.** He does not know she is his old property, because she
is wearing a face she bought. She does not know why the district shut against her, because the reason is
Cain protecting her and nobody will say so. Every person is being kind to her in a way that looks exactly
like rejection.

**Bastien owns the Undertow, and the game left the hole for it three times** — `1_metadata:982` (*"it
plainly belongs to someone; nobody says who"*), `5_scenes.toml:9580` (*"the one it answers to now never
shows his face"*) and `:9618` (Sol, aloud, on an owner who never comes down). Sol knows Bastien as a man
who uses the back room and has no idea he holds the deed, so both shipped lines stay literally true.

**The scenes.**

| # | Scene | Lane | Gate | Place |
|---|---|---|---|---|
| 1 | **cap_chip_read** | L4 auto-fire capstone | `leash_cut is_true` + `chip_read is_false` + `npc_at_location kess_berth / npc_kess / is_present` → **SETS `chip_read`** | `kess_berth` |
| 1b | **hub_kess_berth — RE-BAND** *(not a new canvas)* | L1 hub | ⚠️ **A repair, not an addition.** Every band on the shipped canvas crosses `feed_line_days` × `part_held` × `mercer_attempts` and **none reads `leash_cut`**, so after the chip comes out its always-available rung answers about a job that finished. A new `leash_cut is_true` band retires the parts talk. ⚠️ Adjacent `[group]`s merge into one if/elseif chain — the new band goes **first** or carries an exclusive clause | `kess_berth` |
| 1c | **quest card N** | Story-Goal | `chip_read is_true` + `face_bought is_false`. ⚠️ **Card M's boundary sentence and its `terminal_text` come off in the SAME edit** — the two-step rule, and M is currently the one card in the game that names the build boundary | Quests page |
| 2 | **wall_sol · wall_colm · wall_rue · wall_market** | L1 one-shot rungs | `chip_read is_true` + own flag `is_false`; each bumps **`wall_refusals`**. Nobody explains, and the prose must not let the reason leak | their own locations |
| 3 | **cap_the_wall** | L4 auto-fire | `wall_refusals gte 3` + `wall_understood is_false` → **SETS `wall_understood`**. She concludes she has to stop being Wren. ⚠️ She does **not** learn that Cain did this — that is reserved | `underworld_strip` |
| 4 | **activity_buy_face** | L1 solo card | `wall_understood is_true` + `face_bought is_false` → coin → **SETS `face_bought`**, grants the cover **un-worn**. The market already sells *"weapons, gear, and people, penned at the far end like the rest of the stock"* | `underworld_market` |
| 5 | **activity_berth_wardrobe** *(SYSTEM)* | solo host | `berth_home is_true`. ⚠️ **A blocker, not a nicety** — the only wardrobe rack is the engine-injected one at `wren_room`, inside the Spire the 1a close sealed, so she currently **cannot change clothes at all**. ⚠️ verify at build whether a canvas can carry this or whether the rack must be re-homed | `kess_berth` |
| 6 | **cap_stranger_at_the_bar** | L4 auto-fire | `face_bought is_true` + cover equipped + `stranger_seen is_false` → **SETS `stranger_seen`**. Sol does not know her, and his shipped *"decided he likes her — mostly"* is what it costs. Play it once; do not underline it | `underworld_bar` |
| 7 | **cap_sol_hires** | L4 auto-fire | `stranger_seen is_true` + `bar_hired is_false` + `npc_at_location underworld_bar / npc_sol / is_present` → **SETS `bar_hired`, `bar_rung = 1`**. Sol is promoted from set-dressing to the man who hires her — he has a 14-beat hub and no arc, and this is the first thing the game has ever asked him to do | `underworld_bar` |
| 8 | **activity_bar_work** | L1 repeatable work card | `bar_hired is_true` + cover equipped; **four exclusive bands on `bar_rung`** (1 floor / 2 counter / 3 in less / 4 the room). **Pays under The House** — she takes a worse job because it puts her ten feet from a door, said once in her own voice and never again. Shape copied from `underworld_brothel_work` | `underworld_bar` |
| 9 | **cap_bastien_notices** | L4 auto-fire | `bar_rung eq 1` + `npc_bastien.relation gte N` → **`bar_rung = 2`**. He notices from a chair aimed at that door, four hours a night — which is not a coincidence, it is what his schedule row says he does | `underworld_bar` |
| 10 | **rung_bar_promotion_3** | L1 rung, auto-fire | `bar_rung eq 2` + relation band → **`bar_rung = 3`**. Public, in less, in front of Sol and Colm and a room of men. ⚠️ **First spend of ceiling row (b)**, which is unsigned | `underworld_bar` |
| 11 | **rung_bar_promotion_4** | L1 rung, auto-fire | `bar_rung eq 3` + relation band → **`bar_rung = 4`** — serving the back room directly | `underworld_bar` |
| 12 | **bastien_door_search** *(RESTORED)* | L1 flat solo link | `bar_rung gte 4` + `npc_at_location bastien_backroom / npc_bastien / is_present` (his 20:00–23:59 row is the clock). **Structure and prose survive intact**; the three routes re-band from `controller_seated` × `stealth` to **`taser_held` × `stealth`**, which is stronger — now the search has something real to find. Its exit is the **only** route into the back room | `underworld_bar` |
| 13 | **hub_bastien** *(RESTORED, re-banded)* | L1 hub (portrait) | `bar_rung gte 4`. Bands were `controller_through` + `leash_cut is_false`; re-band on the promotion rung and whether she has drained him | `bastien_backroom` |
| 14 | **loop_bastien_backroom + loop_bastien_finisher** *(rig KEPT, prose REWRITTEN)* | L1 rung → pose menu → finisher, triggerless | Reached from the hub. ⚠️ **THE HEAT PASS, and the release's largest single beat.** Measured: every repeatable sex loop in the shipped game scores **0 or 1** hot beats (Colm 0/10, Renner 0/8, Calloway 0/8, `mercer_serve` 0/7; best in game `loop_mercer_lockup` 2/15), against 8/28 in the one-shot `cap_owner_print`. Whole-game `explicit in repeatable` reads **20.5%** against a 50% floor. Bastien was shelved at rev 141 for exactly this and cannot ship below it | `bastien_backroom` |
| 15 | **bastien_drain_canvas** *(payload REWRITTEN)* | triggerless drain payload | Reached from the finisher's ass-finish choices. **The anonymous buyer / three hands / "same man" content is dead canon.** New payload: the paper trade is his cover, **Cain is why**, Cain is an early weak model rebuilding himself out of stolen Vance tech, nobody knows where he lives — **but he comes here whenever he likes** (the line that keeps the chapter alive after the drain), he has known for a year who took her, and he still does not know why Cain wanted that one. ⚠️ **Cain is not named here**; her one interior beat still cannot hold the four words, which keeps `captivity_cain`'s closing line true | `bastien_backroom` |
| 16 | **activity_buy_taser** | L1 solo card | `bastien_drained is_true` + `taser_held lt 1` → coin → **`taser_held = 1`**. ⚠️ **An item, not a third `equipped_weapon` value** — the whole point is that it runs off nothing and so does not compete with the drain on the one-core-one-weapon axis. She cannot bring the emitter: she needs the drain live for Bastien, and company hardware does not survive a man who checks the seam under the arm | `underworld_market` |
| 17 | **cap_the_raid** | L4 auto-fire capstone (Tier-3) | `bastien_drained is_true` + `taser_held gte 1` + N days since the drain + `raid_done is_false` → **SETS `raid_done`**. The attack lands **while she is on shift**. She overhears Cain and Bastien — nothing expository, the shape of two men who know each other mid-crisis. She tasers Bastien to keep Cain in the room. He sees the mole on her neck. They run, and the units say the thing that tells her the company knows she is alive. ⚠️ **Cain fails at everything he came for** — he does not save Bastien, the place, or the documents | `underworld_bar` |
| 18 | **cap_the_lab** | L4 auto-fire capstone (Tier-3) | Chained off the raid's exit → **SETS `lab_seen`, `cain_named`**. She still has the taser on him, which is why the line works: *"Now put down the weapon, Wren."* **The release ends here.** She walks out freely and the next release resumes in the same scene | `cain_lab` *(new)* |
| 18b | **cain_lab** *(SYSTEM — location)* | location | Entered by canvas, left by canvas. ⚠️ Needs **both** no `entry_from` **and** `auto_exit = false`, per the `bastien_backroom` precedent — dropping only `entry_from` leaves an empty nav grid and trips the list-every-location fallback | new |
| 19 | **bastien_backroom → the burned room** | location re-skin + **nav card** | `raid_done is_true`. The room gains an `entry_from = underworld_bar` card it never had. ⚠️ verify at build that a location shipped with `auto_exit = false` takes a nav card cleanly | `underworld_bar` → `bastien_backroom` |
| 20 | **activity_sift_the_ruin** | L1 repeatable crawl | `raid_done is_true`. The units took the useful documents; what is left is the junk paper Bastien sold for cash — and one or two pieces carry names that match the index on her own page. **This is the only thing left to do after the credits**, and it exists because 0.1.8's ending opened nothing and this one must not repeat that | `bastien_backroom` |
| 21 | **quest ladder N → the boundary card** | Story-Goal + NPC tiers | The spine. ⚠️ **Every capstone that closes the last live card ships its replacement in the same beat** or the page goes blank. **Exactly one card names the build boundary**; moving it is a two-step edit and the check is a grep of the whole table from both sides | Quests page |
| — | **CUT: `cap_bastien_walks_in`** | — | Its whole premise is that he finds her and calls her by the file Mercer keeps her under. He is now avoiding her on Cain's word and she arrives in a face he has never seen. The prose cannot be salvaged; scene 9 replaces it | — |
| — | **DELETE: `kess_seat_controller`** | — | Orphan. It existed only to beat the door search, and the controller thread closed at `leash_cut` | — |

**Systems this chapter owes:** the wardrobe at the cot (scene 5 — a blocker) · the bought face as a
`[[clothing]]` cover with its own `type` and matching `[[player_portrait.outfits]]` rule (the build warns
if a type has no rule; gate content on the **item id**, not the type) · the taser as an item · the bar
work card · `cain_lab` · the burned-room nav card. **And one marked for separate brainstorm at LO's
request:** retiring `company_grays` as her default outfit, which is `initial = true` and the portrait's
`default_image`, and therefore touches shipped content.

**Ceilings owed before any hot beat (both unsigned):** (a) **Bastien outside the cell, on a woman he does
not own** — the signed rev-62/64 row was written for a captive; full crude stands, but the degradation
turns *proprietary rather than jeering*, because she is staff and he is establishing that staff is a
category he owns. (b) **The bar floor** — a register the game has never written: public, semi-sexual,
repeatable, exposure and handling rather than sex, un-crude at rungs 1–2 and crude only where a hand
lands. Scenes 10, 11 and 14 are blocked on these.

**Two secrets held past this release, and neither may leak:** **Bastien is alive** — taken, not killed,
for what he knows about Cain's supply chain. The game must **never state that he is dead**; she assumes
it, everyone assumes it, and exactly one quiet detail is left (no body is found). And **Cain already
holds the useful documents.**

**Plan seed:** `beat_0094` (systems + geography) · `beat_0095` (Kess opens + the `hub_kess_berth` repair) ·
`beat_0096` (the wall) · `beat_0097` (the face) · `beat_0098` (hired + bar work) · `beat_0099` (noticed) ·
`beat_0100` (the climb) · `beat_0101` (the door + his hub) · `beat_0102` (the loop — the heat pass) ·
`beat_0103` (the drain) · `beat_0104` (the wait + the taser) · `beat_0105` (the night) · `beat_0106` (the
lab) · `beat_0107` (after — the ruin + the Quests page) · `beat_0108` (media pass) · `beat_0109` (clean
ship + deploy). All `status=planned`.
