...
> Book revision 130 (Step 5 + authoring. **Header changelog resumed at 52.** *(Revision-number note: this leading number sat stale at 69 from rev 69 through rev 111 while only the inline `Latest:` line moved; from rev 112 onward it tracks the true revision. The unclosed code span that had swallowed this note's closing parenthesis — and with it the `Latest:` marker — was repaired at rev 124.)* Latest: **THE LEASH — THE PARTS LOOP WAS UNREADABLE, AND THE SCHEDULES PAGE WAS LYING (rev 130, 2026-08-11).** LO playtested the chapter and stopped dead — *"I have bought the part from blackmarket but now things are stuck, the mercer menu doesnt show the choice and the kess also does nothing."* Reproduced live against the built game. ✅ **There is no logic bug and no soft-lock** — every gate does exactly what its TOML says, and the lapse is always recoverable, because the cot is a Wren-solo activity with no presence gate. ⚠️ **What was broken is that none of it was visible.** Three gates stacked: the install rung is hidden on a lapsed feed line; the spend rung is hidden when the part is bought but not yet seated; and Kess (10:00–22:00) and Mercer's Lockup (23:00–08:00) have **zero overlapping hours in the whole day**, measured across all 24 — so one cycle is necessarily two trips and nothing said so. The author of the chapter could not find his own next click; a player has no chance. **The fix is prose, not menus.** Kess's hub goes from two upkeep bands to **four**, crossing the feed line with whether she is carrying a part — the new lapsed-and-carrying band is the only place in the game that connects the part in her pocket to the bill she hasn't paid, and it names the cot. Mercer's room splits its "arrangement resumed" band so a player standing in the loop with nothing seated is told why the evening buys her nothing. Both off-hours cards now state the hours as hours, and card I's tip says the cycle takes two trips. ⚠️ **`show_when_locked` could not have done this** — it greys on a choice's *whole* conditions block, so it is all-or-nothing and would have advertised the loop from the moment the canvas existed. The original hide-don't-grey call was right; what was missing was the prose that owes the player a reason. ⚠️ **AND THE SWEEP AT rev 129 WAS A HALF-SWEEP.** `beat_0084` read the Quests page end to end and declared it clean. The **Schedules page** — the only screen that publishes NPC hours, and therefore the one players use to decide where to walk — went on advertising **Mercer as "NOW: Mercer's Penthouse" for fifteen hours of every twenty-four**, with his real address filed as the inactive row, plus Calloway and Vane standing in Vance Securities; all three sealed by `archive_1a_done` an act earlier. **A page that names a locked door is worse than a page that says nothing, because the player walks there.** The schedule rows are correct and cannot be gated — the resolver reads five keys and drops the rest — so the fix is on the page: it now checks each row for reachability, mutes the unreachable ones with their in-world reason, and suppresses the NOW badge entirely for a locked location. **This is an engine change and it lands for every game.** Found on the way: the page's `class="<<print _rowClass>>"` was never evaluated — SugarCube emits macros inside a raw HTML attribute verbatim — so the current-slot highlight had been dead since it shipped. Prior: **THE LEASH — THE QUESTS PAGE, READ END TO END (rev 129, 2026-08-11 — beat_0084).** The first whole-surface read of the Quests page for this chapter, and it found the spine clean and almost everything around it not. ✅ **The Story-Goals spine holds** — exactly one live card at all eleven states from the 1a close to `leash_cut`, D through M in order, and **M alone after the cut**. ⚠️ **THREE CARDS WERE STILL SAYING THE BUILD ENDS AT THEM.** Cards E, F and G each carried *“that’s where this build ends … is the next release”*, written true and left behind when the frontier walked five rungs past. E was the worst: it named **cutting the leash, opening the file and reading her own page** as the next release, in the build that ships all three. Why it survived is the transferable part — the TOML comments above F and G record the check being **run and passed**, against the card’s own rung, and from that rung the sentence is true. **A boundary claim is a statement about the whole build, so it cannot be judged from the card that carries it.** Exactly one card in the game names it now, and **card H got back the Support-Us ask** it lost at `beat_0075`. ⚠️ **AND THE NPC TIER WAS AN ACT-1a ARTEFACT, EXACTLY INVERTED.** The 1a close hard-seals the Spire, and Calloway’s whole four-rung ladder plus half of Colm’s end card went on pointing at it for the rest of the game — while **Renner, who stays reachable, had his section VANISH** on his own drain, because all nine of his cards gate on `renner_drained is_false`. **Nine new NPC cards**: warm-tap end cards for Renner, Calloway and Colm, and **the chapter’s own tier, which did not exist** — three cards for Mercer and three for Bastien, the two conquests the chapter runs on. Mercer’s loop card carries **the one goal bullet in the chapter** (`mercer_attempts — 2 / 3`), because the three failures before the fire were counted on a hidden trait and the player had no way to see how many were left. **Bastien’s third card is the terminal-flag sweep applied to the Quests page.** Kess gets **no** section, deliberately — upkeep belongs on the HUD. ⚠️ **ALSO CLOSED: the game’s only zero-card window.** Between the ledger find and the grab no Story-Goal matched at all, so the page read *No active quests* during the biggest scene in the act; card 16’s opener widened one clause, and the shipped cohort is unaffected. Verified: build green, **85 / 85** live against the real renderer at every state. Prior: **THE LEASH — THE EXTRACTION: THE CHIP OUT, AND HER OWN PAGE (rev 128, 2026-08-11 — beat_0083).** **Blueprint scene 20, and the chapter's LAST CONTENT BEAT** — only the Quests pass, the media harvest and the clean ship follow. **Built: `cap_extraction`** (auto-fire Tier-3 capstone @`kess_berth`, 18 beats / 17 clicks, 762 w, **1.30 : 1**), **quest card M**, and **the retirement pass**. Every piece of it was promised in the chapter's FIRST beat and is shipped canon: `kess_1b_open`'s *"the file rides on the same crack, so it opens when the key does"*, `cap_file_shape`'s *"I open that, you get your page"*, and `salvage_chip_snag`'s refusal to pull it blind. ⚠️ **CAIN DOES NOT LAND HERE, AND THAT REVERSES WHAT rev 127 SAID.** `beat_0082` recorded that the name would be handed to scene 20; §15 says his entrance is a whole chapter, and spending the name on the last page of a release that then goes quiet buys a gasp and costs the door. **The page gives her what §12 actually asks for — what she is:** the programme's real size, her designation **four years older than she is**, the word **BUILD** she could never read past, and the two date columns — the second filled most of the way down, **and hers is blank.** Plus a hook that names nobody: the programme **closed**, and her page was sealed eight months later by a hand on none of the sign-offs. ⚠️ **AND A LIVE DEAD-END THIS BEAT EXISTED TO KILL.** §13 retires the switch and the block at the cut — which is not flavour: `loop_bastien_finisher`'s drain exits required `controller_state gte 1`, so from the extraction onward **every anal finish would have routed to the nothing-happens branch forever** and the player would own the man and get nothing, with no way back and nothing on screen to show it. Five surfaces read the retired state and all five are handled; the finisher went **four exits to six**, provably partitioned on `leash_cut`. ⚠️ **THE SKILL FOLD IS THE OTHER HALF OF A RULE IT ALREADY HAD:** `lanes.md`'s terminal-flag sweep is thorough about surfaces that become a **lie**, and silent about gates the flag makes permanently **FALSE** — which is worse, because nothing looks wrong. It now carries the one-grep audit, the three legitimate answers, the reason `logic = "OR"` cannot express the fix, and the assertion shape that catches it. ⚠️ **AND THE PAGE ITSELF WAS RE-CUT AT MEASURE TIME**, which is the `beat_0082` fold biting its author: the first draft carried the reveal as **182 words of unbroken narration** and measured **2.31 : 1**. She reads it aloud now, Kess talks her through the cut, and it lands at **1.30 : 1**. Playwright **52/52**, plus 58/58, 68/68, 50/50, 69/69, 50/50, 42/42, 58/58, 66/66, 46/46, 51/51, 34/34 — two suites CRASHED rather than failed, because assertions indexed `i["trait_key"]` blind and a `leash_cut` flag clause joined those blocks; both now read conditions key-agnostically. Whole-game ratio 2.55 → **2.52 : 1**. Media debt 21 → **22**. Nothing committed, nothing released. Prior: Latest: **THE LEASH — THE BACK ROOM, THE DRAIN, AND WHAT HE WAS BUYING HER FOR (rev 127, 2026-08-11 — beat_0082).** Blueprint scene 19, the chapter's second drain, and its last new content before the extraction. **Built: `loop_bastien_backroom` + `loop_bastien_finisher` + `bastien_drain_canvas`** (the shipped `loop_colm_*` three-canvas shape) plus **quest card L** and §13's controller **switch**. ⚠️ **TWO CONTRADICTIONS IN THE LOCKED DESIGN WERE SETTLED HERE, both put to LO before building.** **(a) §11 said the payload names CAIN; §15 says he is not on the page. §15 WINS, and the beat is bigger for it:** Bastien does not know the name either — the money came through three hands, the man at his far door never turned round — and what the drain takes is the thing he has never said aloud and **works out while she has him open: they were the same man.** The buyer wanted *what was done to her*, not what she can do, and asked one question twice: *was she made, or was she taken.* That is §11's content in full with only the **label** withheld; it keeps Cain's entrance whole, pays off Bastien's own year-long question **inside** the drain, and hands the name to scene 20. **And she does not supply it either** — `captivity_cain` says the four words are a thing she *cannot hold*, so her one `thought_bubble` finds *a shape where a word should be* rather than retconning that line. **(b) Blueprint row 19 gated the loop on an `npc_bastien.relation` band; §11 says a seduction ladder BOUNCES OFF HIM — *'he'd enjoy it and give nothing.'* The band is deleted:** the door was the gate. ⚠️ **§13's LOADOUT: the switch shipped, THE CHARGE ECONOMY WAS CUT** and the cut is recorded rather than dropped — it is an economy with exactly one customer, and it would buy one more errand in a cycle §8 says must not read as errands. What ships is a **two-condition contract that teaches itself**: the shipped loop already teaches *only the anal finish opens the drain*, and Bastien adds *and only with the key running* — anal with the key asleep routes to the cold-socket nothing, **no bump, no fail screen, just a wasted evening.** ⚠️ **AND A MEASURED SKILL FOLD:** the `beat_0079` carriage rule got its first inherited use, and the payload turned out to be the *smaller* half of the problem — this game's three shipped sex loops measure **8.8 : 1**, **102 : 1** and **132 : 1**, with **zero** dialogue blocks in the finishers and drain payloads. Authored with his voice on, the new chain measures **1.08 / 1.28 / 1.22**, and the fix cost **eight lines**; `beat-authoring.md` now carries the numbers plus the procedural half that would have caught it earlier — run check 3 on **each canvas of a chain separately**, because an average hides a 100 : 1 finisher. Playwright **57/57** first run, plus 67/67, 50/50, 69/69, 50/50, 42/42, 58/58, 66/66, 46/46, 51/51, 34/34. Whole-game ratio 2.60 → **2.55 : 1**. Media debt 14 → **21** (seven pooled slots, the same seven the Colm loop carries; the drain canvas takes none because the payload is a conversation). Nothing committed, nothing released. Prior: Latest: **THE LEASH — THE BACK ROOM, AND THE SEARCH AT THE DOOR (rev 126, 2026-08-10 — beat_0081).** ⚠️ **THE LAST UNSIGNED CEILING ROW IS SIGNED** (LO's call at this beat): *Bastien, outside the cell*, **NOT widened**, with **two clauses added at signature** — **(i) he never learns what she is** (he may be closer than anyone in the game has been and still end the chapter not knowing; if he works it out there is no Bastien left for the next chapter) and **(ii) she is never shown enjoying it** (the back room is work, and the charge is what she is carrying). **Built: `bastien_door_search`** (the gate, played every visit, **0.85 : 1**), **`hub_bastien`** (his portrait hub, 0.75 : 1), **`kess_seat_controller`** (route A's supply side, 1.11 : 1) and **`bastien_backroom`**, his first schedule row in the game. §11's arc is **smuggling, not seduction** — she needs the drain *and* the controller in that room, because on a name that is on the set the drain alone does nothing. ⚠️ **THERE IS NO FAIL BRANCH, and that is a design call.** He is *establishing that he still may*, so this is not a challenge she can lose — a fail state would make it a lock-picking minigame and turn the beat into pain-as-spectacle, which the rev-122 row already named as the failure mode. **She always gets in; what varies is whether what she needs got in with her**, banded three ways: seated in the seam by Kess · in her coat behind `stealth gte 30` (§11's own line — stealth finally gets a second customer in the whole game) · or found, and it sits on his desk while the evening is wasted. ⚠️ **THE ROOM IS NAV-INVISIBLE, AND THAT ONE PROPERTY DOES TWO JOBS**: no `entry_from` means no nav card, so his schedule can never leak a badge (`v2.py:4773`) **and** the search cannot be skipped, because the only way in is its exit. That is strictly better than the locked card `beat_0080` had recorded — a locked card advertises the room before she is invited and, once open, lets her walk past the scene. ⚠️ **AND IT COST A LESSON, CAUGHT LIVE BEFORE ANY CONTENT ASSERTION RAN:** dropping `entry_from` is only half the job. A childless, parentless location renders an empty nav grid, and an empty grid trips the **list-every-location fallback** (`v2.py:19386`) — the built passage came out carrying `All locations:` and the whole world, so from his back room she could walk to the Spire. **`auto_exit = false`** is the other half, and `underworld_gate`'s header in this game's own metadata file spells the trap out. The skill documented the field correctly all along; what it lacked was the **shape** named where map shapes get chosen, so `location-design.md` gained a §4.2 for it. Playwright **67/67**, plus 49/49, 69/69, 50/50, 42/42, 58/58, 66/66, 46/46, 51/51, 34/34 — four suites retargeted, three of them because a `part_gen` assertion was matching a TOML **comment** rather than a declaration. Whole-game ratio 2.66 → **2.60 : 1**. Media debt 13 → **14**. Nothing committed, nothing released. Prior: Latest: **THE LEASH — BASTIEN WALKS BACK INTO PLAY (rev 125, 2026-08-10 — beat_0080).** Blueprint scene 17, and the chapter turns from her owner to the man who bought her file. **Built: `cap_bastien_walks_in`** (auto-fire Tier-3 capstone @`underworld_bar`, 15 beats / 14 clicks, **0.79 : 1** — the new best canvas in the game) plus quest **card K**. ⚠️ **A CORRECTION FIRST: `beat_0079` recorded this beat as BLOCKED on the unsigned Bastien ceiling, and that was one scene too early.** The drafted row governs exactly two things — **the search at the door** and **the back room** — and blueprint row 18 says in its own words that the door-search is `hub_bastien`'s gate. Row 17 is the reveal, and it reaches neither: no nudity, no touch, no sexual act, and **the search is promised in his own mouth rather than played**. That is the shipped `rung_mercer_hands_on` precedent one notch earlier (*"it stops at the touch: nothing explicit, so the unsigned Mercer ceiling stays unspent"*). **The row still comes due at `beat_0081`, which cannot ship without it.** ⚠️ **AND AN ENGINE TRAP IN THE LEDGER'S OWN PLAN ENTRY WAS REFUSED RATHER THAN BUILT.** That entry listed `npc_bastien schedule @underworld_bar` under `introduces`. It is not needed — `cap_mercer_resurfaces` is the shipped **scheduleless-speaker** precedent at this exact bar (no `requires_npc`, no `npc`, no presence predicate) — and it is actively harmful, because a schedule row at a **public** location parks the NPC's badge on the nav card (`getNpcsPresentAtLocation`, `v2.py:4773`, **not** canvas-gated) and `underworld_bar` has been reachable since Act 1 and is unlocked. It would have shown Bastien's face to a player mid-Act-1 who is currently being kidnapped by him. Scene 6c hit this exact wall with Mercer and solved it with a **locked room**; Bastien gets the same shape at `beat_0081`, and **the door-search becomes that location's entry gate**, which is what §11 wanted anyway. The live suite now asserts **no Bastien badge anywhere on the nav grid**, so the next beat inherits a guard rather than a note. **THE SCENE:** he did not release her at the 1a close — **he lost her**, to a man who came through his far door and argued him to a stop, and he has spent the time since not finding out who. So he is looking at a thing that was taken off him, walking and repaired, and his lever is **curiosity, not desire** — *"the cell was never punishment; it was a room with a notebook in it, and she is the only page he never finished."* Cain stays reserved (§15): never named, never guessed at. **The blueprint's off-by-one was fixed at birth rather than patched** — row 17's `mercer_attempts gte 3` gains **`part_installed gte 1`** (*she has been back to Kess since try 3*, which is where band 3 names him), and it works only because `beat_0079` does not burn the fourth part. **Card K was mandatory for the fifth time in this chapter**, and **card J kept its Support-Us ask** — only the build-boundary claim moved, which is the rev-113 precedent re-learned the hard way one beat ago. One §7 split was forced at **51 w**. Playwright **49/49** first run, plus 69/69, 50/50, 42/42, 58/58, 66/66, 46/46, 51/51, 34/34 — and `live_beat_0079.py`'s frontier assertion was split into its two halves so only the boundary half moves next time. Whole-game ratio 2.72 → **2.66 : 1**. Media debt 12 → **13**. Nothing committed, nothing released. Prior: Latest: **THE LEASH — THE FIRST FIRE (rev 124, 2026-08-10 — beat_0079).** The chapter's payoff, and the one **explicit** spend of the Mercer ceiling — its budget line says so in as many words, so the row signed at rev 119 covers this beat and no new signature was owed. **Built: `cap_first_fire`** (auto-fire Tier-3 capstone @`mercer_room`, 18 beats / 17 clicks, **0.93 : 1**) and **`loop_mercer_warm_tap`** (blueprint scene 16, triggerless, 4 beats / 3 clicks, no media), plus quest **card J** and the new counter `mercer_drains_done`. She buys a fourth part, Kess seats it, and this time the thing at her seam answers the challenge **in Mercer's own voice** — the door stays open and her owner comes apart under her hands. **THE CONTROL-CANVAS CARRIAGE RULE IS EXECUTED FOR THE FIRST TIME IN THE GAME.** The rule (see *The control-canvas (the drain) — voice carriage* below) says the extraction is played as a **Q&A in HIS own dialog, not narrated summary** — and measured at this beat, **all three shipped drain payloads break it**: `loop_renner_finisher.drain`, `calloway_drain_canvas.d0` and `colm_drain_canvas.d0` contain **zero** player dialog blocks between them, and every one narrates the take. This canvas plays it: 6 of her questions against 13 of his answers. **The payload:** Halloran, Asset Services, a signature on a transfer and a man Mercer never met, told she was *sensitive recovery, one of theirs, on loan* — four lines; Mercer himself told less than that; **he never had the faintest idea what the key was for** and carried it two years as *a perk, not a job*; and **the bad news** — *mine only speaks for me… like there were others out there with one.* He is the bottom of it and was never anything else. **The part is NOT burned** — §9's own heading is *try 4 holds* — which closes the parts economy (the buy card and the install rung both go dark) and is **load-bearing forward**, because `cap_bastien_walks_in`'s `beat_0080` fix reads `part_installed gte 1`. It exits to `underworld_market` rather than back into his room: an auto-fire that exits into its own location chains, and §10's own closing image agrees. **Card J was mandatory for the fourth time in this chapter** (`owner_drained` closes I, the last Story-Goal card in the game) and **card I's tip was re-cut** — its build boundary named the first fire as *the next release*, which this beat ships. ⚠️ **The first cut of that edit stripped card I's Support-Us ask along with the false sentence, and the `beat_0075` suite caught it**: per the rev-113 precedent set on card D, a prior card keeps its copy of the ask and only the boundary CLAIM can go false. Fixed in the same turn. (Card **H** is the one rung in the ladder carrying no ask — same defect, one rung down, recorded rather than re-cut here.) One §7 split was forced: beat 0 measured **59 w** against the hard 50, so the lead's chair beat became two. Playwright **68/68** plus 50/50, 42/42, 58/58, 66/66, 46/46, 51/51, 34/34 — and `live_beat_0078.py`'s post-loop seed was corrected to `seated=0`, the faithful state after try 3 burns the part, because at `seated=1` the new capstone auto-fires and both of its hub assertions were measuring the wrong page. Whole-game ratio 2.80 → **2.72 : 1** — the best single move on it this chapter, and the carriage rule is why. Media debt 11 → **12** (one slot, a single `file` and not a pool: a pool on a once-only beat is waste). Nothing committed, nothing released. Prior: **THE LEASH — the leash lets go, and the loop closes (rev 123, 2026-08-10 — beat_0078).** The third and last failure, and the chapter's hinge. **Built: `loop_mercer_attempt.try3`** (7 beats / 6 clicks, no media) plus its routing choice, and **band 3 of `kess_install_part`** — finding 3, *not a man, a set of names; I've been spoofing one; two I can't place; one's Bastien* — which is what arms the Bastien thread. The governor releases, her drain catches on Mercer for the first time in two years, and then **something goes down a list, finds a gap, and shuts the door on the way back**. She gets the *sensation* of a list-shaped check and **Kess gets the diagnosis**, which is §14's rule that she never diagnoses, split across two surfaces without a new canvas. **No new ceiling was signed and none was needed:** the rev-122 row says in its own first line that it governs tries 2 **and** 3, and try 3 spends *less* of it than try 2 did — no clamp, no pain, nothing to conceal, because canon is explicit that a drained man notices nothing (`salvage_session_11`: *"emptied and none the wiser"*). A ceiling is a maximum, not a quota. **The register risk was named and then tested from both sides:** *the leash lets go while a man is finishing inside her* has an obvious wrong reading — that she feels pleasure for the first time — and she does not, because the leash suppresses her **drain**, not her body. So the node quotes the game's own instrumental drain lexicon (*the old pull… it catches… Cold socket, dead as before*) and the suite asserts both halves: that no arousal vocabulary appears anywhere, and that the shipped vocabulary does. **The rung gate widened `lt 2` → `lt 3` and that is the last widening** — at 3 the rung is dark for good and `cap_first_fire` takes the fourth evening as an auto-fire, so *the rung is absent* is now a **terminal** state rather than an in-progress fence. **One claim withdrawn:** the loop canvas's header argued try 3 would need a different exit because it arms the Bastien thread. It does not — the arming rides the routing choice, so `.try3` exits to `mercer_room` at +30 exactly like its siblings. **Two stale annotations corrected** while they were under the pen: the canvas description still described only try 1, and the install's measuring note still read *"bands 0 and 1 … 68 words"*. **An ordering hazard found and recorded rather than built:** blueprint scene 17 is gated `mercer_attempts gte 3`, which is now off by one install — the counter hits 3 on the click, but the set is not *known* until Kess speaks band 3 — and the free fix (adding `part_installed gte 1`) belongs to `beat_0080`. Playwright **50/50** plus 42/42, 58/58, 65/65, 46/46, 51/51, 34/34, with four more frontier assertions retargeted and made self-deriving across the 0075/0076/0077 suites. Whole-game ratio 2.82 → **2.80 : 1**. Nothing committed, nothing released. Prior: **THE LEASH — the loop bites back, and the leash gets its own ceiling (rev 122, 2026-08-10 — beat_0077).** **A NEW CEILING ROW IS SIGNED** (LO's call at this beat): *the governor acting on her body, outside the cell*. Every row in the Content register until now governed a **person's** register — Mercer's what he does, Kess's what his hands do, Bastien's what he says — and none of them reached **her body acting on its own**, which is the whole content of try 2. It was measured before it was drafted: across the entire scenes file there were exactly two pieces of pain-register prose, the cell's overflow (which the captivity re-spec scopes to `captive_room` and says so) and the install stating explicitly that what she feels is *not* pain — so **damage to her outside that room had never shipped**. The row says the clamp is **damage and not sexual** (no forced orgasm, no arousal; the governor is *design, not damage* in `salvage_verdict`'s own words, and a guard bites the way a machine bites); that **the captivity re-spec is not widened** (one event, once — no pack, no restraint, Tier-3 zero); and that **the charge is CONCEALMENT, not pain** — pain as spectacle is the failure mode, and the beat is a woman going quietly rigid under a man who is still inside her and still telling a story about the penthouse. **Built: `loop_mercer_attempt.try2`** (8 beats / 7 clicks) plus its routing choice, and **band 2 of `kess_install_part`** — finding 2, *it isn't a safety, safeties fail open; that's a guard, and a guard means somebody's still using it*, which is the hinge into try 3. **The clamp lands at the HOLD, not mid-stroke** — §9's sketch said mid-fuck, but the shared evening node ends at the finish and the reach *is* the routing click, and the hold is the worse moment anyway because he is still and quiet and paying attention. **The concealment engine** is what lets the scene work without a fail branch: the half of the clamp he can feel is her gripping down on him, which is the one thing he has always wanted from her, so he misreads it as her finally responding. She only has to hide the rest, and he accepts *"it's cold in here"* because looking at her face would cost him an effort he has not spent in two years. **The rung gate widened `lt 1` → `lt 2` in the same edit as the node** — the one thing in this beat that fails silently. Two stale sketches corrected in place while they were under the pen (try 1's *on her back / in her chest*, superseded at `beat_0076`). No new flags, traits, media or quest work; debt holds at **eleven**. Playwright **42/42** plus 58/58, 65/65, 46/46, 51/51 and 34/34 — four frontier assertions in the 0075 and 0076 suites were **retargeted, not deleted**, and three of them rewritten to derive the frontier so they stop needing a hand-edit every beat. Whole-game ratio 2.84 → **2.82 : 1**. Nothing committed, nothing released. Prior: **THE LEASH — the loop spends for the first time, and it fails (rev 121, 2026-08-10 — beat_0076).** The supply side built at beat_0075 now has somewhere to go. **Built: `loop_mercer_attempt`, triggerless off a new hidden rung on `hub_mercer_room`** — a shared evening node (7 beats, 1.25 : 1) that all four generations run through, routing by exclusive choice condition into a per-generation failure node, of which `try1` (6 beats) is authored here. **A change from blueprint scene 13, surfaced rather than patched in:** that row placed the four attempts as one canvas with an exclusive banded `[group]` chain. One canvas is right and stays; the `[group]` chain is not, because each try is four to six beats with its own cascade and §7 check 2 sums the whole node lead into beat 0 — four multi-beat bands parked there would measure as a single ~250-word beat. Per-generation **nodes** give each try its own exit, beat count and word budget. **The burn and the bump ride the routing CHOICE, not a node exit** — the first authoring decision the beat_0075 render-time finding has actually changed: an exit block's effects fire on render, a choice's on click, so the loop's central state transition now belongs to the instant she reaches rather than to her arrival in the room. **The held anal lands here**, and for a reason rather than as an escalation: `mercer_finisher` (Act 1, shipped) already canonises that his anal finish passes nothing to her, so the reach rides exactly the act where her body has spent two years finding nothing — which is what makes try 1's **identical nothing** cruel. **The rung's gate widens one notch per beat** (`mercer_attempts lt 1` here, `lt 2` at beat_0077, `lt 3` at beat_0078) so the loop can never route into a try node that does not exist yet. **Band 1 of `kess_install_part` lands**, discharging the beat_0075 debt: Kess's finding 1 — it isn't a lock, it's listening, and he hit the wrong wire. No new flags, no new traits, and **no quest work** — card I already spans the whole loop by design. Two new media slots take the debt to **eleven**. Playwright **56/56** plus 65/65, 46/46, 51/51 and 34/34 regressions (beat_0075's in-progress assertion about the missing band was **retargeted**, not deleted, since this beat fills it). Whole-game ratio 2.88 → **2.84 : 1**. Nothing committed, nothing released. Prior: **THE LEASH — the parts loop opens, and the Kess ceiling is signed (rev 120, 2026-08-10 — beat_0075).** **The Kess-invasive ceiling is SIGNED** (LO's call at this beat) and gains a third clause at signature: **the install is explicit and it is NOT sexual.** The Salvage row's *"explicit at the ceiling"* was written for scenes with sex in them; this one has none — it is a man's hands inside the seam at the base of her spine while she lies face-down on a bench. So the precision is **anatomical and surgical, not erotic**, with no arousal anywhere on either side, **no pain**, and no comfort: she is **handled**, and that is the horror. She is bare from the hip because that is where the seam is and the scene never treats it as nakedness. And the words stay on **her body**, never on the hardware (Rule 9 — an install scene is the corpus's maximum-risk case for the drain/plate/socket lexicon). **Built: three canvases and the loop's whole supply side.** `kess_print_read` (auto-fire one-shot, 12 beats / 11 clicks, **0.28 : 1** — the most dialogue-dominant canvas in the game): the print *moved* the key and it still isn't a solve, because he has the **shape** of what Mercer's hand said and not the hand, and a shape played back cold reads as a recording. So he builds something that says it live **from inside her**, and the only bench that can test it is the man himself. `activity_buy_part` (25 coin, gated so she can hold exactly one — the **no-stockpiling rule** is what makes the burn bite). `kess_install_part` (**triggerless**, 9 beats / 8 clicks, 1.10 : 1), reached from a new hidden rung on Kess's hub. **`part_gen` is dropped** — it counted the same thing `mercer_attempts` counts, and two counters for one fact is a desync waiting to happen. Quest **card H is re-cut** (its closer moves to `parts_loop_open`, and its *"that's where this build ends"* sentence goes, because this beat ships what it called the next release) and **card I** takes the frontier. `design_the_leash.md` §18's open item is closed: **Kess's three findings ride the install canvas** as bands 1/2/3, not separate bench beats — a second auto-fire at the berth would re-invite the beat_0074 chaining trap. One engine fact verified and recorded: **an exit block's `flagEffects` and `time_progression_minutes` fire on RENDER, not on the exit click.** Verified: build green, four gates at baseline, clean no-dev build 0 missing, §7 checks 1/2/3 pass on both canvases (whole game 2.97 → **2.88 : 1**), Playwright **63/63** plus 46/46, 51/51 and 34/34 regressions. Nothing committed, nothing released. Prior: **THE LEASH — the print, the Mercer ceiling signed, and two engine gates found live (rev 119, beat_0074):** `cap_owner_print` (auto-fire capstone, 14 beats / 12 clicks, **1.01 : 1**) — the controller comes out of her coat pocket and under the hand he is leaning on, and he never looks down at it. The Mercer-nostalgia ceiling was signed with clause (iii), *warm and crude in the same breath*, and the row's "Tier-3 budget: one" line was corrected (explicit spend and beat-count tier are separate axes). Quest card H was mandatory: the exit closed G and G was the last Story-Goal card in the game. Two engine findings bounced up and fixed in the skill: **`requires_npc` does not gate an auto-fire** (`v2.py:4447`) — beat_0073's rung had shipped firing at 12:00 in a padlocked empty room — and **an auto-fire that exits into its own location will chain**. Prior: **THE LEASH — Mercer's two standing surfaces and the nostalgia meter going live (rev 118, beat_0073):** `hub_mercer_room` (portrait hub, presence does the 23:00–08:00 clock), `mercer_end_table` (the bar solo-link, gated `is_absent` from his room so the two surfaces are exact complements, carrying **ask #2**), `mercer_room_offhours` (the daylight guard), and `rung_mercer_hands_on` (auto-fire at `relation gte 12`, carrying **ask #3, the silence**, and stopping at the touch). `mercer_hospitality_open` was re-pointed from a stale plan row — beat_0072 had already spent the invitation in his own mouth — and now means *the arrangement is back on*. Full record in `authoring_state.json` → `decisions_log`. Prior: **THE LEASH — the upkeep CORRECTED, and it bounced up from the build (rev 114, 2026-08-10 — beats 0068 + 0069).** Two of the folded design's assumptions did not survive contact with the engine and with its own prose, so both bounce UP here rather than getting worked around in TOML. **(1) A "paid through day N" trait is NOT AUTHORABLE.** Effect values are **literals only** (`_resolve_effect_value`, `v2.py:13519-13578` — a number or a `random` dict, and anything else raises; the runtime path turns a string expression into `NaN`→`0`, a **silent no-op with no build error**), and **no condition type reads the clock** — of the 16 types in `triggerConditionsSatisfied` (`v2.py:3816`+) only `days_since_flag` touches `time_state.day`, and weekday gating exists only as a trigger *schedule*, never as a condition. So `rent_paid_through` is renamed **`feed_line_days`** and re-shaped as a **decaying counter**: a literal 3 set by the paid night, decremented 1 per day rollover by `[player.trait_decay]` (`v2.py:5532-5544`, floors at 0), read by an ordinary `trait gte 1`. **(2) The cadence is PER NIGHT, not weekly.** The rev-112 fold said "a weekly rent" in four places; the beat_0068 build then put the terms in Kess's own mouth as *"You pay per night, in coin… Coin dries up, so does the arrangement"* (`5_scenes.toml:5214`) with Wren thinking *"rented by the night"* (`:5216`), and the cot's 10-coin tier plus quest card E (*"Keep the cot paid"*) both shipped on that cadence. Rather than write a **second** bill the terms scene never named, **the paid night IS the upkeep** — 10 coin buys three days of Kess's time on the key — which drops the planned `activity_pay_rent` canvas entirely and makes the fiction and the mechanic finally say the same thing. **What replaces it:** `hub_kess_berth`, the portrait hub the berth has never had post-close — `salvage_repair_hub` is gated `salvage_done is_false` and is the ONLY canvas in the game carrying `npc = "npc_kess"`, so once the verdict fires Kess stands at his 10:00–22:00 schedule as pure scenery with no way to speak to him at all. The new hub bands two exclusive `[group]`s on `feed_line_days` (working / stalled), and **Kess is the read-out — no sidebar row**, per LO's 2026-07-16 call retiring the `Leash: Uncut` line (*the reveal lives in the scene, not as a standing sidebar line*). Pricing: ~23 coin a week against a brothel finisher at +10, the pit at +8/+20, Marsh at +15 — one pit win and a client. Also verified and recorded in §16 of `design_the_leash.md`: the engine `[settings.rent]` system stays off for **measured** reasons, not stylistic ones — its due-day intercept hard-redirects every `Location_*` and `Navigation` passage to `RentDay` (`v2.py:15245-15255`), which blocks her from *acting*; it charges `money`, not `coin`; and `backfillStateDefaults` has no `game_state` branch, so enabling it on a shipped title leaves **every carried save silently rent-free forever** while fresh players get it. **beat_0068 (rev 113) shipped green ahead of this** — the five shelved 1b blocks restored from `08ec2e1`, `reach_safehouse` renamed `berth_home` (0 hits in every release, so free), the D-card exclusivity clause restored, `berth_offhours` widened, and a five-line prose-truth sweep on `cap_1a_close`; **the live post-ending dead-end in 0.1.7 is fixed.** Prior: **THE LEASH (Act 2 · 1b) — design folded into the book (rev 112, 2026-08-09 — see `## The Leash` + `### The Leash blueprint`)** — the chapter the shipped 0.1.7 end card promises the player **by name** (*"cracking the key — cutting your leash, opening the file, and finally reading what you are — plus what becomes of Mercer loose in the Reach"*, `5_scenes.toml:7465`), and the one that repairs a **live defect in the shipped build**: every Charge restore is at the `cradle` (`3_activities.toml:260`/`:291`) and the Spire seals permanently at `archive_1a_done` (`3_activities.toml:102-110`/`:180-186`), so the post-ending player has **no bed and no charger** — the fix was written (`08ec2e1`) then cut with 1b (`187f521`) and is increment 1 here. Authoritative design: `games/vesper/design_the_leash.md`; it **supersedes** the three-bullet §12 sketch in `design_beat_archive_v2.md` (banner-marked in place), keeping its destinations and replacing its shape — **there is no "deal" scene**, because the controller trade already happened in the 1a close. THE SPINE: she rents a bunk on **Kess's feed line** (her charger now, and she **pays weekly rent** for it — an authored `coin` sink, not the engine rent system; missing a week **stalls Kess's work**, with a free crash-rough floor so the player can never brick themselves) → he reads the file and gets its **shape** (build documents, a **programme**, **eleven years**, many subjects, dates in two columns) but **her own page is sealed in the same hand as the controller** — *the file and the key are one lock* → he needs **the owner's hand print** → **Mercer resurfaces** at the Undertow under a **new name** (a black-market stall selling **Spire paper** — the one craft he has, since he *signed for her*), **delighted** to see her, asking her *quietly* to stop using the old name — **and she doesn't stop**, no defiance, no smirk, just says it again in the next breath; he asks three times across the visits, quieter each time, then **stops asking** (she is a walking security hole in his new life, so he keeps her close — **a leash neither of them names**) → she takes the print → **the print is a start, not a solve** → **THE LOOP** (the heart): **buy a part** at the market (`coin`) → **Kess plants it in her** → she goes to Mercer, lets him have her exactly the way he always did, and **tries to fire the thing hidden inside her while he is on top of her** → it fails → **the part burns** → she finishes, smiles, leaves, and he never notices → earn → repeat. **THREE failures, then the win, each failing DIFFERENTLY** — *nothing at all* (it isn't a lock, it's **listening**) → *it bites back*, the governor clamps and her body locks up mid-fuck and she has to hide it from a man inside her (it **defends itself** — a guard, not a safety) → *it works for a second* and snaps back, and she has **tasted** it (it was never keyed to a **man**, it's keyed to a **SET OF NAMES**, and he's been spoofing one) → **it holds**. **THE FIRST FIRE:** she has stopped expecting it; it catches; she drains **her owner** for the first time in her life (control canvas, Q&A in HIS dialog per the carriage rule) — and the payload is **the bad news: the key only ever spoke for one name.** She isn't free; she's carrying a hall pass. **BASTIEN runs parallel and is NOT optional** — his name comes off that set (a list older than she is, with underworld names on it → **she wasn't fitted with a leash, she was built around one that was already there**), so she walks back into the bar of the man who kept her in a room; he **strips her at the door** every visit (he took her drain in the cell, `design_captivity_the_room.md:61`), so the arc is **smuggling, and her body is the bag** — worse now, because she must get **two** things past him, the drain AND the controller. His drain answers **why he was buying her build file: for Cain.** **THE EXTRACTION** closes it — a live key lets Kess hold the governor quiet while he cuts (the thing his Salvage line ruled out), the chip comes out, and **her own page opens with it**. **LO's calls, locked 2026-08-09:** she does NOT serve Mercer (the close paid that off — every visit is her walking in on her own feet, *humouring* not obeying) · Mercer never learns anything, ever, and gains a `relation` odometer **re-cut as NOSTALGIA** (it buys hospitality access, never register — he is at his ceiling from beat one) · the controller is a **carried switch with a state** (`equipped_weapon`/`drain_charge` shape; *on* = running = leash suppressed) that surfaces **only on blocked targets**, so no shipped content gains a new step · the parts are a **TRAIT counter, never an inventory item** (the backfill has no wardrobe branch — the `cover_analyst` post-mortem) · the four attempts are **ONE canvas with an exclusive banded `[group]` chain**, since adjacent groups merge · Bastien's ceiling stays **cell-scoped** · Cain, the Chairman, Vane and `the_site` are **reserved, not spent**. **Three ceilings are DRAFTED BUT UNSIGNED** (Kess-invasive · Mercer-nostalgia + the drain inversion · Bastien-outside-the-cell) — **no hot beat ships until LO fills them** (`kink-ceilings.md` §2). Save-safety: **ADD-ONLY**; the one gate change (`kess_berth` drops its `salvage_done is_false` clause) **opens** a shut door and *heals* every 0.1.7 save stranded on the strip. Design/story folded; blueprint placed (`beat_0068–0086`, all `status=planned`); **no TOML yet.** Prior: **THE ARCHIVE 1a — REDESIGNED (v2) + REWORK STARTED (rev 72, 2026-07-22).** The whole 1a discovery was rebuilt with LO: **copies-not-theft** (Vane, a trusted teammate, copies files; pipe Vane→Colm [off-record courier]→Bastien; nothing missing = why 2yr unsolved). Detection via the **case-work + a cold Colm drain** (no gadgets); **docs = her build file** (Vance humanoid records, she's the guinea pig — 1b reveal); **blackmail Vane to retrieve**; the **watcher-flush + Mercer-panic** ending. Authoritative design: `games/vesper/design_beat_archive_v2.md` (supersedes `design_beat_archive.md`; the `### The Archive blueprint`/`wiring`/`## The Archive` sections below are the OLD design, banner-superseded + kept for reference). Save-safety CLEAR (Archive never publicly shipped — releases stop at v0.1.3). Ledger reconciled (plan[] 0056-0064 statuses; +beat_0066 Colm / +beat_0067 blackmail; 0060+0063 removed). Prior: **THE ARCHIVE (Act 2) — design fold IN PROGRESS (Step-5 blueprint, propose-first; rev 69).** The chapter after Salvage, built **fresh** from the shipped 0.1.4 end-state (leashed, paid-clean, Mercer wary); the old Phase-1 Calloway/Site/Cain ladder is **set aside** for the forward story. Full locked spine + cast + rules + canon anchors: `games/vesper/design_beat_archive.md`. Split into two shippable chunks — **1a · The Archive** (topside: infiltrate Calloway's un-indexed file room, plant a doc-bug, seduce+drain, discover the big theft, get burned — **build first**) and **1b · The Deal** (underworld: the deal, **Vane** the hidden mole + **Bastien** the buyer, read the build file, trade Mercer his escape for the leash key). Two NEW NPCs: **`npc_calloway`** (the mark → nemesis) + **`npc_vane`** (Calloway's mole → her kept asset). **Pass 1 (player thread) folded:** the **used→user** turn now ADVANCES — she takes the **leash controller** (1b) but stays leashed (still-Failing core + needs Kess); the controller is **purely in-fiction** (no meter/marker — LO); her **build file ("what she is")** is set up in 1a (the discovery it was stolen) and read in 1b (Tier-3, rationed); the Calloway seduction is a **cold belief-lever** (no warmth breached — reserved for Cain). **All four passes folded** (player · Calloway/Vane on the belief-lever · world + the Bastien-overhear (Fork B) · wiring); **plan seeded `beat_0056`–`beat_0065`** (1a; Step-7 authoring next — the scaffold first). Forks A + B locked with LO 2026-07-21. See `## The Archive` + `### The Archive blueprint`. Prior: **SALVAGE — THE REPAIR BRIDGE, design folded into the book (rev 65 — see `## Salvage — The Repair` + `### Salvage blueprint`)** — the chunk after captivity, and the one that PAYS the frozen `Core: Failing` promise. Wren wakes broken and, for the first time, seeks help OFF the books (a company refit would WIPE her): **KESS**, an off-books dockside synth-mechanic, repairs her in **STAGED** fixes, each proven on a **brought body — a weapon-test** (Tolly = the charge/core test · Reeves = the drain-test whose payload drops the **Calloway file-room** lead — the test IS the intel beat). New verb = **supplicant / test-bench** (the first NON-conquest on the roster). The rescuer hook pays off through a **cold channel** — Kess reads the re-seated drain (*"somebody who'd had their fingers in you before, who didn't need to look"*) + a once-only **Tier-3 glitch-leak** that reuses `beat_0042`'s exact devices so it reads as the *same* memory surfacing — with Cain never on-page, **"Vesper" off-page**, the alliance unreportable, the Site closed. Meter **FLIPS**: `core_strain`→0 (Failing clears — the renderer emits nothing at no-match) + a new hidden `core_sealed` driving a **"Core: Locked"** row (the sealed partition only the Site opens; honors "no cure shipped"). She walks **owing** a growing `kess_debt` (untraceable `coin`, never company Credits — the broke→rich seed; Kess recurs) and re-launches at Mercer's **Mission-3 (Calloway)** order, private want + company order now at one address. **Six forks LOCKED with LO 2026-07-15** (visible flip · staged weapon-tests · Kess recurs · walk owing · awakening KEPT STILL · Reeves names Calloway). **Grind-guard:** staged scenes are count-locked + `is_repeatable=false` — staged-and-distinct is the point, a repeatable loop re-ships the review. Extend-only (new: `kess`/`kess_berth`/`core_sealed`/`kess_debt`/`salvage_*`). Design/story folded; blueprint placed (`beat_0046–0055`; 0045 is the parked captivity undo-hatch); no TOML yet. Prior: **CAPTIVITY SCENES → DEEP, HOT, PURE-WITNESS CHAINS (rev 64 — TOML build)** — the eight cell shelf scenes (+ the `cell_sleep` night-use) rewritten from flat one-clip captions into **scripted multi-beat, pure-witness chains** (approach → strip → invasive inspection → the acts → finish → read-out; advanced on `Continue`, one clip per beat, no player agency — *rape doesn't start at penetration; the helpless click is the point*). The captivity ceiling is **RE-SPEC'd on LO's three calls**: Bastien **and** the crew are now **hot and degrading** (the earlier *clinical / operates-not-degrades* ceiling struck; the crew's register decoupled into a jeering pack); the floor **widened** to spit/drool/face-use · marks & rough handling · piss/heavier degradation; and *damage-to-the-machine-not-the-meat* loosened to permit **surface marks** (the core-break stays the only lasting damage). Her interior stays **rationed to one `thought_bubble` per scene** — the loud degrading room around a flat blank is the horror. Media = **full clip-per-beat depth**, ~30–40 NEW clips = the open find-media harvest (scenes build with `[VIDEO MISSING]` until filled). Doctrine gap flagged for skill capture: the `author-game` skill has **no pure-witness / no-agency scene pattern** (the reason they shipped as captions) — write it back as a `sex-loop.md` counterpart once the pattern proves out. Prior: **CAPTIVITY — THE ROOM (Act 2 · Bastien's arc, part 1) — the STORY folded into the book (see `## Captivity — The Room`)** — the chunk where the mopoga *"grind not content"* cure fully lands: the densest erotic stretch in the game, zero grind, and the third distinct conquest verb (**she is taken**). Bastien's grab no longer releases her — it ports her into a **sealed cell** with two verbs (sleep · attend), where he and a **faceless crew** use her across three days while `core_strain` climbs (sex *charges* her; he stripped the drain, so the input has nowhere to go and the core cooks). At the break, **Cain** arrives, argues with Bastien behind a door **we never show**, and frees her without saying why. She leaves with her gear back and a fault that does not heal — the `Core: Failing` row in her sidebar is the promise the repair chunk pays. Two ceilings **declared** this revision (both previously blank): **Bastien's vocab register** — *ownership-as-curiosity*, maximum from the first scene, clinical rather than hot (he doesn't degrade her, he **operates** her) — and a scoped **RE-SPEC of the non-con floor**, which until now read *"no torture/mutilation unless re-specced."* Inside the cell the ceiling lifts to restraint/BDSM, gangbang, sustained non-con, and her machine broken; **the damage is to the machine, not the meat**, and the lift is scoped to the cell only. Design/story only — no TOML this revision; the blueprint (the named scene table) is next. Prior: **UNDERWORLD HUNT — Act-2 on-ramp, DESIGN landed (Steps 3+5 — casting + blueprint)** — the fix for the mopoga *"grind not content / lacks content"* review, whose real cause was **roster**: only 2 developed NPCs and Act 2 designed-but-unbuilt, so the story ran out of people. Locked the **Underworld Hunt** (the trail that *reveals* Bastien) and did its casting. Beats: Renner's drain will hand her **two dead men's names** (Doss + Rourke, NEW) → **Sol promoted from placeholder to the informant** (resolves his add/cut fork) → the fixer **Marsh** (one-off drain) books **Rue** at The House on Sundays → she pays Rue off the slot (the underworld coin's first real *use*), serves + drains Marsh → the crew's place → finds **Bastien**, whose people **grab her**. **Bastien re-cast**: dealer→owner (bar + brothel), submissive-cover→**capture-and-flip**, supply-transaction→secret Cain-alliance (saved reveal). His *arc* (flip/drain) = next chunk; this one only reveals him. The underworld is a **breadcrumb, not a new grind** (no second currency; every stop is a scene). Casting table + sketches reconciled; deeper "submissive dealer" arc-refs marked superseded-pending-Step-4. **Blueprint seeded (Step 5): 5 scenes (plan beat_0030–0034) gated on `names_known` (a TRAIT — triggerless drain setter) + a located flag chain (`hunt_lead`/`crew_known`/`bastien_found`); Sunday via a canvas `trigger.schedules weekdays=[6]` (NOT an NPC schedule row — spoiler); the grab = an auto-fire capstone that ends the chunk; one new room `crew_den`; Rue pay-off = the coin's first real sink. Design-only; Step-6 quick-check next; no TOML yet.** Prior: **RENNER QUEST → STEPPED CORRUPTION LADDER** — the single Renner objective (*Break him to the drain — N / 50*) is replaced by a chain of corruption-banded quest cards that name the CURRENT lever, so the goal finally matches the real stepped unlock (tease 0 / flash 10 / grope 20 / first-oral 30 / loop 40 / anal-drain 50) instead of hiding it behind one far-off number. Six rungs, one live at a time — the picker swaps as `npc_renner.corruption` crosses each band; ranges are exclusive via `gte`+`lt` so exactly one matches: R1 tease → R2 flash → R3 grope → R4 he-cracks → R5 break-to-50 → R6 *take him to bed* (the 50-but-not-yet-bedded step, the only numberless rung — it also closes a pre-existing blank-*next*-row dead-spot where a met goal + no `ready_canvas` rendered nothing, engine `renderQuestsGoalBlock` Frame 3 v2.py:13988). The **sidebar *next* row and the Quests page are the same renderer** (`pickQuestsCard`→`renderQuestsGoalBlock`, v2.py:15137-15145) — there was never a separate "sidebar quest" to align; both step together for free. Per LO: **keep the number** (each rung shows *N / next-threshold*; the coaching verb rides in the goal LABEL because the sidebar renders only the goal block, not text/tip). READY pair + Story-Goal tier untouched. Authoring-only, zero engine change; build green (no new flags → flag chains still valid) + 28/28 headless live-test. Prior: **DRAIN FOLDED INTO THE HUB SEX LOOP + underworld lore** — the first Renner drain is no longer a separate cold auto-fire anal scene (retired `cap_renner_anal_drain` + its standalone clip `sex/renner_anal_t5.webm`). It now happens INSIDE the office sex loop like every repeat: oral→vaginal→anal, and the **anal finish IS the drain** — she builds up instead of jump-cutting straight to anal. It's **player-driven** (no longer force-fired) and gated on the drain weapon being **worn** (`equipped_weapon=1`) + **charged** (`drain_charge>=1`); carry the emitter or run dry and the anal still resolves but doesn't drain (reload at the cradle). **First** drain = the full Q&A extraction; every **repeat** = the short "ask again." The anal pose opens on his corruption≥50 + in cover (was gated on the retired first-anal flag). **Lore swap:** the drain no longer names the two unbuilt targets (Bastien/Calloway) — Renner now confesses the real feud (two of his own people betrayed him, feeding Cain supplies off his own floor; he killed them and their families; Cain torched his yard for it) and points her at the **underworld** (already built + reachable off the Waterfront), converging with the burned-yard evidence thread (the two dead men + the underworld origin). *(D2: the broader Bastien/Calloway Mission 2/3 references elsewhere in this book stay as future-frontier intent, now underworld-first — reconcile when those targets are built.)* Engine note: the post-drain awakening (glitch II) was re-gated onto a new hidden `drains_done` trait counter so the flag-chain validator doesn't demand a located setter (the drain now fires in a triggerless loop). Authoring-only, zero engine change; build green + 22/22 headless live-test. Prior: **EMITTER-FIRE VIDEOS SPLIT** — the two arousal-weapon fire beats no longer share one clip: the **gate** doorman keeps `sex/arousal_weapon_use.webm`, and the **burned-yard** area-fire now points at a new `sex/yard_emitter_fire_t5.webm`. The prose already differed (one bored doorman vs. a knot of men going slack); now the video does too. Engine silent-skips the missing yard clip → the yard beat shows its prose + the find-media search-links until the clip is harvested (the slot joins the next find-media pass). Authoring-only, one TOML video block retargeted, gate left as-is. Prior: **WEAPON LOADOUT — one weapon carried at a time** (new hidden trait `equipped_weapon`: 1 = drain, 2 = emitter; one core feeds one weapon). She swaps at the bench in her room (*Switch weapon*, free); which she carries gates use — the drain only fires the Renner extraction, the emitter only zaps guards (yard/gate). The **first drain is not exempt** — the capstone trigger waits until the drain is carried, and the quest page directs her (flipping to "switch back to the drain" if she's on the emitter). The **two weapon reloads are now separate dedicated cradle actions** (*Recharge the drain* / *Recharge the emitter*); **Power down / Charge up no longer reload weapons** (Charge only). Authoring-only, zero engine change (a literal clothing-slot weapon was engine-blocked — `VALID_CLOTHING_SLOTS` is hardcoded to 7). Prior: **BROTHEL → FULL SEX LOOP** — The House promoted from a single-variant repeat to the Renner/Mercer pose-ladder shape (oral/vaginal/anal → his pleasure climbs → elected finish: mouth/inside/ass), and **coin moved to finish-only** (it was paid on the entry choice — a faucet). Two new triggerless canvases (`underworld_brothel_loop` + `underworld_brothel_finisher`), authoring-only, cold register kept. 5 new t5 clips pending find-media (engine silent-skips → plays text-only meanwhile; vaginal reuses the existing ride clip). Prior: **LOCKED-CHOICE LABELS — the 4 Renner depot seduction rungs now show their own action label greyed when locked** (was distinct prose via `locked_text`), so the locked and unlocked text agree and it matches the energy gate's "(Requires 15 …)" look. Per LO (Option A): bare same-label, no hand-written requirement — the engine can't auto-derive a reason from `conditions` (only resource `costs` do, via `getCostBlockedMessage`). The 7 sex-loop/serve finishers keep their prose `locked_text` (reads better mid-scene). Prior: **ONBOARDING (audit P3, minimal)** — coin is now established in-fiction at the Underworld gate as closed underworld-only scrip (earned/spent only down there, worthless up top); this also explains why the toll can't be paid on a cold first visit. Per LO, the other P3 touches were dropped (the sidebar already surfaces Condition/Fighting/Stealth/Coin from frame one). Prior: **CLEANUP (audit P5 A+B)** — removed the dead `underworld_known` flag (set, read by nothing — the Underworld shipped reachable) and corrected the stale "underworld is deferred" notes to "reachable" (its deep end / Cain still locked). Prior: **PROSE PASS — Underworld sex rewritten to a declared cold-but-explicit register** (graphic at the ceiling, transactional/detached — the opposite of the Renner/Mercer heat; see *## The Underworld*), and the two interior tics ("files it under nothing" / "the way she does everything") **rationed** to their load-bearing beats. Prior: **BUGFIX — Condition (hygiene) was over-capping past 100 (Wash) and going negative (sex drops)**, both making the banded Condition card vanish; clamped all 9 hygiene effects so Condition is bounded 0–100 like Charge (the Charge-bug twin the content-depth audit caught; see *## Reset & reload*). Prior: **Quests restructured into two tiers** — the top **Story Goals** carry the mission (Mission 1 + the Burned Yard); **Renner's own section** carries his seduction as a one-card-at-a-time chain (*Earn the office* → *Break him to the drain*), which also lights the Renner sidebar panel's *next* row (see *### Quests page — two tiers*). Prior: **BUGFIX — Charge (energy) was sliding negative**; spends moved onto gated `costs`/clamp so Charge is now bounded 0–100 and the rungs/work block when too tired (travel never blocks; see *## Reset & reload*). Prior: the **arousal weapon's use is now a real beat** — arouse → fuck → he *passes out* (non-lethal) → she slips by; it's sex, so it drops Condition (see *## Reset & reload*). Prior: **reset & reload** (Condition + the two weapon reloads + Charge up); **The Underworld** (coin economy / second life); The Burned Yard; the Training activity; the Renner grind retune.)
