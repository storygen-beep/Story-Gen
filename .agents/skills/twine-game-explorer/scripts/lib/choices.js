...
function filterToContentRegion(items, {
  sidebarRightEdge = 270,
  frameWidth = null,
  frameHeight = null,
  rightSidebarStart = null,
  yMargin = 40,
} = {}) {
  ...
  const kept = items.filter((c) => {
    ...
    // Off-screen y-bounds — only drop when the element ALSO lacks text.
    // Text-bearing elements can be reached by Playwright's locator-by-text
    // click path (which auto-scrolls into view), so we keep them even when
    // they're currently off-screen. Only text-less elements are hopeless
    // offscreen — coord-click is their only path, and it misses.
    const hasClickableText = c.t && c.t.trim().length > 0;
    if (!hasClickableText && c.y < -yMargin) { reasons.push({ t: c.t, reason: 'offscreen_above_notext', y: c.y }); return false; }
    if (!hasClickableText && frameHeight != null && c.y > frameHeight + yMargin) { reasons.push({ t: c.t, reason: 'offscreen_below_notext', y: c.y }); return false; }
